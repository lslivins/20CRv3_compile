...
typedef struct NewFeedBack {
   ...
   double            mpmemfg;   // Ensemble Mean First Guess Pressure minus Modified Observation Pressure
   double            emap;      // Ensemble Mean Analysis Pressure
   double            sdeap;     // Standard Deviation of Ensemble Analysis Pressure
   double            mpmema;    // Ensemble Mean Analysis Pressure minus Modified Observation Pressure
...
typedef struct SrcMarine {
   ...
   int               dck;           // Deck ID 
   int               pt;            // Platform
   char	           orig_hr[4];      // hour UTC in hundredths of hour
   char	           uid[6];	    // base36 Marince Unique ID 
   char	           rn1[1];	    //Release no.: primary 0 Z base36
   char	           rn2[1];	    //Release no.: secondary 0 Z base36
   char	           rn3[1];	    //Release no.: tertiary 0 Z base36
   int	           rsa;		    //Release status indicator 0 2 0=Prelim., 1=Aux., 2=Full
   int	           irf;             //intermediate reject flag 0=Reject, 1=Retained
} SrcMarine;
...
typedef struct Hurricanes {
   ...
   int               d2l;            // distances to land km
   int               lnd;            // Minimum distance to land between current location and next.
} Hurricanes;
