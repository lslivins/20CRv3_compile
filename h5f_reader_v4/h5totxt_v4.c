...
int main (int argc, char *argv[], char *envp[]) {
    ...
    if (argc < 2){
        fprintf (stderr,"Usage: h5totxt_v4  <analtime>\n");
        exit(1);
    }

    if(!(datapath = strdup(getenv("obsdirh5")))){
...
void print_dat (char* filename, int analtime) {
    ...
    char ST[DIM];
    herr_t status;
    hsize_t nfields_out, nobs, nobstype, nsrcmar, ntrack, nmeta;
    ...
    nfields_out = nobs = nobstype = nsrcmar = ntrack = nmeta = 0;
    ...
    status=H5TBget_table_info (h5file,"/SupplementalData/Tracking/ICOADS/TrackingICOADS", &nfields_out, &nsrcmar);
    ...
    srcmar = malloc(sizeof(SrcMarine)*nsrcmar);
    ...
    get_srcmarine(h5file,srcmar);
    //Supplemental marine data is only for Marine need to check unique id to match up records
    // for each data
    for(i=0; i < nobs; i++) {
        char obid[20];
        ...
        else if (obstype[i].ncep_type >= 194 && obstype[i].ncep_type <= 199){
            ob = observations[i].slp;
            pserr = OBBERDICT_ZERO;
            elevation = 0;
        }
        ...
        //first set the marine source and deck to missing
        ...
        if((strcmp(srcmar[j].unoc, observations[i].unoc) == 0 )&& (strcmp(srcmar[j].timestamp,observations[i].timestamp)== 0)) {
             sid = srcmar[j].sid;
             dck = srcmar[j].dck; }
         }         
        printf ("%-19s %3i %01c %7.2f %6.2f %5i %6.2f %8.2f %8.2f %5.2f %30s %13s %5i %5i\n",obid,obstype[i].ncep_type,obtype,meta[i].lon,meta[i].lat,elevation,analtime_offset,ob,observations[i].slp,pserr,sname,meta[i].id,sid,dck);
