...
void get_hurricanes (hid_t loc_id, Hurricanes* dat){
    herr_t status;
    size_t field_offsets[NFIELDS9] = { HOFFSET( Hurricanes, timestamp ),
    ...
    size_t dset_sizes[9] = { sizeof(char)*13,
                            sizeof(char)*8,
                            sizeof(int),
                            sizeof(int),
                            sizeof(float),
...
herr_t append_srcmar(hid_t loc_id, SrcMarine* dat) {
    ...
    size_t dst_offset[NFIELDS5] = { HOFFSET( SrcMarine, timestamp ),
	                           HOFFSET( SrcMarine, unoc ), 
	                           HOFFSET( SrcMarine, sid ), 
	                           HOFFSET( SrcMarine, dck ), 
                                   HOFFSET( SrcMarine, pt )};
...
herr_t append_hurricanes(hid_t loc_id, Hurricanes* dat){
    ...
    size_t dst_offset[NFIELDS9] = { HOFFSET( Hurricanes, timestamp ),
                                   HOFFSET( Hurricanes, unoc ),
                                   HOFFSET( Hurricanes, dir ),
                                   HOFFSET( Hurricanes, trm ),
                                   HOFFSET( Hurricanes, trmm ),
...
herr_t make_srcmar_table(hid_t loc_id, hsize_t nrecs, SrcMarine dat[]) {
    ...
    /* Define field information */
    const char *field_names[NFIELDS5]  = 
        { "Timestamp", "Unique Observation Number", "Source ID", "Deck ID", "Platform Type" };
    ...
    status=H5TBmake_table( "TrackingICOADS", loc_id, "TrackingICOADS",(hsize_t) NFIELDS5, (hsize_t)nrecs, dst_size, 
                            field_names, dst_offset, field_type, 
                            chunk_size, fill_data, compress, dat );
    return status;
}
...
herr_t make_hurricanes_table(hid_t loc_id, hsize_t nrecs, Hurricanes dat[] ) {
    ...
    /* Define field information */
    const char *field_names[NFIELDS9]  =
        { "Timestamp", "Unique Observation Number", "Maximum Sustained Wind Direction", "Translation Speed in Miles/hour", "Translation Speed in m/s", "Translation Speed in Knots", "Wind Speed in Miles/hour", "Wind Speed in m/s", "Wind Speed in knots" };

    hid_t      field_type[NFIELDS9];
    hsize_t    chunk_size = 10;
    int        *fill_data = NULL;
    int        compress  = 1;
    herr_t     status;
    ...
    field_type[8] = H5T_NATIVE_INT;
     ...
     status=H5TBmake_table( "TropicalStorms", loc_id, "TropicalStorms",(hsize_t) NFIELDS9, (hsize_t)nrecs, dst_size,
                            field_names, dst_offset, field_type,
                            chunk_size, fill_data, compress, dat  );
    return status;
}
...
void make_tables (hid_t* file){
    ...
    // ISPD Version
    status = H5LTmake_dataset_string (*file, "ISPD_Format_Version", "10.11");
    // ISPD 2.2
    status = H5LTmake_dataset_string (*file, "ISPD_Version", "2.3");
...
herr_t process_dataindex(hid_t h5file){
    ...
    DatasetIndex p_data[NRECORDS_DSETINDEX] = {{"0100","ICOADS Release 2.1","Global Marine Surface Observations","1784-2002","1170","DS540.0","Scott.Woodruff@noaa.gov",""},
        {"0104","ICOADS Release 2.4","Global Marine Surface Observations","1784-2002","","DS540.0","Scott.Woodruff@noaa.gov","" },
        {"0105","ICOADS Release 2.5","Global Marine Surface Observations","1784-2002","","DS540.0","Scott.Woodruff@noaa.gov","" },
        {"0200","ICOADS Auxiliary Kobe","Global Marine Surface Observations","1889-1943","","DS530.0","Worley@ucar.edu","" },
        {"0300","ICOADS Auxiliary Whaling","Global Marine Surface Observations","1950-1984","","DS530.0","Worley@ucar.edu","" },
        {"0400","ICOADS Auxiliary Russian","Global Marine Surface Observations","1950-2000","","DS530.0","Worley@ucar.edu","" },
        {"0500","ICOADS Auxiliary Russian","Global Marine Surface Observations","1950-2000","","DS530.0","Worley@ucar.edu","" },
        {"0700","ICOADS Auxiliary Challenger","Global Marine Surface Observations","1872-1876","","DS530.0","Worley@ucar.edu","" },
        ...
        {"1002","CDMP SAO","US Land Surface Observations","1928-1948","3851_99","DS506.0", "Joe.D.Elms@noaa.gov",""},
        {"1003","Russian Empire Staions","Russian Land Surface Observations","1849-2000","td9290c","", "Pasha.Groisman@noaa.gov",""},
        {"1004","Air Weather Servcie TD13","Global Land Surface Observations","1901-1973","td13","DS467.0", "worly@ucar.edu",""},
        {"1005","Hadley Center","Individual Stations from Hadley Center","1833-present","", "","rob.allan@metoffice.gov.uk",""},
        {"1006","CDMP-International","Chile,Mexico,Uruguay","1800s-1980","", "","Tom.Ross@noaa.gov", ""},
        {"1007","READER Antarctic & Southern Hemisphere","20 Stations via British Antarctic Survey","","", "1947-2007","www.antarctica.ac.uk",""},
        {"1010","DATSAV","US Air Force Compilation","1967-1980","", "",""},
        {"1011","KNMI","KNMI Stations","","", "","Theo.Brandsmal@knmi.nl"},
        {"1012","CDMP Forts","US Army Signal Service and other 10th C Voluntary Obs","","", "","Karen Andsager, andsager@sws.uiuc.edul"},
        {"2000","NCEP-NCAR BUFR Archive","Global Observations","1924-2003","6148_99","ds090.0", "Robert.Kistler@noaa.gov, worly@ucar.edu",""},
        {"2001","NCEP Operational BUFR Archive","Global Observations","1928-1948,2003-2005","6148_99","ds090.0", "Jack.Woolen@noaa.gov",""},
        {"3002","WASA Stations Observations Sea Level Pressure","Northern Europe, Greenland","1871-1996","9941_99","", "Torben Schmith <ts@dmi.dk>",""},
        {"3004","Environmental Canada Pressure Observations","Canadian Stations","1842-2005","","", "Xiaolan.Wang@ec.gc.ca",""},
        {"3005","West African Synoptic observations","11 West African Land Surface","1850-1980","","", "Tom.Peterson@noaa.gov",""},
        {"3006","The Australian Bureau of Meteorology Station Pressure Dataset","50 Australian Land stations","1900-1956","","", "David.Jone <D.Jones@bom.gov.au>",""},
        {"3007","Northern Italian Pressure Observations","","","","", "Maurizio.maugeri@unimi.it",""},
        {"3008","Brazil Surface Observations","","1951-1980","", "DS.486.0","dattore@ucar.edu",""},
        {"3009","Spanish Hourly Pressure Observations from EMULATE","4 Hourly Spanish Land Stations","1850-2003","", "","Manola.brunet@urv.net",""},
        {"3010","German","DWD Web Archive","1876-2002","","","www.dwd.de",""},
        ...
        {"3013","South Africa","South African Weather Service Stations","1850-2003","", "","Andries.kruger@weathersa.co.za",""},
        {"3014","Norway","22 Stations","1863-2007","", "","Oyvind.nordli@met.no"},
        {"3015","Croatian Meteorological and Hydrological Service land stations","4 Croatian Land Stations","1858-2005","","","srnec@cirus.dhz.hr",""},
        {"3016","Portugal","Portugese SIGN Stations","","","","mavalente@fc.ul.pt",""},
        {"4000","Hong Kong Hourly Pressure Observations","Hong Kong Observatory","1885-1939","","", "",""},
        {"4001","Jakarta/Batavia Pressure Observations","Dutch Royal Observatory","1866-1944","9963_99","ds490.0", "spangler@ucar.edu",""},
        {"4002","Liverpool","Proudman Ocean Lab Stations","1768-1793","", "",""},
        {"4003","Jersey, Channel Islands Pressure Observations","4 Channel Island stations","1864","","","leblancq.f@jerseymet.gov.je","" },
        {"4004","CMDP-USNO","US Naval Observatory at Washington","","","","tom.ross@noaa.gov","" },
        {"5002","Byrd Antarctic Expeditions Observations","Monthly Weather Review Supplemental No. 41","1929-1930","","", "Gil.Compo@noaa.gov",""},
        {"5xxx","Antarctic Expedition keyed by Hadley Centre","","","","", "Rob.Allan@metoffice.gov.uk",""},
        {"6xxx","Antarctic Expedition keyed by Hadley Centre","","","","", "Rob.Allan@metoffice.gov.uk",""},
        {"010000","NCAR Upper Air Stations","","","", "",""},
        {"8000","Atlantic/North Eastern Pacific Hurricane Reanalysis","US Hurricane Re-Analysis Dataset","1851-2006","","", "Chris.Landsea@noaa.gov", ""},
        ...
        {"7000","University of Arizona/ETH Zurich Upper-Air Dataset","European Upper Air Observations (Radiosonde, Aircraft)","1939-1944","","", "stefan.bronninmann@env.ethz.ch",""}};
...
herr_t make_datasetindex (hid_t loc_id) {
    ...
    DatasetIndex p_data[NRECORDS_DSETINDEX] = {{"0100","ICOADS Release 2.1","Global Marine Surface Observations","1784-2002","1170","DS540.0","Scott.Woodruff@noaa.gov",""},
        ...
        {"0400","ICOADS Auxiliary Russian","Global Marine Surface Observations","1950-2000","","DS530.0","Worley@ucar.edu","" },
        {"0500","ICOADS Auxiliary Russian","Global Marine Surface Observations","1950-2000","","DS530.0","Worley@ucar.edu","" },
        {"0700","ICOADS Auxiliary Challenger","Global Marine Surface Observations","1889-2000","","DS530.0","Worley@ucar.edu","" },
        {"1000","Federal Climate Complex Integrated Surface Hourly","Global Land Surface Observations","1901-2005","3505","DS463.3", "Neal.Lott@noaa.gov",""},
        {"1002","CDMP SAO","US Land Surface Observations","1928-1948","3851_99","DS506.0", "Joe.D.Elms@noaa.gov",""},
        {"1003","Russian Empire Staions","Russian Land Surface Observations","1849-2000","td9290c","", "Pasha.Groisman@noaa.gov",""},
        {"1004","Air Weather Servcie TD13","Global Land Surface Observations","1901-1973","td13","DS467.0", "worly@ucar.edu",""},
