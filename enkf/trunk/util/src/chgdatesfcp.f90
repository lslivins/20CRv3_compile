...
! read each ensemble member.
 ...
 iunit = 21; iunit2 = 61
 call sfcio_srohdc(iunit,trim(filenamein),sfchead,sfcdata,iret)

! change date and time.

 sfchead%idate = idateout
 sfchead%fhour = real(fhour)

! write out

 call sfcio_swohdc(iunit2,trim(filenameout),sfchead,sfcdata,iret)

! clean up
 call sfcio_axdata(sfcdata,iret)
 call sfcio_sclose(iunit,iret)
 call sfcio_sclose(iunit2,iret)
