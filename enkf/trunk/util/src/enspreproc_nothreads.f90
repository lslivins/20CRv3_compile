...
! assumes uv_hyb_ens=.true.
 ...
 real, dimension(:,:), allocatable :: psg,zsg,subdgrd2,tmpwork,tmpworkv
 real, dimension(:,:,:), allocatable :: ug,vg,tempg,subdgrd3
 ...
 allocate(tmpwork(nlons,nlats))
 allocate(tmpworkv(nlons,nlats))
...
! deallocate arrays, close file
 call sigio_axdata(sigdata,ierr)
 call sigio_sclose(iunit,ierr)
 deallocate(tmpwork,tmpworkv)
...
! partition and write out data on subdomains.
 ...
 do nsub = 1,ndomains
    ...
    write(iunit) subdgrd2
    do k=1,nlevs
       call subset(istart,jstart,ilat1,jlon1,nlons,nlats+2,subdgrd3(:,:,k),ug(:,:,k))
    enddo
    write(iunit) subdgrd3
    do k=1,nlevs
       call subset(istart,jstart,ilat1,jlon1,nlons,nlats+2,subdgrd3(:,:,k),vg(:,:,k))
    enddo
    write(iunit) subdgrd3
    do k=1,nlevs
       call subset(istart,jstart,ilat1,jlon1,nlons,nlats+2,subdgrd3(:,:,k),tempg(:,:,k))
    enddo
    write(iunit) subdgrd3
    do k=1,nlevs
       call subset(istart,jstart,ilat1,jlon1,nlons,nlats+2,subdgrd3(:,:,k),qg(:,:,k,1))
    enddo
    write(iunit) subdgrd3
    do k=1,nlevs
       call subset(istart,jstart,ilat1,jlon1,nlons,nlats+2,subdgrd3(:,:,k),qg(:,:,k,2))
    enddo
    write(iunit) subdgrd3
    do k=1,nlevs
       call subset(istart,jstart,ilat1,jlon1,nlons,nlats+2,subdgrd3(:,:,k),qg(:,:,k,3))
    enddo
    write(iunit) subdgrd3
    close(iunit)
