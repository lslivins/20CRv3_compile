...
EOF
cat $HOSTFILE
#$MPICH/bin/mpirun_rsh -hostfile $HOSTFILE -np 1 $CHGRESEXEC
time $MPICH/bin/mpirun_rsh -hostfile $HOSTFILE -np 1 OMP_NUM_THREADS=$OMP_NUM_THREADS OMP_STACKSIZE=$OMP_STACKSIZE $CHGRESEXEC
#$MPICH/bin/mpiexec -hostfile $HOSTFILE -n 1 $CHGRESEXEC
