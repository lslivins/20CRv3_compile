...
export GRBINDEX=${GRBINDEX:-${EXECGLOBAL}/grbindex$XC}
...
export CTLFILE=${CTLFILE:-$FIXGLOBAL/gfs_cntrl.parm}
#  Other variables.
...
if [ ${OUTTYP} -le 1 ] ; then
 ...
 #$CHGRESSH
 time $MPICH/bin/mpirun_rsh  -hostfile $HOSTFILE -np 1 OMP_NUM_THREADS=$OMP_NUM_THREADS OMP_STACKSIZE=$OMP_STACKSIZE ${EXECGLOBAL}/spectogfsio.x $SIGINP $GFSOUT
...
cp $FIXGLOBAL/nam_micro_lookup.dat ./eta_micro_lookup.dat

time $MPICH/bin/mpirun_rsh  -hostfile $HOSTFILE -np $nprocs SIGINP=$SIGINP FLXINP=$FLXINP SFCINP=$SFCINP PGBOUT=$PGBOUT $POSTGPEXEC < itag 
...
#copygb=${COPYGB:-${EXECGLOBAL}/copygb}
#
