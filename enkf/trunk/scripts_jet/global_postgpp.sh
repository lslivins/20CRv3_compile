...
export XC=${XC}
export POSTGPEXEC=${POSTGPEXEC:-${EXECGLOBAL}/global_postgp}
...
echo "mpirun -hostfile $HOSTFILE -np $nprocs $POSTGPEXEC"
#$MPICH/bin/mpirun_rsh -hostfile $HOSTFILE -np $nprocs $POSTGPEXEC
mpirun -hostfile $HOSTFILE -np $nprocs $POSTGPEXEC
