...
!$$$ end documentation block
  ...
  subroutine convinfo_read(mype,iunitout)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!$$$
    ...
    integer(i_kind),intent(in   ) :: mype,iunitout
...
! open convbias.in file
    ...
    do i=1,nlines
       ...
       if(mype == izero)write(iunitout,1031)ioctype(nc),ictype(nc),icsubtype(nc),icuse(nc),ctwind(nc),ncnumgrp(nc), &
            ncgroup(nc),ncmiter(nc),cgross(nc),cermax(nc),cermin(nc),cvar_b(nc),cvar_pg(nc) &
            ,ithin_conv(nc),rmesh_conv(nc),pmesh_conv(nc),npred_conv(nc)
