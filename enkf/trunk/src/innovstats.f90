...
use obsmod, only:  oberrvar,ob,ensmean_ob,obtype,nobs_conv,nobs_oz,&
                   nobs_sat,nobstot,obloclat,ensmean_obnobc,obpress,stattype,&
                   indxsat,oberrvar_orig
...
real(r_kind) sumps_nh,biasps_nh,sumps_sh,biasps_sh,&
 ...
 sumpsab_nh,sumpsab_sh,sumpsab_tr,&
 sumtab_nh,sumtab_sh,sumtab_tr,&
 sumwndab_nh,sumwndab_sh,sumwndab_tr,&
 sumqab_nh,sumqab_sh,sumqab_tr,&
...
if (nobs_conv+nobs_oz > 0) then
  ...
  do nob=1,nobs_conv
     if(oberrvar(nob) < 1.e10_r_kind)then
         if (obtype(nob)(1:3) == ' ps' .and. oberrvar(nob) .lt. 1.e3) then
            call obstats(obfit(nob),obfit2(nob),oberrvar_orig(nob),&
                 obsprd(nob),obloclat(nob),&
                 sumps_nh,sumpsab_nh,biasps_nh,sumps_spread_nh,sumps_oberr_nh,nobsps_nh,&
                 sumps_sh,sumpsab_sh,biasps_sh,sumps_spread_sh,sumps_oberr_sh,nobsps_sh,&
                 sumps_tr,sumpsab_tr,biasps_tr,sumps_spread_tr,sumps_oberr_tr,nobsps_tr)
         else if (obtype(nob)(1:3) == '  t' .and. stattype(nob) /= 121) then
            call obstats(obfit(nob),obfit2(nob),oberrvar_orig(nob),&
                 obsprd(nob),obloclat(nob),&
                 sumt_nh,sumtab_nh,biast_nh,sumt_spread_nh,sumt_oberr_nh,nobst_nh,&
                 sumt_sh,sumtab_sh,biast_sh,sumt_spread_sh,sumt_oberr_sh,nobst_sh,&
                 sumt_tr,sumtab_tr,biast_tr,sumt_spread_tr,sumt_oberr_tr,nobst_tr)
        ! all winds
        ...
        !         (stattype(nob) >= 230 .and. stattype(nob) <= 235) ) then
            call obstats(obfit(nob),obfit2(nob),oberrvar_orig(nob),&
                 obsprd(nob),obloclat(nob),&
                 sumwnd_nh,sumwndab_nh,biaswnd_nh,sumwnd_spread_nh,sumwnd_oberr_nh,nobswnd_nh,&
                 sumwnd_sh,sumwndab_sh,biaswnd_sh,sumwnd_spread_sh,sumwnd_oberr_sh,nobswnd_sh,&
                 sumwnd_tr,sumwndab_tr,biaswnd_tr,sumwnd_spread_tr,sumwnd_oberr_tr,nobswnd_tr)
         else if (obtype(nob)(1:3) == '  q') then
            call obstats(obfit(nob),obfit2(nob),oberrvar_orig(nob),&
                 obsprd(nob),obloclat(nob),&
                 sumq_nh,sumqab_nh,biasq_nh,sumq_spread_nh,sumq_oberr_nh,nobsq_nh,&
                 sumq_sh,sumqab_sh,biasq_sh,sumq_spread_sh,sumq_oberr_sh,nobsq_sh,&
                 sumq_tr,sumqab_tr,biasq_tr,sumq_spread_tr,sumq_oberr_tr,nobsq_tr)
         end if
      end if
  end do ! loop over non-radiance obs
!--> print innovation statistics for subset of conventional data.
   print *,'conventional obs'
   print *,'region type #ob mean(O-Hx) rms(O-Hx) cov(Hx-Hxb,O-Hx) sprd+oberr sprd oberr:'
   call printstats('ps',sumps_nh,sumpsab_nh,biasps_nh,sumps_spread_nh,sumps_oberr_nh,nobsps_nh,&
        sumps_sh,sumpsab_sh,biasps_sh,sumps_spread_sh,sumps_oberr_sh,nobsps_sh,&
        sumps_tr,sumpsab_tr,biasps_tr,sumps_spread_tr,sumps_oberr_tr,nobsps_tr)
   call printstats(' t',sumt_nh,sumtab_nh,biast_nh,sumt_spread_nh,sumt_oberr_nh,nobst_nh,&
        sumt_sh,sumtab_sh,biast_sh,sumt_spread_sh,sumt_oberr_sh,nobst_sh,&
        sumt_tr,sumtab_tr,biast_tr,sumt_spread_tr,sumt_oberr_tr,nobst_tr)
   call printstats('uv',sumwnd_nh,sumwndab_nh,biaswnd_nh,sumwnd_spread_nh,sumwnd_oberr_nh,nobswnd_nh,&
        sumwnd_sh,sumwndab_sh,biaswnd_sh,sumwnd_spread_sh,sumwnd_oberr_sh,nobswnd_sh,&
        sumwnd_tr,sumwndab_tr,biaswnd_tr,sumwnd_spread_tr,sumwnd_oberr_tr,nobswnd_tr)
   call printstats(' q',sumq_nh,sumqab_nh,biasq_nh,sumq_spread_nh,sumq_oberr_nh,nobsq_nh,&
        sumq_sh,sumqab_sh,biasq_sh,sumq_spread_sh,sumq_oberr_sh,nobsq_sh,&
        sumq_tr,sumqab_tr,biasq_tr,sumq_spread_tr,sumq_oberr_tr,nobsq_tr)
...
subroutine obstats(obfit,obfit2,oberrvar,obsprd,obloclat,&
                   sumfit_nh,sumfit2_nh,sumbias_nh,sumspread_nh,sumoberr_nh,nobs_nh,&
                   sumfit_sh,sumfit2_sh,sumbias_sh,sumspread_sh,sumoberr_sh,nobs_sh,&
                   sumfit_tr,sumfit2_tr,sumbias_tr,sumspread_tr,sumoberr_tr,nobs_tr)

  implicit none
  real(r_kind), intent(in out) ::  sumfit_nh, sumbias_nh, sumspread_nh, sumoberr_nh,&
       sumfit2_nh,sumfit2_tr,sumfit2_sh,&
       sumfit_tr, sumbias_tr, sumspread_tr, sumoberr_tr,&
       sumfit_sh, sumbias_sh, sumspread_sh, sumoberr_sh
  real(r_kind), intent(in) :: obfit, obfit2, oberrvar, obsprd, obloclat
...
! compute innovation statistics in nh,sh,tropics.
  ...
  if (obloclat > latbound) then
     if (nobs_nh == 0) then
       sumfit_nh = obfit**2
       sumfit2_nh = (obfit2-obfit)*obfit
       sumbias_nh = obfit
       sumspread_nh = obsprd
       sumoberr_nh = oberrvar
     else
       sumfit_nh = sumfit_nh + obfit**2
       sumfit2_nh = sumfit2_nh + (obfit2-obfit)*obfit
     ...
     if (nobs_sh == 0) then
       sumfit_sh = obfit**2
       sumfit2_sh = (obfit2-obfit)*obfit
       sumbias_sh = obfit
       sumspread_sh = obsprd
       sumoberr_sh = oberrvar
     else
       sumfit_sh = sumfit_sh + obfit**2
       sumfit2_sh = sumfit2_sh + (obfit2-obfit)*obfit
     ...
     if (nobs_tr == 0) then
       sumfit_tr = obfit**2
       sumfit2_tr = (obfit2-obfit)*obfit
       sumbias_tr = obfit
       sumspread_tr = obsprd
       sumoberr_tr = oberrvar
     else
       sumfit_tr = sumfit_tr + obfit**2
       sumfit2_tr = sumfit2_tr + (obfit2-obfit)*obfit
...
subroutine printstats(obtype,sum_nh,sumab_nh,bias_nh,sum_spread_nh,sum_oberr_nh,nobs_nh,&
              sum_sh,sumab_sh,bias_sh,sum_spread_sh,sum_oberr_sh,nobs_sh,&
              sum_tr,sumab_tr,bias_tr,sum_spread_tr,sum_oberr_tr,nobs_tr)
  implicit none
  real(r_kind), intent(in out) ::  bias_nh, sum_spread_nh, sum_oberr_nh,&
       ...
       sum_nh,sum_sh,sum_tr,sumab_nh,sumab_sh,sumab_tr
...
!   print *,'obtype,nobs_nh,nobs_sh,nobs_tr ',obtype,nobs_nh,nobs_sh,nobs_tr
  if (nobs_nh > 0) then
     ...
     sum_nh = sum_nh*denom
     sumab_nh = sumab_nh*denom
     bias_nh = bias_nh*denom
     sum_oberr_nh = sum_oberr_nh*denom
     sum_spread_nh = sum_spread_nh*denom
     write(6,9805) &
     'NH',obtype,nobs_nh,bias_nh,sqrt(sum_nh),sqrt(sumab_nh),sqrt(sum_spread_nh+sum_oberr_nh),sqrt(sum_spread_nh),sqrt(sum_oberr_nh)
  end if
  if (nobs_tr > 0) then
     denom=one/real(nobs_tr,r_kind)
     sum_tr = sum_tr*denom
     sumab_tr = sumab_tr*denom
     bias_tr = bias_tr*denom
     sum_oberr_tr = sum_oberr_tr*denom
     sum_spread_tr = sum_spread_tr*denom
     write(6,9805) &
     'TR',obtype,nobs_tr,bias_tr,sqrt(sum_tr),sqrt(sumab_tr),sqrt(sum_spread_tr+sum_oberr_tr),sqrt(sum_spread_tr),sqrt(sum_oberr_tr)
  end if
  if (nobs_sh > 0) then
     denom=one/real(nobs_sh,r_kind)
     sum_sh = sum_sh*denom
     sumab_sh = sumab_sh*denom
     bias_sh = bias_sh*denom
     sum_oberr_sh = sum_oberr_sh*denom
     sum_spread_sh = sum_spread_sh*denom
     write(6,9805) &
     'SH',obtype,nobs_sh,bias_sh,sqrt(sum_sh),sqrt(sumab_sh),sqrt(sum_spread_sh+sum_oberr_sh),sqrt(sum_spread_sh),sqrt(sum_oberr_sh)
  end if
9805 format(a2,1x,a2,1x,i6,6(1x,e10.3))
