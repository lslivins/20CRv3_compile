...
subroutine get_num_satobs(obspath,datestring,num_obs_tot,id)
    ...
    type(diag_header_chan_list),pointer :: header_chan0(:)=>null()
    type(diag_data_fix_list   )         :: data_fix0
    type(diag_data_chan_list  ),pointer :: data_chan0(:)=>null()
    type(diag_data_extra_list) ,pointer :: data_extra0(:,:)=>null()
...
subroutine get_satobs_data(obspath, datestring, nobs_max, h_x, h_xnobc, x_obs, x_err, &
  ...
  use radinfo, only: iuse_rad,nusis,jpch_rad,nuchan,npred,npreds,adp_anglebc
  ...
  real(r_kind), dimension(npreds,nobs_max) :: x_biaspred
  ...
  type(diag_header_chan_list),pointer :: header_chan1(:)=>null(),header_chan2(:)=>null()
  type(diag_data_fix_list   )         :: data_fix1,data_fix2
  type(diag_data_chan_list  ),pointer :: &
  data_chan1(:)=>null(),data_chan2(:)=>null()
  type(diag_data_extra_list) ,pointer :: data_extra1(:,:)=>null(),data_extra2(:,:)=>null()
...
!!         x_biaspred(1,nobs) = data_chan1(n)%bifix! fixed angle dependent bias
         ...
         if (adp_anglebc) then
            ...
            x_biaspred(7,nobs)  = data_chan1(n)%bifix(1) ! 4th order scan angle (predictor)
            x_biaspred(8,nobs)  = data_chan1(n)%bifix(2) ! 3rd order scan angle (predictor)
            x_biaspred(9,nobs)  = data_chan1(n)%bifix(3) ! 2nd order scan angle (predictor)
            x_biaspred(10,nobs) = data_chan1(n)%bifix(4) ! 1st order scan angle (predictor)
         endif
