...
use constants, only: zero, one, deg2rad, constants_initialized, rad2deg, &
     & rd, cp, pi
use params, only: & 
...
character(len=64), public, allocatable, dimension(:) :: obids
...
real(r_kind), public :: fail=1.e31
real(r_kind), public :: failm=1.e30
real(r_kind), public :: failqc=1.e32
...
call mpi_getobs(datapath, datestring, nobs_conv, nobs_oz, nobs_sat, nobstot, &
                ...
                obtime, oberrvar_orig, stattype, obtype, obids, biaspreds,&
...
do nob=1,nobstot
  tcob = stattype(nob) >= 300 .and. stattype(nob) < 600 ! free pass for TC obs in 20CRV3
  ...
  if (varqc .and. .not. tcob) oberrvar(nob) = oberrvar(nob)*(min(one,0.5_r_kind+0.125_r_kind*(zhuberleft+zhuberright)))**2
  ...
  if (obtype(nob) == ' ps' .and. (stattype(nob) == 111 .or. stattype(nob) == 112 .or. tcob)) then
      ! tcvitals obs get a free pass for background check.
      if (nproc == 0) then
      print *,'tcvitals ps ob found (lat,lon,time,ob,O-F,sprd,oberrvar):'
      write(6,9101) stattype(nob),obloclat(nob),obloclon(nob),obtime(nob),ob(nob),obfit_prior(nob),obsprd_prior(nob),sqrt(oberrvar(nob))
      endif
  else
     ! background checks.
     if (abs(obfit_prior(nob)) > sprd_tol*sqrt(obsprd_prior(nob)+oberrvar(nob)) .and. .not. tcob) then
       ...
       if(nproc == 0)print *, 'reject ob due to obsprd_prior > sprd_tol ',&
          obtype(nob)(1:3),stattype(nob),obfit_prior(nob),sprd_tol*sqrt(obsprd_prior(nob)+oberrvar(nob))
       oberrvar(nob)= failqc
       if (obtype(nob)(1:3) == '  u') oberrvar(nob)=failqc
       if (obtype(nob)(1:3) == '  v' .and. oberrvar(nob-1) >= failm) oberrvar(nob-1)=failqc
       cycle
     end if
  end if
