...
use params, only: datapath,nlevs,nvars,ndim,datestring,&
                  nlons,nlats,reducedgrid,massbal_adjust
use kinds, only: r_kind, i_kind, r_double, r_single
use constants, only: one,zero,pi,cp,rd,grav,constants_initialized,rearth
use sigio_module, only: sigio_head
use specmod, only: sptezv_s, sptez_s, init_spec_vars, isinitialized, gaulats,&
    ndimspec => nc
...
type(sigio_head), save, public :: sighead
contains
...
integer(i_kind) iret,i,j,ntrunc
...
! distribute to all processors.
   ...
   if (reducedgrid) then
      call reducedgrid_init(nlons,nlats,asin(gaulats))
      npts = nptsred
   else
      npts = nlons*nlats
   end if
   ...
   deallocate(spressmn,presslmn,pressimn,tmpspec)
end if
