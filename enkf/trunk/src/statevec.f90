...
t1 = mpi_wtime()
!print *,'min/max pert inc',nproc,minval(anal_chunk(:,:,ndim)-anal_chunk_prior(:,:,ndim)),maxval(anal_chunk(:,:,ndim)-anal_chunk_prior(:,:,ndim))
!print *,'min/max mean inc',nproc,minval(ensmean_chunk(:,ndim)-ensmean_chunk_prior(:,ndim)),maxval(ensmean_chunk(:,ndim)-ensmean_chunk_prior(:,ndim))
do nn=1,ndim
 do i=1,numptsperproc(nproc+1)
  do nanal=1,nanals
   n = ((nanal-1)*ndim + (nn-1))*npts_max + i
