...
!   2017-04-27  modified QC to make tcobs in ISPD not subject to QC 
!		and not skippable
!
! attributes:
!   language: f95
...
use obsmod, only: oberrvar, ob, ensmean_obnobc, ensmean_ob, obloc, oblnp, &
                  nobsgood, nobs_conv, nobs_oz, nobs_sat, obids, &
                  obfit_prior, obfit_post, obsprd_prior, obsprd_post, obtime,&
                  obtype, oberrvarmean, numobspersat, deltapredx, biaspreds,&
                  biasprednorm, oberrvar_orig, probgrosserr, prpgerr, failqc,&
                  corrlengthsq,lnsigl,obtimel,obloclat,obloclon,obpress,stattype
use constants, only: pi, one, zero, rearth
...
real(r_kind),allocatable, dimension(:) :: rannum,corrlengthsq_orig,lnsigl_orig
...
real(r_kind), allocatable, dimension(:) :: paoverpb_save
...
integer(i_kind) nanal,nn,nnn,nobm,nsame,nn1,nn2,ncount
real(r_kind),dimension(nlevs_pres):: taperv
logical lastiter, kdgrid, kdobs, tcob
...
if (iassim_order == 1) then
  ...
  do nob=1,numobsperproc(nproc+1)
     ...
     tcob = stattype(nob1) >= 300 .and. stattype(nob1) < 600 ! TC obs
     paoverpb_chunk(nob) = oberrvar(nob1)/(oberrvar(nob1)+obsprd_prior(nob1))
     if (tcob) then
        ! make values for tc obs really small, so they will be done first.
        paoverpb_chunk(nob) = paoverpb_chunk(nob)/1000.
     endif
  enddo
  do nob=1,nobsgood
      indxassim2(nob) = nob
...
! reset ob error to account for gross errors 
  if (niter > 1 .and. varqc) then
    if (huber) then ! "huber norm" QC
      do nob=1,nobsgood
        tcob = stattype(nob) >= 300 .and. stattype(nob) < 600 ! TC obs
        ...
        ! don't use varqc for TC obs in 20CRV3
        if (tcob) pnge=1.0
	...
	if (nproc == 0) then  
		if (tcob) print*,niter,'tcob',oberrvaruse(nob),oberrvar(nob),pnge
	endif
    ...
    do nob=1,nobsgood
      ...
      tcob = stattype(nob) >= 300 .and. stattype(nob) < 600 ! TC obs
      if (nproc == 0) then
      	if (tcob) print*,niter,'tcob',oberrvaruse(nob),oberrvar(nob)
      endif
    end do
  end if
  ...
  obsloop: do nobx=1,nobsgood
      ...
      if (iassim_order == 2) then
         if (niter == 1) then
            ...
            if (numobsperproc(nproc+1) > 0) then
	    ...
	    nob2 = paoverpb_min(2)
            tcob = stattype(nob2) >= 300 .and. stattype(nob2) < 600 ! TC obs
	    
            if ((paoverpb_min(1) >= paoverpb_thresh) .and. .not. tcob) then
                if (nproc .eq. 0) &
      ...
      if ((paoverpb_save(nob) >= paoverpb_thresh .or. &
          oberrvaruse(nob) > 1.e10_r_kind) .and. .not. tcob) then
          iskip(nob) = 1
          if (iassim_order == 2) then
             if (nproc .eq. 0) &
             print *,'exiting obsloop after ',nobx,' obs processed' 
...
if (nproc .eq. 0 .and. nhr_anal == 6) then
    ...
    do nobx=1,nobs_conv
       ...
       write(9,9802) obids(nob),obtype(nob)(1:3),stattype(nob),obloclon(nob),obloclat(nob),&
       obpress(nob),obtime(nob),ob(nob),iskip(nob),ob(nob)-obfit_prior(nob)-ensmean_obnobc(nob),&
       obfit_prior(nob),obfit_post(nob),obsprd_prior(nob),obsprd_post(nob),&
       oberrvar_orig_out,oberrvar_out,oberrvaruse(nob),paoverpb_save(nob),&
       probgrosserr(nob),sqrt(corrlengthsq(nob))*rearth/1.e3_r_kind,lnsigl(nob),ifailqc
    enddo
    9802 format(a64,1x,a3,1x,i3,1x,f7.2,1x,f6.2,1x,f7.2,1x,f6.2,1x,f7.2,1x,i1,1x,&
                5(e10.3,1x),3(f7.4,1x),f8.6,1x,f4.2,1x,f7.2,1x,f7.5,1x,i1)
    close(9)
end if
...
! put barrier here so all prints will show up in stdout.
