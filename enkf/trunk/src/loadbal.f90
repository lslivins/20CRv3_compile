...
!   
!
! Modules Used: mpisetup, params, kinds, constants, obsmod, gridinfo,
...
use obsmod, only: nobsgood, obloc, oblnp, ensmean_ob, obtime, anal_ob, corrlengthsq, nobstot, &
...
if(simple_partition)then
  ...
  ! **TODO** adjust background QC check, so stricter check applied when there are few
  ! nearby obs.
  if (nproc == 0) then
    print *,'time in estimate_work_enkf2 = ',mpi_wtime()-t1,' secs'
    print *,'min/max close obs = ',minval(numobs1),maxval(numobs1)
    print *,'count with numobs < 1',count(numobs1 < 1)
  endif
  t1 = mpi_wtime()
  rtmp = 0
  numobsperproc = 0
