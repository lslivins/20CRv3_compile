...
!               kdtree2_module, obsmod, radinfo, radbias, gridinfo
!
! program history log:
...
use obsmod, only: oberrvar, ob, ensmean_ob, obloc, oblnp, &
...
real(r_kind) corrlength_save,lnsigl_save,covl_fact,covl_minfact,covl_efold
...
! reset ob error to account for gross errors 
  ...
  obsloop: do nobx=1,nobsgood
      ...
      covl_minfact = 1./8.; covl_efold = 1./5. ! these should be namelist parms
      if (niter == 1 .and. covl_minfact < 0.99) then
! modify localization based on HPaHT/HPbHT
         corrlength_save = sqrt(corrlengthsq(nob))
         lnsigl_save = lnsigl(nob)
         covl_fact = 1. - exp( -((1.-paoverpb)/covl_efold) )
         corrlengthsq(nob) = (covl_fact*sqrt(corrlengthsq(nob)))**2
         lnsigl(nob) = covl_fact*lnsigl(nob)
         if (sqrt(corrlengthsq(nob)) .lt. covl_minfact*corrlength_save) &
         corrlengthsq(nob)=(covl_minfact*corrlength_save)**2
         if (lnsigl(nob) .lt. covl_minfact*lnsigl_save) &
         lnsigl(nob)=covl_minfact*lnsigl_save
         !if (nproc == 0 .and. paoverpb <= paoverpb_thresh) &
         !print *,'nob,paoverpb,covl_fact=',nobx,paoverpb,sqrt(corrlengthsq(nob))/corrlength_save
         ! reset to original values for testing
         !corrlengthsq(nob) = corrlength_save**2
         !lnsigl(nob) = lnsigl_save
      endif
