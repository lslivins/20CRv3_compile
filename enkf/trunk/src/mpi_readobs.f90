...
!
! Public Subroutines:
!  mpi_readobs: called by subroutine readobs in module obsmod. 
...
subroutine mpi_getobs(obspath, datestring, nobs_conv, nobs_oz, nobs_sat, nobs_tot, &
                      ...
                      obtime, oberrorig, obcode, obtype, obids, &
    ...
    character(len=64), allocatable,  dimension(:) ::  obids
...
! if nobs_tot != 0 (there were some obs to read)
    if (nobs_tot > 0) then
       ...
       obids(nobs_tot),biaspreds(npreds, nobs_sat),indxsat(nobs_sat))
    else
...
! first nobs_conv are conventional obs.
      call get_convobs_data(obspath, datestring, nobs_conv,               &
        ...
        oberrorig(1:nobs_conv), obtype(1:nobs_conv), obids(1:nobs_conv), id,id2)
