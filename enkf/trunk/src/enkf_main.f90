...
!$$$
 ...
 use params, only : read_namelist,letkf_flag,readin_localization
 ...
 use obsmod, only : readobs, obfit_prior, obsprd_prior, &
 ...
 if (nproc == 0) then
    ...
    call print_innovstats(obfit_prior, obsprd_prior, obfit_prior)
 end if
 ...
 if (nproc == 0) then
    ...
    call print_innovstats(obfit_post, obsprd_post, obfit_prior)
