...
mean20cr = np.empty(era.shape,dtype=np.float32)
sprd20cr = np.empty(era.shape,dtype=np.float32)
for nt,date in enumerate(dates):
    grbs_mean = pygrib.open(datapath+'/%s/pgrbensmeananl_%s' % (date,date))
    grb = grbs_mean.select(shortName=var,typeOfLevel='isobaricInhPa',level=level)[0]
    mean20cr[nt] = grb.values
    grbs_mean.close()
    grbs_sprd = pygrib.open(datapath+'/%s/pgrbenssprdanl_%s' % (date,date))
    grb= grbs_sprd.select(shortName=var,typeOfLevel='isobaricInhPa',level=level)[0]
    sprd20cr[nt] = grb.values
    grbs_sprd.close()
errall = mean20cr - era
