...
export OMP_NUM_NTHREADS=${OMP_NUM_THREADS:-1}
...
cat << EOF > fort.912
 &NAMCHG JCAP=$JCAP, LEVS=$LEVS, LONB=$LONB, LATB=$LATB,
         NTRAC=$NTRAC, IDVC=$IDVC, IDSL=$IDSL,IDVM=$IDVM,IDVT=$IDVT, NVCOORD=$NVCOORD,
...
# use ssh
#nodename=`head -1 $HOSTFILE`
#nthreads=$OMP_NUM_THREADS
#echo "ssh -n $nodename cd ${DATA}; setenv OMP_STACKSIZE $OMP_STACKSIZE; setenv OMP_NUM_THREADS $nthreads; limit stacksize unlimited; ${CHGRESEXEC} < fort.912"
#time ssh -n $nodename "cd ${DATA}; setenv OMP_STACKSIZE $OMP_STACKSIZE; setenv OMP_NUM_THREADS $nthreads; limit stacksize unlimited; echo $hostname; pwd; ${CHGRESEXEC} < fort.912"

# use aprun
#time aprun -n 1 ${CHGRESEXEC} < fort.912

# run interactively
echo "${CHGRESEXEC} < fort.912"
${CHGRESEXEC} < fort.912
