...
# updated to include checks for ensemble statistics files (13 Jan 2018, G. Compo)
...
set ncycles=8 # number of DA cycles to run
...
###@ timemax = 20 #for short jobs, hardwire timemax to 20 minutes
@ timemax = $timemax - 45
...
# in parallel
setenv maxcorespernode 64 # on KNL the cores per node and maxcoresper node are set separately
setenv maxnhosts `expr $NNODES \* $maxcorespernode` #5200 
 
# GFS and EnKF MPI and OpenMP settings                                                
setenv fgmpitasks 65#127#65 # number of mpi tasks per ensemble member for the GFS
...
setenv fgcorespermpitask 4 # number of cores for each GFS MPI task and available for OpenMP thread

setenv corespermpitask 8 # number of cores used by EnKF MPI task and available for OpenMP thread 
		#and other: 
setenv corespermpitaskd5 52# number of cores per mpi task to use for NNODESd5 on knl 
setenv corespermpitaskd8 32#number of cores to use for NNODESd8 on knl
...
setenv corespersmlmpitask 4 # number of cores to assign for post-processor and other smaller codes


setenv fg_proc 20  # this is for global_postgp postprocessor, number of mpi tasks per member
...
set fg_gfs = "run_fg_gfs_ens_iau_knl" # calls run_gefs_knl
...
echo "spinup year start is $spinyear"
