...
#SBATCH -J 1829_451_gfsenkf
#SBATCH -A m958
#SBATCH -q premium
###SBATCH --qos=premium
#SBATCH -N 80    
#SBATCH -t 01:48:14
#SBATCH -e gfsenkf_haswell.err
#SBATCH -o gfsenkf_haswell.out
#SBATCH -C haswell

#SBATCH -L cscratch1

##Directives for the "DataWarp" or "BurstBuffer"
## We use about 75GB for 1 analysis cycle
##DW jobdw capacity=2000GB access_mode=striped type=scratch
##DW stage_in source=/global/cscratch1/sd/compo/gfsenkf_20crV3_cmip5oz_CoriII/ensda_499_1914 destination=$DW_JOB_STRIPED/ensda_499_1914 type=directory
##DW stage_out source=$DW_JOB_STRIPED/ensda_499_1914 destination=/global/cscratch1/sd/compo/gfsenkf_20crV3_cmip5oz_CoriII/ensda_499_1914 type=directory
##DW stage_out source=$DW_JOB_STRIPED/logs/ensda_499_1914 destination=/global/cscratch1/sd/compo/logs/ensda_499_1914 type=directory
...
# re-organized MPI and cpu related environment variables (23 Feb 2017, G. Compo)

# changed model-station height zthres from 1000m to Mt. Everest distance (6 November 2017, G. Compo)
# added timeelap, timemax to track amount of time remaining to avoid hangs (13 December 2017, G. Compo)


# updated to include specific -N directive to srun
# for srun being sent to background "&" (16 Feb 2017, G. Compo)
# updated to include checks for ensemble statistics files (17 Jan 2018, G. Compo)
...
# intended for Gaea
#setenv nhosts 2560 # 80*32
# need to add back in the setting of the environment variables that was consistent for Gaea

# In this version of the script, 
#the MPI tasks for the 
# GFS and EnKF are set separately 
# using fgmpitasks (GFS)
# enkf_taskspernode

#echo "check on DW"
#ls -l $DW_JOB_STRIPED/ensda_499_1914
#cat $DW_JOB_STRIPED/ensda_499_1914/analdate.csh
...
set ncycles=12 # number of DA cycles to run
set timemax=2 # this is in hours. Make timemax one hour larger than you want to run for. 
@ timemax = $timemax * 60 # this is in minutes
###@ timemax = 20 #for short jobs, hardwire timemax to 20 minutes (test 12 minutes)
@ timemax = $timemax - 60

setenv nanals 80  # how many ensemble members
 

# intended for Cori II knl
# make divisions of the number of nodes
setenv NNODES $SLURM_NNODES # set in the SBATCH -N above
setenv NNODESd2 `expr $NNODES \/ 2`
setenv NNODESd3 `expr $NNODES \/ 3`

setenv NNODESd4 `expr $NNODES \/ 4`
setenv NNODESd5 `expr $NNODES \/ 5`
setenv NNODESd6 `expr $NNODES \/ 6`
setenv NNODESd8 `expr $NNODES \/ 8`
setenv NNODESd10 `expr $NNODES \/ 10`
# test if I can put all 80 mpi for the averaging on one node
		      # or do I have to have 2


# these are used to compare the number of cores available to those being requested for the postprocessor
# in parallel
setenv maxcorespernode 32 # on KNL the cores per node and maxcoresper node are set separately
setenv maxnhosts `expr $NNODES \* $maxcorespernode` #5200 
 
# GFS and EnKF MPI and OpenMP settings                                                
setenv fgmpitasks 32#127#65 # number of mpi tasks per ensemble member for the GFS
setenv fgnhosts `expr $NNODES \* $fgmpitasks`
setenv enkf_taskspernode 32 #64 # number of MPI tasks to use per node available to the global_enkf

# think all of these numbers are 2 for Haswell, since I should have 32 MPI tasks for each node (26 Nov 2017)
setenv fg_omp_threads 2# number of OMP threads for the GFS and the ENKF (note below says 1,2,3, or 6 for EnKF)
setenv fgcorespermpitask 2 # number of cores for each GFS MPI task and available for OpenMP thread

setenv corespermpitask 2 # number of cores used by EnKF MPI task and available for OpenMP thread 
		#and other: 
setenv corespermpitaskd5 12# number of cores per mpi task to use for NNODESd5 on Haswell 
setenv corespermpitaskd8 8#number of cores to use for NNODESd8 on Haswell

# may want to try different values of the following arguments to -c 
# this determines how many of the logical cores are dedicated to each MPI rank
# 	There could be a trade off with using OMP_NUM_THREADS (maybe try 2 threads and cores per mpi task of 2)
# [tests so far suggest OpenMP not super useful]

setenv corespersmlmpitask 2 # number of cores to assign for post-processor and other smaller codes


setenv fg_proc 10  # this is for global_postgp postprocessor, number of mpi tasks per member
		 


setenv gen_omp_threads 1 # how many threads to use throughout the scripts
setenv OMP_NUM_THREADS $gen_omp_threads  # G Compo 22 Feb - tried 2 and 4 - no difference
					# revisit if shtns compiled for OpenMP
...
set ensda = "run_enkf_ps_h5_biascor_iau_haswell"
set fg_gfs = "run_fg_gfs_ens_iau_haswell" # calls run_gefs_haswell
...
# for DW comment above and uncomment below
#set datadir=$DW_JOB_STRIPED

# where the data will be created
setenv expname `basename $PWD`
setenv spinyear `cut -c11-114`
...
setenv obsdirh5 /global/cscratch1/sd/cmccoll/HDF5/V47/
...
# now using sodasi.3 (not c), which has the 1981-2010 climate of HadISST2.2
setenv sstpath /global/cscratch1/sd/compo/bound_cond_sodasi.3/
#setenv sstpath /global/cscratch1/sd/lslivins/bound_cond_sodasi.3c/
# now using hadisst2.2.0.0
#setenv sstpath /global/cscratch1/sd/compo/bound_cond_hadisst2.2/


# climo ice of HadISST ice?
setenv use_climoice "false" # use HadISST ice
####setenv use_climoice "true"
setenv lastndays 60
setenv NTRAC 3
...
setenv zthresh 8848  # don't assim if station - model elev more than t his (SLP to mt everest), was 1000 
...
setenv FHMIN 3
setenv FHDFI 0
##1 did not seem to work for FHOUT
setenv FHOUT 3
setenv FHLWR 3600
setenv FHSWR 3600
setenv ODELTIM 1200 # this can be reduced in the iterations
setenv dtphys 600
...
setenv ISOL 2          # 0--fixed solar constant; 1--old changing solar constant; 2--new changing solar constant
...
set startdate=${spinyear}111000 #change to ${spinyear}111000 for production
...
# starting time in minutes since the Unix epoch.
set timestart=`date +%s`
set timestart=`expr $timestart / 60`
set timeelap=0

set ncycle=1
while ($ncycle < = $ncycles && $timeelap < $timemax)
...
while ($fh <= $FHMAX)
  ...
  if ($cleanup_ensmean == 'true' || ($cleanup_ensmean == 'false' && ! -s ${datapath}/${analdate}/bfg_${analdate}_${charfhr}_ensmean)) then
     ...
     time srun  -N $NNODESd8 -n $nanals -c $corespermpitaskd8 --cpu_bind=cores  ${EXECGLOBAL}/getsfcensmeanp.x ${datapath}/${analdate}/ bfg_${analdate}_${charfhr}_ensmean bfg_${analdate}_${charfhr} ${nanals}  &
  endif
  @ fh = $fh + $FHOUT
end
...
while ($fh <= $FHMAX)
  ...
  if ($cleanup_ensmean == 'true' || ($cleanup_ensmean == 'false' && ! -s ${datapath}/${analdate}/sfg_${analdate}_${charfhr}_ensmean)) then
   ...
   time srun  -N $NNODESd8 -n $nanals -c $corespermpitaskd8 --cpu_bind=cores  ${EXECGLOBAL}/getsigensmeanp.x ${datapath}/${analdate}/ sfg_${analdate}_${charfhr}_ensmean sfg_${analdate}_${charfhr} ${nanals} & 
  endif
  #@ fh = $fh + $FHOUT
  @ fh = $fh + $FHOUT
end
...
wait
set nens_calcstat = `find ${datapath}/${analdate} -name "*ens*" -print |wc -l`
# should have the following 12 files now
#bfg_analdate_fhr03_ensmean
#bfg_analdate_fhr06_ensmean
#bfg_analdate_fhr09_ensmean

#pgrbensmeanfg_analdate_fhr06
#pgrbenssprdfg_analdate_fhr06
#sflxgrbensmeanfg_analdate_fhr03
#sflxgrbenssprdfg_analdate_fhr03
#sflxgrbensmeanfg_analdate_fhr06
#sflxgrbenssprdfg_analdate_fhr06
#sfg_analdate_fhr03_ensmean
#sfg_analdate_fhr06_ensmean
#sfg_analdate_fhr09_ensmean

# can be 21 if this is a re-run
  if ($nens_calcstat < 12) then
	echo "Postprocessing failed: ${analdate}"
	echo "found $nens_calcstat ens statistics files"
	find ${datapath}/${analdate} -name "*ens*" -print
     	exit 1
  endif
  set nens_calcstat = `find ${datapath}/${analdatem1} -name "pgrbens*anl*" -print |wc -l`
  
# should have the following files now in analdatem1, but only need to check for pgrbens anl (sanl is checked in ensda)
#diag_conv_ges.analdate_ensmean
#pgrbensmeananl_analdate
#pgrbensmeananl_analdate
#pgrbenssprdanl_analdate
#pgrbenssprdanl_analdate
#sanl_analdate_fhr03_ensmean
#sanl_analdate_fhr06_ensmean
#sanl_analdate_fhr09_ensmean
#sfcanl_analdate_fhr03_ensmean
 if($nens_calcstat != 4) then
	echo "Pgrbanl Postprocessing failed: ${analdatem1}"
	echo "found  $nens_calcstat pgrb anl ens statistics files"
	find ${datapath}/${analdatem1} -name "pgrbens*anl*" -print
     	exit 1
  endif
...
while ($alldone == 'no' && $niter <= $nitermax)
    ...
    setenv DELTIM `expr $ODELTIM / 2` # reduce time step
    csh ${enkfscripts}/${fg_gfs} >>& ${current_logdir}/run_fg.out
    set exitstat=$status
    endif
    if ($exitstat == 0) then
...
if (($day == '01' || $day == '05' || $day == '10' || $day == '15' || $day == '20' || $day == '25') && $hr == '00') then
   ...
   /bin/rm -f ${datapath2}/bfg*fhr0${fh}*mem* &
    ...
    #endif
    # let's keep this for now 4 March 2017 G. Compo
   ...
   # just keep a small subset on dist (ensmean means, spreads)
	# remember to change this to keep more for production 4 November 2017 G. Compo
   #/bin/rm -f ${datapath2}/sflx*mem* &# eventually will want this
   #/bin/rm -f ${datapathm1}/pgrbanl*mem* &# eventually will want this
   #/bin/rm -f ${datapath2}/pgrbfg*mem* &# eventually may want this
   #/bin/rm -f ${datapath2}/sanl*mem* &# eventually will want this
   #/bin/rm -f ${datapath2}/sfcanl*mem* & # eventually will want this
...
@ ncycle = $ncycle + 1
 ...
 # time (in minutes) elapsed since start of job.
set timeelap=`date +%s`
set timeelap=`expr $timeelap / 60`
@ timeelap = $timeelap - $timestart

echo "elapsed walltime = ${timeelap}"
...
sbatch gfsenkf_20crV3_cmip5oz_new_haswell.csh >&! current_jobid
...
# need to subtract one from ncycle, since added one at the end of the final cycle completed
@ ncycle = $ncycle - 1
echo "$SLURM_JOB_ID LF `date +%Y%m%d%H%M` ncycles=$ncycle $analdate $walltime $NNODES $fgmpitasks $fgcorespermpitask $enkf_taskspernode $corespermpitask $OMP_NUM_THREADS $fg_omp_threads" >> ${enkfscripts}/walltime.out
#endif
