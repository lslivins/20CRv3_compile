...
#SBATCH -N 80    
#SBATCH -t 04:29:15
#SBATCH -J 1915_gfsenkf_419
#SBATCH -e gfsenkf.err
#SBATCH -o gfsenkf.out
#SBATCH -C knl,cache,quad
## knl has 68 cpus, only using up to 65 MPI ranks per node
...
## not sure how much working space we use for 1 analysis cycle or even 1 day
##
#DW jobdw capacity=5000GB access_mode=striped type=scratch
#DW stage_in source=/global/cscratch1/sd/compo/gfsenkf_20crV3_cmip5oz_CoriII/ensda_419_1915 destination=$DW_JOB_STRIPED/ensda_419_1915 type=directory
#DW stage_out source=$DW_JOB_STRIPED/ensda_419_1915 destination=/global/cscratch1/sd/compo/gfsenkf_20crV3_cmip5oz_CoriII/ensda_419_1915 type=directory
#DW stage_out source=$DW_JOB_STRIPED/logs/ensda_419_1915 destination=/global/cscratch1/sd/compo/logs/ensda_419_1915 type=directory
...
# removed at suggestion of T.Kurth 14 July 2017 as MKL bug is fixed
###module load cray-memkind

#module load cray-netcdf/4.4.0
#module load cray-hdf5
# T. Kurth recommend craype-hugepages2M or 4M module for compiling and running
# one can compile with one and run with another
...
setenv fgmpitasks 64#124#96#65#96#65#128#65#128#65 # number of mpi tasks per ensemble member for the GFS

setenv enkf_taskspernode 32#32#16#32 #64 # number of MPI tasks to use per node available to the global_enkf

setenv fg_omp_threads 3#2# number of OMP threads for the GFS and the ENKF (note below says 1,2,3, or 6 for EnKF)
setenv fgcorespermpitask 4#4 # number of cores for each GFS MPI task and available for OpenMP thread

setenv corespermpitask 8#8 # number of cores used by EnKF MPI task and available for OpenMP thread 
		#and other: 
...
setenv gen_omp_threads 2 # how many threads to use throughout the scripts
...
set startdate=1914050100 #changed starting as of 1914070812
...
setenv DELTIM $ODELTIM # reset the DELTIM to the original in case it was changed during a cycle
...
while ($fh <= $FHMAX)
  ...
  if ($cleanup_ensmean == 'true' || ($cleanup_ensmean == 'false' && ! -s ${datapath}/${analdate}/bfg_${analdate}_${charfhr}_ensmean)) then
     ...
     srun  -N $NNODESd6 -n $nanals -c $corespermpitask --cpu_bind=cores  ${EXECGLOBAL}/getsfcensmeanp.x ${datapath}/${analdate}/ bfg_${analdate}_${charfhr}_ensmean bfg_${analdate}_${charfhr} ${nanals}  &
  endif
  @ fh = $fh + $FHOUT
end
...
while ($fh <= $FHMAX)
  ...
  if ($cleanup_ensmean == 'true' || ($cleanup_ensmean == 'false' && ! -s ${datapath}/${analdate}/sfg_${analdate}_${charfhr}_ensmean)) then
   ...
   srun  -N $NNODESd6 -n $nanals -c $corespermpitask --cpu_bind=cores  ${EXECGLOBAL}/getsigensmeanp.x ${datapath}/${analdate}/ sfg_${analdate}_${charfhr}_ensmean sfg_${analdate}_${charfhr} ${nanals} & 
  endif
  #@ fh = $fh + $FHOUT
  @ fh = $fh + $FHOUT
end
...
if ($POSTPROC == "YES") then
  ...
  srun  -N $NNODESd5 -n $nanals -c $corespermpitask --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapathm1 $IO $JO $analdatem1 p anl  &
  echo "compute ens mean panl+3 file"
  srun  -N $NNODESd5 -n $nanals -c $corespermpitask --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapathm1 $IO $JO $analdate_p3 p anl  &
  echo "compute ens mean pfg file"
  srun  -N $NNODESd5 -n $nanals -c $corespermpitask --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapath2 $IO $JO $analdate p fg 06 &
  echo "compute ens mean sflxf03 file"
  srun  -N $NNODESd5 -n $nanals -c $corespermpitask --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapath2 $LONB $LATB $analdate sflx fg 03 & 
  echo "compute ens mean sflxf06 file"
  srun  -N $NNODESd5 -n $nanals -c $corespermpitask --cpu_bind=cores  ${EXECGLOBAL}/gribmeanp.x $datapath2 $LONB $LATB $analdate sflx fg 06 & 
endif
...
wait
echo "done ens mean computation and previous cycle cleanup `date`"
...
echo "$SLURM_JOB_ID DW `date +%Y%m%d%H%M` ncycles=$ncycles $analdate $walltime $NNODES $fgmpitasks $fgcorespermpitask $enkf_taskspernode $corespermpitask $OMP_NUM_THREADS $fg_omp_threads" >> ${enkfscripts}/walltime.out
#endif
