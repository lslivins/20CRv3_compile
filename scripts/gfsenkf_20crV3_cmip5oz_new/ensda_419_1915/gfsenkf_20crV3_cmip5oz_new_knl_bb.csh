...
#
# added directives to stage data in and out of DataWarp [did not seem to have much speed effect]
...
echo "check on DW"
ls -l $DW_JOB_STRIPED/ensda_419_1914
cat $DW_JOB_STRIPED/ensda_419_1914/analdate.csh
...
setenv coresperVsmlmpitask  4 # this is usually going with 80 or 81 MPI tasks on 1 node
...
sbatch gfsenkf_20crV3_cmip5oz_new_knl_bb.csh >&! current_jobid
...
echo "$SLURM_JOB_ID DW `date +%Y%m%d%H%M` ncycles=$ncycles $analdate $walltime $NNODES $nhosts $corespernode $corespermpitask $enkf_taskspernode $OMP_NUM_THREADS $fg_omp_threads" >> ${enkfscripts}/walltime.out
#endif
