...
###module switch intel intel/16.0.3.210
 ...
 # to overcome strange SIGSEGV (segmentation fault)
  # removed 14 July as suggested by Thorsten Kurth - no longer needed
...
set ncycles=2 # number of DA cycles to run
...
setenv FHOUT 3
setenv FHZER 3 # changed from Default of 6
setenv FHLWR 3600
setenv FHSWR 3600
setenv DELTIM 1200
setenv dtphys 600
...
while ($fh <= $FHMAX)
  ...
  if ($cleanup_ensmean == 'true' || ($cleanup_ensmean == 'false' && ! -s ${datapath}/${analdate}/bfg_${analdate}_${charfhr}_ensmean)) then
     ...
     time srun  -N $NNODESd6 -n $nanals -c $corespermpitask --cpu_bind=cores  ${EXECGLOBAL}/getsfcensmeanp.x ${datapath}/${analdate}/ bfg_${analdate}_${charfhr}_ensmean bfg_${analdate}_${charfhr} ${nanals}  &
  endif
  @ fh = $fh + $FHOUT
end
...
while ($fh <= $FHMAX)
  ...
  if ($cleanup_ensmean == 'true' || ($cleanup_ensmean == 'false' && ! -s ${datapath}/${analdate}/sfg_${analdate}_${charfhr}_ensmean)) then
   ...
   time srun  -N $NNODESd6 -n $nanals -c $corespermpitask --cpu_bind=cores  ${EXECGLOBAL}/getsigensmeanp.x ${datapath}/${analdate}/ sfg_${analdate}_${charfhr}_ensmean sfg_${analdate}_${charfhr} ${nanals} & 
  endif
  #@ fh = $fh + $FHOUT
  @ fh = $fh + $FHOUT
end
...
echo "$SLURM_JOB_ID LF `date +%Y%m%d%H%M` ncycles=$ncycles $analdate $walltime $NNODES $fgmpitasks $fgcorespermpitask $enkf_taskspernode $corespermpitask $OMP_NUM_THREADS $fg_omp_threads" >> ${enkfscripts}/walltime.out
#endif
