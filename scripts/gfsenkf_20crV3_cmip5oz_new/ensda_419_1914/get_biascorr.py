...
for ncode in range(300,400):
    ...
    # 31 March 2007 make errors 
    # and the same smaller based on 
    # 30 September 2008 email with C. Landsea
    if cat in [0,1]:
        oberrdict[ncode] = 2.5#3.0
    elif cat == 2:
        oberrdict[ncode] = 2.5#3.5
    else:
        oberrdict[ncode] = 2.5#4.5
	#Pac Storms should have larger error
    # 31 March 2017 make all have slightly larger error, G. Compo
    if basin in [1,2,8]:
	oberrdict[ncode] = 1.1*oberrdict[ncode]
    # S. Hem storms should have larger error.
    # 31 March 2017 make all have slightly larger error, G. Compo
    if basin in [5,6,7]:
        oberrdict[ncode] = 1.1*oberrdict[ncode] 
# observations derived from wind.
# 2 June 2017 make these slightly smaller, L. Slivinski
for ncode in range(500,599,10):
    basin = int(str(ncode)[-2])
    oberrdict[ncode]   = 2.75#4.0
    oberrdict[ncode+1] = 2.75#3.0#5.0
    oberrdict[ncode+2] = 2.75#3.25#5.5
    oberrdict[ncode+3] = 3#3.5#6.0
    oberrdict[ncode+4] = 3#4.0#6.0
    oberrdict[ncode+5] = 3#5.0#6.0 
    #if basin in [1,2,8]:
