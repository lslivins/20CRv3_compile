...
expt = 'gfsenkf_t126_1999iau_biascor2'
date1 = sys.argv[1]
date2 = sys.argv[2]
...
lats = None
for date in dates:
    filefg='/scratch2/scratchdirs/whitaker/%s/%s/pgrbensmeanfg_%s_fhr06' % (expt,date,date)
    fileanl='/scratch2/scratchdirs/whitaker/%s/%s/pgrbensmeananl_%s' % (expt,date,date)
    ...
    psinc = 0.01*(grbanl.values - grbfg.values)
...
plt.title('ps inc (A-F) %s-%s' % (date1,date2))
