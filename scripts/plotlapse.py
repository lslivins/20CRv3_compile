...
filename = sys.argv[1]
f = open(filename)
data=N.reshape(N.fromstring(f.read(nlons*nlats*4*2),'<f'),(2,nlats,nlons))
f.close()
...
im =\
m.contourf(x,y,data[0,:,:],clevs,extend='both',cmap=plt.cm.hot_r)
...
m.drawcoastlines(linewidth=1.5,color='w')
im =\
m.contourf(x,y,data[1,:,:],clevs,extend='both',cmap=plt.cm.hot_r)
plt.colorbar(im,shrink=0.8)
# draw parallels and meridians.
delat = 30.
circles = N.arange(0.,90.,delat).tolist()+\
