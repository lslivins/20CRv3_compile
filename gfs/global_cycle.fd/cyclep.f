...
! redirect standard output to a file for each group
      ...
      if (nproc.eq.0) write(6,*) 'running on ',numproc, ' processors'
!
      use_ufo = .false.

      !READ(5,NAMCYC)
      write(inputfile,"('global_cycle.nml.mem',i3.3)") nproc+1  
      write(6,*) 'namelist:',trim(inputfile)
...
!
      write(6,*) LUGB,IDIM,JDIM,LSOIL,DELTSFC,
     1            IY,IM,ID,IH,FH
...
!CFPP$ NOCONCUR R
     ...
     2        OROIBM, MSKIBM, BGIIBM, BGOIBM, use_ufo
      ...
      integer, parameter :: nlunit=912, me=0
!
...
!
      ...
      ENDIF
      if (.not. use_ufo) then
        write(6,*) ' FNOROG_UF=',FNOROG_UF, ' but use_ufo=',use_ufo
        write(6,*)' Resetting FNOROG_UF to blank'
        FNOROG_uf = '        '
        write(6,*)' FNOROG_UF=',FNOROG_UF, ' use_ufo=',use_ufo
      endif
!
      IJDIM = IDIM * JDIM
!
...
!
      write(6,*)' calling mskrd idim,jdim,ijdim=',idim,jdim,ijdim
...
!
        ...
        write(6,*)' TOTAL Number of Points = ', LENSFC
!       do i=1,lensfc
!         write(6,*)' I=',I,' RLO=',RLO(KK+I-1),' RLA=',RLA(KK+I-1)
!       enddo
      ELSE
...
!     write(6,*)' tsfinp=',(tsffcs(419422+i),i=1,5)
!     write(6,*)' sicinp=',(sicfcs(419422+i),i=1,5)
!
...
!     write(6,*)' tsfoup=',(tsffcs(419422+i),i=1,5)
!     write(6,*)' sicoup=',(sicfcs(419422+i),i=1,5)
!
      DO K=1,LSOIL
        II = (K-1)*LENSFC
        DO I=1,LENSFC
      ...
      do ii = 1, lensfc
        if (slmask(ii) .eq. 1 .and. vetfcs(ii) .eq. 13) then
          if (tg3fcs(ii) > 273.16) then
            write(6,*) 'tg3 rla, rlo , orog ',ii,rla(ii),rlo(ii),
     & orog(ii),tg3fcs(ii)
          end if
        end if
      enddo
...
!
!
      write(6,*)' fnbgso=',fnbgso
      write(6,*)' idate=',idate,' im=',im,' jdim=',jdim,' lsoil=',lsoil
...
!
      ...
      write(6,*)' IRET after sfcio_srohdc'
      if (iret .ne. 0) then
        WRITE(6,*) ' ERROR IN OPENING FILE ',trim(FNBGSI),' iret=',iret
        CALL ABORT
      endif
      write(6,*)' FNBGSI=',trim(FNBGSI)
...
c
     ...
     &               CHAR(0)//CHAR(0)//CHAR(0)//CHAR(0)
        ...
        head%zsoil   = zsoil
!
        write(6,99) nw,head%fhour,head%IDATE,
     &           head%lonb,head%latb,head%ivs
