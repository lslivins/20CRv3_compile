...
!        based on whether the old or new radiation is selected
     ...
     &          SICSMX=1.0,SICSMN=0.0,SICIMX=1.0,SICIMN=0.15, ! SICIMIN changed from 0.5 to 0.15 by jsw
     &          SICJMX=1.0,SICJMN=0.15) ! SICJMN changed from 0.5 to 0.15 by jsw
!
...
!  DEFAULT ice concentration limit (50%), new ice thickness (20cm)
      ...
      !DATA AISLIM/0.15/,    SIHNEW/0.2/  ! jsw changed
...
!
      ...
      iprnt = 1
!     do i=1,len
!       if (ifp .eq. 0 .and. rla(i) .gt. 80.0) write (6,*) ' rla=',rla(i)
!    *,' rlo=',rlo(i)
...
!         lprnt = .true.
!         iprnt = i
!         write (6,*) ' lprnt=',lprnt,' iprnt=',iprnt
!         write (6,*) ' rla(i)=',rla(i),' rlo(i)=',rlo(i)
!       endif
!     enddo
      if (ialb == 1) then
        kpdabs = kpdabs_1
      ...
      IF(IFP.EQ.0) THEN
        ...
        DO K=1,LSOIL
          ...
          FSTCS(K) = 0.
        ENDDO
!     write (6,*) ' IN SFCSUB NLUNIT=',NLUNIT,' me=',me,' ialb=',ialb
        rewind(NLUNIT)
        READ (NLUNIT,NAMSFC)
!       WRITE(6,NAMSFC)
!
        if (me .eq. 0) then
          write (6,*) 'FTSFL,FALBL,FAISL,FSNOL,FZORL=',
     &    FTSFL,FALBL,FAISL,FSNOL,FZORL
          write (6,*) 'FSMCL=',FSMCL(1:LSOIL)
          write (6,*) 'FSTCL=',FSTCL(1:LSOIL)
          write (6,*) 'FTSFS,FALBS,FAISS,FSNOS,FZORS=',
     &    FTSFS,FALBS,FAISS,FSNOS,FZORS
          write (6,*) 'FSMCS=',FSMCS(1:LSOIL)
          write (6,*) 'FSTCS=',FSTCS(1:LSOIL)
          write (6,*) ' AISLIM=',aislim,' SIHNEW=',SIHNEW
        endif
!
        DELTF = DELTSFC / 24.0
!
...
! gpc do not impose any max ice limits 
    	CRIT=0.0 
...
!
      DO I=1,LEN
        ...
        icefl1(i) = .true.
      ENDDO
!     if(lprnt) write (6,*) ' tsffcsIN=',tsffcs(iprnt)
!
...
!     if(lprnt) write (6,*) 'tsfclm=',tsfclm(iprnt),' tsfcl2=',tsfcl2(iprnt)
!
...
!     if(lprnt) write (6,*) ' aisclm=',aisclm(iprnt),' sliclm='
...
!
      IF (MONCLM) THEN
       if (me .eq. 0) then
        write(6,*) ' '
        write(6,*) 'MONITOR OF TIME AND SPACE INTERPOLATED CLIMATOLOGY'
        write(6,*) ' '
...
!Clu [+1L] add irt() for vmn, vmx, slp, abs
     &,          IRTVMN,IRTVMX,IRTSLP,IRTABS, 
     &           IMSK, JMSK, SLMSKH, RLA, RLO, GAUSM, BLNMSK, BLTMSK,me)
!     if(lprnt) write (6,*) ' tsfanl=',tsfanl(iprnt)
     ...
     &     SICANL(I).NE.1.) THEN
        ...
        DO I=1,LEN
          IF((SLIANL(I).EQ.0.).AND.(SICANL(I).GE.CRIT)) THEN
            SLIANL(I)=2.
!           write(6,*) 'cycle - NEW ICE FORM: FICE=',SICANL(I)
          ELSE IF((SLIANL(I).GE.2.).AND.(SICANL(I).LT.CRIT)) THEN
            SLIANL(I)=0.
!           write(6,*) 'cycle - ICE FREE: FICE=',SICANL(I)
          ELSE IF((SLIANL(I).EQ.1.).AND.(SICANL(I).GE.SICIMN)) THEN
!           write(6,*) 'cycle - LAND COVERED BY SEA-ICE: FICE=',SICANL(I)
            SICANL(I)=0.
          ENDIF
        ENDDO
!       ZNNT=10.
!       CALL NNTPRT(ACNANL,LEN,ZNNT)
!     if(lprnt) write (6,*) ' acnanl=',acnanl(iprnt)
!       DO I=1,LEN
...
!     if(lprnt) write (6,*) ' acnanl=',acnanl(iprnt)
        ...
        ENDDO
      ENDIF
!     if(lprnt) write (6,*) ' aisanl1=',aisanl(iprnt),' glacir='
...
!     if(lprnt) write (6,*) ' aisanl=',aisanl(iprnt),' slianl='
...
!
      IF (MONANL) THEN
       if (me .eq. 0) then
        write(6,*) ' '
        write(6,*) 'MONITOR OF TIME AND SPACE INTERPOLATED ANALYSIS'
        write(6,*) ' '
...
!     if(lprnt) write (6,*) ' tsffcs=',tsffcs(iprnt)
!
      PERCRIT=CRITP3
!
...
!cggg landice end
      CALL SETZRO(SNOANL,EPSSNO,LEN)
!     if(lprnt) write (6,*) ' tanlm=',tsfanl(iprnt),' tfcsm=',tsffcs(iprnt)
!     if(lprnt) write (6,*) ' sliam=',slianl(iprnt),' slifm=',slifcs(iprnt)
...
!     if(lprnt) write (6,*) 'tsfanl=',tsfanl(iprnt),' tsffcs=',tsffcs(iprnt)
!     if(lprnt) write (6,*) ' slian=',slianl(iprnt),' slifn=',slifcs(iprnt)
!
...
!
!     if(lprnt) write (6,*) ' tsfbc=',tsfanl(iprnt)
      if (use_ufo) then
        ZTSFC = 1.
...
!     if(lprnt) write (6,*) ' tsfaf=',tsfanl(iprnt)
!
...
!
      IF (MONMER) THEN
       if(me .eq. 0) then
        write(6,*) ' '
        write(6,*) 'MONITOR OF UPDATED SURFACE FIELDS'
        write(6,*) '   (Includes angulation correction)'
        write(6,*) ' '
!       CALL COUNT(SLIANL,SNOANL,LEN)
        write(6,*) ' '
        CALL MONITR('TSFANL',TSFANL,SLIANL,SNOANL,LEN)
...
!           TSFFCS(I) = TGICE
      ...
      DO I=1,LEN
        ...
        ELSE IF ((SLIFCS(I).GE.1.5).AND.(SICFCS(I).LT.CRIT)) THEN
          write(6,*) 'WARNING: CHECK, SLIFCS and SICFCS',
     &            SLIFCS(I),SICFCS(I)
        ENDIF
      ENDDO
...
!
       ...
       if(me .eq. 0) then
       write (6,*) 'DBGX --fixratio:',(FIXRATIO(K),K=1,LSOIL)
       endif
...
!
      IF(FSNOL .LT. 99999.) THEN  
       if(me .eq. 0) then
       write (6,*) 'DBGX -- scale snwdph from sheleg'
       endif
       DO I = 1, LEN
...
!Clu---------------------------------------------------------------------------
!
!     if(lprnt) write (6,*) ' tsffcsF=',tsffcs(iprnt)
      RETURN
...
!
      ...
      write(6,*) '1) NO. OF NOT SNOW-COVERED LAND POINTS   ',L0,' ',RL0
      write(6,*) '2) NO. OF SNOW COVERED LAND POINTS       ',L3,' ',RL3
      write(6,*) '3) NO. OF OPEN SEA POINTS                ',L1,' ',RL1
      write(6,*) '4) NO. OF NOT SNOW-COVERED SEAICE POINTS ',L2,' ',RL2
      write(6,*) '5) NO. OF SNOW COVERED SEA ICE POINTS    ',L4,' ',RL4
      write(6,*) ' '
      write(6,*) '6) NO. OF LAND POINTS                    ',L5,' ',RL5
      write(6,*) '7) NO. SEA POINTS (INCLUDING SEA ICE)    ',L7,' ',RL7
      write(6,*) '   (NO. OF SEA ICE POINTS)          (',L6,')',' ',RL6
      write(6,*) ' '
      write(6,*) '9) NO. OF TOTAL GRID POINTS               ',L8
!     write(6,*) ' '
!     write(6,*) ' '
...
!
      WRITE(6,100) LFLD
      WRITE(6,101) RMAX(1),RMIN(1)
      WRITE(6,102) RMAX(2),RMIN(2), RMAX(4), RMIN(4)
      WRITE(6,103) RMAX(3),RMIN(3), RMAX(5), RMIN(5)
!
!     WRITE(6,*) 102,RMAX(2),RMIN(2)
!     WRITE(6,*) 103,RMAX(3),RMIN(3)
!     WRITE(6,*) 104,RMAX(4),RMIN(4)
!     WRITE(6,*) 105,RMAX(5),RMIN(5)
  100 FORMAT('0  *** ',A8,' ***')
...
!
      if (me == 0) write (6,*) ' NUM_THREADS =',NUM_THREADS,' me=',me
!
!     if(me .eq. 0) then
!     write(6,*) 'RLON=',RLON,' me=',me
!     write(6,*) 'RLAT=',RLAT,' me=',me,' imxin=',imxin,' jmxin=',jmxin
!     endif
!
!     DO J=1,JMXIN
!       IF(RLAT.GT.0.) THEN
...
!     if (me .eq. 0) then
!       write(6,*) 'RINLAT='
!       write(6,*) (RINLAT(J),J=1,JMXIN)
!       write(6,*) 'RINLON='
!       write(6,*) (RINLON(I),I=1,IMXIN)
!
!       write(6,*) 'OUTLAT='
!       write(6,*) (OUTLAT(J),J=1,LEN)
!       write(6,*) (OUTLON(J),J=1,LEN)
!     endif
!
!     DO I=1,IMXIN
...
!       write(6,*) 'LA2GA'
!       write(6,*) 'IINDX1'
!       write(6,*) (IINDX1(N),N=1,LEN)
!       write(6,*) 'IINDX2'
!       write(6,*) (IINDX2(N),N=1,LEN)
!       write(6,*) 'JINDX1'
!       write(6,*) (JINDX1(N),N=1,LEN)
!       write(6,*) 'JINDX2'
!       write(6,*) (JINDX2(N),N=1,LEN)
!       write(6,*) 'DDY'
!       write(6,*) (DDY(N),N=1,LEN)
!       write(6,*) 'DDX'
!       write(6,*) (DDX(N),N=1,LEN)
!     endif
!
        SUM1 = 0.
        SUM2 = 0.
...
!
        ...
        ENDIF
!
!     write (6,*) ' SUM1=',SUM1,' SUM2=',SUM2
!    *,' SUM3=',SUM3,' SUM4=',SUM4
!     write (6,*) ' RSLMSK=',(RSLMSK(I,1),I=1,IMXIN)
!     write (6,*) ' SLMASK=',(SLMASK(I),I=1,IMXOUT)
...
!
    	...
    	if (me == 0) write (6,*) ' in nearest neighbor interpolation'
          DO I=I1_T,I2_T
            JY = JINDX1(I)
...
!cggg routine assign a default.
              ...
              if (NUM_THREADS == 1) then
                write(6,*) 'no matching mask found ',i,i1,j1,ix,jx
                write(6,*) 'set to default value.'
              endif
              gauout(i) = 0.0
...
!cggg end
     ...
     &                  CALL MAXMIN(GAUOUT(I1_T),LEN_THREAD,KMAMI)
        ELSE  ! nearest neighbor interpolation
    	    ...
    	    if (me == 0) write (6,*) ' in quasi-bilinear interpolation'
...
!
     ...
     &                        * WSUMIV
                  ENDIF
!       write (6,*) ' slmask=',slmask(i),' sums=',sums,' sumn=',sumn
...
!
     ...
     &,            OUTLAT, OUTLON, me)
        ...
        ELSEIF(IRET.EQ.-1) THEN
          if (me .eq. 0) then
        write(6,*) 'OLD T SURFACE ANALYSIS PROVIDED, Indicating proper',
     &            ' file name is given.  No error suspected.'
          WRITE(6,*) 'FORECAST GUESS WILL BE USED'
          endif
        ELSE
          if (me .eq. 0) write(6,*) 'T SURFACE ANALYSIS PROVIDED.'
        ENDIF
      ELSE
        if (me .eq. 0) then
!       write(6,*) '************************************************'
        write(6,*) 'NO TSF ANALYSIS AVAILABLE.  CLIMATOLOGY USED'
        endif
      ENDIF
!
! TSF0
...
!         CALL ABORT
!       ELSE
!         write(6,*) 'T SURFACE ANALYSIS AT FT=0 FOUND.'
!       ENDIF
!     ELSE
!       DO I=1,LEN
!         TSFAN0(I)=-999.9
...
!
     ...
     &,              OUTLAT, OUTLON, me)
          ...
          ELSEIF(IRET.EQ.-1) THEN
            if (me .eq. 0) then
           write(6,*) 'OLD ALBEDO ANALYSIS PROVIDED, Indicating proper',
     &              ' file name is given.  No error suspected.'
            WRITE(6,*) 'FORECAST GUESS WILL BE USED'
            endif
          ELSE
            if (me .eq. 0 .and. kk .eq. 4)
     &                  write(6,*) 'ALBEDO ANALYSIS PROVIDED.'
          ENDIF
        ENDDO
      ELSE
        if (me .eq. 0) then
!       write(6,*) '************************************************'
        write(6,*) 'NO ALBEDO ANALYSIS AVAILABLE.  CLIMATOLOGY USED'
        endif
      ENDIF
!
...
!
     ...
     &                  write(6,*) 'ALBEDO ANALYSIS PROVIDED.'
      ...
      ELSE
        if (me .eq. 0) then
!       write(6,*) '************************************************'
        write(6,*) 'NO VEGFALBEDO ANALYSIS AVAILABLE.  CLIMATOLOGY USED'
        endif
      ENDIF
!
!  Soil Wetness
!
     ...
     &,            OUTLAT, OUTLON, me)
        ...
        ELSEIF(IRET.EQ.-1) THEN
          if (me .eq. 0) then
          write(6,*) 'OLD WETNESS ANALYSIS PROVIDED, Indicating proper',
     &            ' file name is given.  No error suspected.'
          WRITE(6,*) 'FORECAST GUESS WILL BE USED'
          endif
        ELSE
          if (me .eq. 0) write(6,*) 'BUCKET WETNESS ANALYSIS PROVIDED.'
        ENDIF
     ...
     &,            OUTLAT, OUTLON, me)
        ...
        ELSEIF(IRET.EQ.-1) THEN
          if (me .eq. 0) then
          write(6,*) 'OLD LAYER SOIL WETNESS ANALYSIS PROVIDED',
     &            ' Indicating proper file name is given.'
          write(6,*) ' No error suspected.'
          WRITE(6,*) 'FORECAST GUESS WILL BE USED'
          endif
        ELSE
      if (me .eq. 0) write(6,*) 'LAYER SOIL WETNESS ANALYSIS PROVIDED.'
        ENDIF
      ELSE
        if (me .eq. 0) then
!       write(6,*) '************************************************'
      write(6,*) 'NO SOIL WETNESS ANALYSIS AVAILABLE.  CLIMATOLOGY USED'
        endif
      ENDIF
!
...
!cggg snow mods end
        ...
        ELSEIF(IRET.EQ.-1) THEN
          if (me .eq. 0) then
       write(6,*) 'OLD SNOW DEPTH ANALYSIS PROVIDED, Indicating proper',
     &            ' file name is given.  No error suspected.'
          WRITE(6,*) 'FORECAST GUESS WILL BE USED'
          endif
        ELSE
          if (me .eq. 0) write(6,*) 'SNOW DEPTH ANALYSIS PROVIDED.'
        ENDIF
        IRTSNO=0
     ...
     &,            OUTLAT, OUTLON, me)
        ...
        ELSEIF(IRET.EQ.-1) THEN
          if (me .eq. 0) then
        write(6,*)'OLD SNOW COVER ANALYSIS PROVIDED, Indicating proper',
     &            ' file name is given.  No error suspected.'
          WRITE(6,*) 'FORECAST GUESS WILL BE USED'
          endif
        ELSE
          if (me .eq. 0) write(6,*) 'SNOW COVER ANALYSIS PROVIDED.'
        ENDIF
      ELSE
        if (me .eq. 0) then
!       write(6,*) '************************************************'
        write(6,*)'NO SNOW/SNOCOV ANALYSIS AVAILABLE.  CLIMATOLOGY USED'
        endif
      ENDIF
!
!  Sea ice mask
!
     ...
     &,            OUTLAT, OUTLON, me)
        ...
        ELSEIF(IRET.EQ.-1) THEN
          if (me .eq. 0) then
          write(6,*) 'OLD ICE CONCENTRATION ANALYSIS PROVIDED',
     &            ' Indicating proper file name is given'
          write(6,*) ' No error suspected.'
          WRITE(6,*) 'FORECAST GUESS WILL BE USED'
          endif
        ELSE
        if (me .eq. 0) write(6,*) 'ICE CONCENTRATION ANALYSIS PROVIDED.'
        ENDIF
     ...
     &,            OUTLAT, OUTLON, me)
        ...
        ELSEIF(IRET.EQ.-1) THEN
          if (me .eq. 0) then
        write(6,*) 'OLD ICE-MASK ANALYSIS PROVIDED, Indicating proper',
     &            ' file name is given.  No error suspected.'
          WRITE(6,*) 'FORECAST GUESS WILL BE USED'
          endif
        ELSE
          if (me .eq. 0) write(6,*) 'ICE MASK ANALYSIS PROVIDED.'
        ENDIF
      ELSE
        if (me .eq. 0) then
!       write(6,*) '************************************************'
        write(6,*) 'NO SEA-ICE ANALYSIS AVAILABLE.  CLIMATOLOGY USED'
        endif
      ENDIF
!
!  Surface Roughness
!
     ...
     &,            OUTLAT, OUTLON, me)
        ...
        ELSEIF(IRET.EQ.-1) THEN
          if (me .eq. 0) then
        write(6,*) 'OLD ROUGHNESS ANALYSIS PROVIDED, Indicating proper',
     &            ' file name is given.  No error suspected.'
          WRITE(6,*) 'FORECAST GUESS WILL BE USED'
          endif
        ELSE
        if (me .eq. 0) write(6,*) 'ROUGHNESS ANALYSIS PROVIDED.'
        ENDIF
      ELSE
          if (me .eq. 0) then
!       write(6,*) '************************************************'
      write(6,*)'NO SRFC ROUGHNESS ANALYSIS AVAILABLE. CLIMATOLOGY USED'
        endif
      ENDIF
!
...
!
     ...
     &,            OUTLAT, OUTLON, me)
        ...
        ELSEIF(IRET.EQ.-1) THEN
          if (me .eq. 0) then
          write(6,*) 'OLD DEEP SOIL TEMP ANALYSIS PROVIDED',
     &            ' Indicating proper file name is given.'
          write(6,*) ' No error suspected.'
          WRITE(6,*) 'FORECAST GUESS WILL BE USED'
          endif
        ELSE
          if (me .eq. 0) write(6,*) 'DEEP SOIL TMP ANALYSIS PROVIDED.'
        ENDIF
     ...
     &            'iIndicating proper file name is given.'
        ...
        ELSE
          if (me .eq. 0) write(6,*) 'LAYER SOIL TMP ANALYSIS PROVIDED.'
        ENDIF
      ELSE
        if (me .eq. 0) then
!       write(6,*) '************************************************'
       write(6,*) 'NO DEEP SOIL TEMP ANALY AVAILABLE.  CLIMATOLOGY USED'
        endif
      ENDIF
!
!  VEGETATION COVER
!
     ...
     &,            OUTLAT, OUTLON, me)
        ...
        ELSEIF(IRET.EQ.-1) THEN
          if (me .eq. 0) then
          write(6,*) 'OLD VEGETATION COVER ANALYSIS PROVIDED',
     &            ' Indicating proper file name is given.'
          write(6,*) ' No error suspected.'
          WRITE(6,*) 'FORECAST GUESS WILL BE USED'
          endif
        ELSE
        if (me .eq. 0) write(6,*) 'GEGETATION COVER ANALYSIS PROVIDED.'
        ENDIF
      ELSE
        if (me .eq. 0) then
!       write(6,*) '************************************************'
      write(6,*) 'NO VEGETATION COVER ANLY AVAILABLE. CLIMATOLOGY USED'
        endif
      ENDIF
!
!  VEGETATION type
!
     ...
     &,            OUTLAT, OUTLON, me)
        ...
        ELSEIF(IRET.EQ.-1) THEN
          if (me .eq. 0) then
          write(6,*) 'OLD VEGETATION type ANALYSIS PROVIDED',
     &            ' Indicating proper file name is given.'
          write(6,*) ' No error suspected.'
          WRITE(6,*) 'FORECAST GUESS WILL BE USED'
          endif
        ELSE
          if (me .eq. 0) write(6,*) 'VEGETATION type ANALYSIS PROVIDED.'
        ENDIF
      ELSE
        if (me .eq. 0) then
!       write(6,*) '************************************************'
        write(6,*) 'NO VEGETATION type ANLY AVAILABLE. CLIMATOLOGY USED'
        endif
      ENDIF
!
!  soil type
!
     ...
     &,            OUTLAT, OUTLON, me)
        ...
        ELSEIF(IRET.EQ.-1) THEN
          if (me .eq. 0) then
          write(6,*) 'OLD soil type ANALYSIS PROVIDED',
     &            ' Indicating proper file name is given.'
          write(6,*) ' No error suspected.'
          WRITE(6,*) 'FORECAST GUESS WILL BE USED'
          endif
        ELSE
          if (me .eq. 0) write(6,*) 'soil type ANALYSIS PROVIDED.'
        ENDIF
      ELSE
        if (me .eq. 0) then
!       write(6,*) '************************************************'
        write(6,*) 'NO soil type ANLY AVAILABLE. CLIMATOLOGY USED'
        endif
      ENDIF
...
!
     ...
     &,            OUTLAT, OUTLON, me)
        ...
        ELSEIF(IRET.EQ.-1) THEN
          if (me .eq. 0) then
          write(6,*) 'OLD shdmin ANALYSIS PROVIDED',
     &            ' Indicating proper file name is given.'
          write(6,*) ' No error suspected.'
          WRITE(6,*) 'FORECAST GUESS WILL BE USED'
          endif
        ELSE
          if (me .eq. 0) write(6,*) 'shdmin ANALYSIS PROVIDED.'
        ENDIF
      ELSE
        if (me .eq. 0) then
!       write(6,*) '************************************************'
        write(6,*) 'NO shdmin ANLY AVAILABLE. CLIMATOLOGY USED'
        endif
      ENDIF
...
!
     ...
     &,            OUTLAT, OUTLON, me)
        ...
        ELSEIF(IRET.EQ.-1) THEN
          if (me .eq. 0) then
          write(6,*) 'OLD shdmax ANALYSIS PROVIDED',
     &            ' Indicating proper file name is given.'
          write(6,*) ' No error suspected.'
          WRITE(6,*) 'FORECAST GUESS WILL BE USED'
          endif
        ELSE
          if (me .eq. 0) write(6,*) 'shdmax ANALYSIS PROVIDED.'
        ENDIF
      ELSE
        if (me .eq. 0) then
!       write(6,*) '************************************************'
        write(6,*) 'NO shdmax ANLY AVAILABLE. CLIMATOLOGY USED'
        endif
      ENDIF
...
!
     ...
     &,            OUTLAT, OUTLON, me)
        ...
        ELSEIF(IRET.EQ.-1) THEN
          if (me .eq. 0) then
          write(6,*) 'OLD slope type ANALYSIS PROVIDED',
     &            ' Indicating proper file name is given.'
          write(6,*) ' No error suspected.'
          WRITE(6,*) 'FORECAST GUESS WILL BE USED'
          endif
        ELSE
          if (me .eq. 0) write(6,*) 'slope type ANALYSIS PROVIDED.'
        ENDIF
      ELSE
        if (me .eq. 0) then
!       write(6,*) '************************************************'
        write(6,*) 'NO slope type ANLY AVAILABLE. CLIMATOLOGY USED'
        endif
      ENDIF
...
!
     ...
     &,            OUTLAT, OUTLON, me)
        ...
        ELSEIF(IRET.EQ.-1) THEN
          if (me .eq. 0) then
          write(6,*) 'OLD snoalb ANALYSIS PROVIDED',
     &            ' Indicating proper file name is given.'
          write(6,*) ' No error suspected.'
          WRITE(6,*) 'FORECAST GUESS WILL BE USED'
          endif
        ELSE
          if (me .eq. 0) write(6,*) 'snoalb ANALYSIS PROVIDED.'
        ENDIF
      ELSE
        if (me .eq. 0) then
!       write(6,*) '************************************************'
        write(6,*) 'NO snoalb ANLY AVAILABLE. CLIMATOLOGY USED'
        endif
      ENDIF
...
!
      if(raiss.eq.1..or.irtacn.eq.-1) then
        if (me .eq. 0) write (6,*) 'use forecast land-sea-ice mask'
        do i = 1, LEN
          aisanl(i) = aisfcs(i)
          slianl(i) = slifcs(i)
        enddo
...
!     write (6,*) ' ralfl=',ralfl,' ralfs=',ralfs,' rsotl=',rsotl
...
!CluX  
      if(me .eq. 0) then
        write (6,*)  'DBGX-- CSMCL:', (CSMCL(K),K=1,LSOIL)
        write (6,*)  'DBGX-- RSMCL:', (RSMCL(K),K=1,LSOIL)
        write (6,*)  'DBGX-- CSNOL, CSNOS:',CSNOL,CSNOS
        write (6,*)  'DBGX-- RSNOL, RSNOS:',RSNOL,RSNOS
      endif

!     write (6,*)  RTSFS, QTSFS, RAISS , QAISS
!    *,        RSNOS , QSNOS, RZORS , QZORS, RVEGS , QVEGS
!    *,        RvetS , QvetS, RsotS , QsotS
!    *,        RCV, RCVB, RCVT, QCV, QCVB, QCVT
!    *,        RALBS, QALBS, RALFS, QALFS
!     write (6,*)  RTSFL, QTSFL, RAISL , QAISL
      ...
      DO I=1,LEN
        IF(SLIFCS(I).NE.SLIANL(I)) THEN
          IF(SLIFCS(I).EQ.1..OR.SLIANL(I).EQ.1.) THEN
            write(6,*) 'INCONSISTENCY IN SLIFCS OR SLIANL'
            WRITE(6,910) RLA(I),RLO(I),SLIFCS(I),SLIANL(I),
     &                TSFFCS(I),TSFANL(I)
...
!         IF(GLACIR(I).NE.0..AND.SNOANL(I).LT.SNOVAL*0.5) THEN
        ...
        IF(KOUNT.GT.0) THEN
          if (me .eq. 0) then
          write(6,*) 'SNOW filled over glacier points at ',KOUNT,
     &            ' POINTS (',PER,'percent)'
          endif
        ENDIF
!cggg landice mods start
...
!cggg landice mods end
      ...
      IF(KOUNT.GT.0) THEN
        if (me .eq. 0) then
        write(6,*) 'SNOW set to zero over open sea at ',KOUNT,
     &          ' POINTS (',PER,'percent)'
        endif
      ENDIF
      RETURN
...
!
      ...
      DO I=1,LEN
        IF(AIS(I).NE.AICICE.AND.AIS(I).NE.AICSEA) THEN
          write(6,*) 'SEA ICE MASK NOT ',AICICE,' OR ',AICSEA
          write(6,*) 'AIS(I),AICICE,AICSEA,RLA(I),RLO(I,=',
...
!     ENDDO
      ...
      IF(KOUNT.GT.0) THEN
        if(me .eq. 0) then
        write(6,*) ' Sea ice over land mask at ',KOUNT,' points (',PER,
     &          'percent)'
        endif
      ENDIF
      ...
      IF(KOUNT1.GT.0) THEN
        if(me .eq. 0) then
        write(6,*) ' Sea ice set over glacier points over ocean at ',
...
!     IF(KOUNT.GT.0) THEN
!       write(6,*) ' Sea ice exceeds maxice at ',KOUNT,' points (',PER,
!    &          'percent)'
!     ENDIF
!
...
!
      if(me .eq. 0) then
!     write(6,*) ' '
      write(6,*) 'Performing QC of ',TTL,' MODE=',MODE,
...
!
     ...
     &         FLD(I).LT.FLDLMN-EPSFLD) THEN
          ...
          if(me == 0 . and. it == 1 .and. NUM_THREADS == 1) then
            ...
            DO I=1,NPRT
              IJ = IWK(I)
              WRITE(6,8001) RLA(IJ),RLO(IJ),FLD(IJ),FLDLMN
...
!
     ...
     &         FLD(I).GT.FLDLMX+EPSFLD) THEN
          ...
          if(me == 0 . and. it == 1 .and. NUM_THREADS == 1) then
            ...
            DO I=1,NPRT
              IJ = IWK(I)
              WRITE(6,8002) RLA(IJ),RLO(IJ),FLD(IJ),FLDLMX
...
!
     ...
     &         FLD(I).LT.FLDSMN-EPSFLD) THEN
          ...
          if(me == 0 . and. it == 1 .and. NUM_THREADS == 1) then
            ...
            DO I=1,NPRT
              IJ = IWK(I)
              WRITE(6,8003) RLA(IJ),RLO(IJ),FLD(IJ),FLDSMN
...
!
     ...
     &         FLD(I).GT.FLDSMX+EPSFLD) THEN
          ...
          if(me == 0 . and. it == 1 .and. NUM_THREADS == 1) then
            ...
            DO I=1,NPRT
              IJ = IWK(I)
              WRITE(6,8004) RLA(IJ),RLO(IJ),FLD(IJ),FLDSMX
...
!
     ...
     &         FLD(I).LT.FLDOMN-EPSFLD) THEN
          ...
          if(me == 0 . and. it == 1 .and. NUM_THREADS == 1) then
            ...
            DO I=1,NPRT
              IJ = IWK(I)
              WRITE(6,8005) RLA(IJ),RLO(IJ),FLD(IJ),FLDOMN
...
!
     ...
     &         FLD(I).GT.FLDOMX+EPSFLD) THEN
          ...
          if(me == 0 . and. it == 1 .and. NUM_THREADS == 1) then
            ...
            DO I=1,NPRT
              IJ = IWK(I)
              WRITE(6,8006) RLA(IJ),RLO(IJ),FLD(IJ),FLDOMX
...
!
     ...
     &         FLD(I).LT.FLDIMN-EPSFLD) THEN
          ...
          if(me == 0 . and. it == 1 .and. NUM_THREADS == 1) then
            ...
            DO I=1,NPRT
              IJ = IWK(I)
              WRITE(6,8007) RLA(IJ),RLO(IJ),FLD(IJ),FLDIMN
...
!    &         FLD(I).GT.FLDIMX+EPSFLD) THEN
          ...
          if(me == 0 . and. it == 1 .and. NUM_THREADS == 1) then
            ...
            DO I=1,NPRT
              IJ = IWK(I)
              WRITE(6,8008) RLA(IJ),RLO(IJ),FLD(IJ),FLDIMX
...
!
     ...
     &         FLD(I).LT.FLDJMN-EPSFLD) THEN
          ...
          if(me == 0 . and. it == 1 .and. NUM_THREADS == 1) then
            ...
            DO I=1,NPRT
              IJ = IWK(I)
              WRITE(6,8009) RLA(IJ),RLO(IJ),FLD(IJ),FLDJMN
...
!    &         FLD(I).GT.FLDJMX+EPSFLD) THEN
          ...
          if(me == 0 . and. it == 1 .and. NUM_THREADS == 1) then
            ...
            DO I=1,NPRT
              IJ = IWK(I)
              WRITE(6,8010) RLA(IJ),RLO(IJ),FLD(IJ),FLDJMX
...
!
      ...
      ENDDO
!
!     write(6,*) 'GAUSSIAN LAT (DEG) FOR JMAX=',K
!     write(6,*) (GAUL(N),N=1,K)
!
      RETURN
   70 WRITE(6,6000)
...
!
   ...
   30 CONTINUE
      ...
      if (me .eq. 0) write(6,*) 'RJDAY,SEA1,SEA2,WEI1S,WEI2S=',
...
!cbosu
        if (me == 0) write(6,*) 'bosu first time thru'
!
...
!cbosu  new snowfree albedo database is monthly.  
          if (ialb == 1) then
            write(6,*) 'first call to fixrdc for snowfree alb ', first,
     &              nn, mon
            kpd7=-1
            DO K = 1, 4
...
!     if(lprnt) write (6,*) ' mon1=',mon1,' mon2=',mon2
!
...
!
     ...
     &,              IMSK, JMSK, SLMSKH, GAUS,BLNO, BLTO
     &,              OUTLAT, OUTLON, me)
!     if(lprnt) write (6,*) ' tsf=',tsf(iprnt,nn),' nn=',nn
!
...
!
      if (me .eq. 0) write (6,*) ' mon1s=',mon1s,' mon2s=',mon2s
...
!     if(lprnt) write (6,*) ' rjdayh=',rjdayh,' mon1=',mon1,' mon2='
...
!cbosu
          if (ialb == 1) then
            if (me == 0) write(6,*) 'bosu 2nd time in clima for month ',
     &                   mon, k1,k2
            kpd7=-1
            DO K = 1, 4
              CALL FIXRDC(LUGB,FNALBC,KPDALB(K),kpd7,MON,SLMASK,
...
!
      DO I=1,LEN
        ...
        tsfcl2(I) = tsf2(i)
      ENDDO
!     if(lprnt) write (6,*) ' tsfclm=',tsfclm(iprnt),' wei1m=',wei1m
...
!     if(lprnt) write (6,*) ' tsf2=',tsf2(iprnt),' wei1x=',wei1x
...
!cbosu  diagnostic print
      if (me == 0) write(6,*) 'monthly albedo weights are ', 
...
!Clu [+1L] add slope (=236)
        ...
        if (me .eq. 0) then
        if(inttyp.eq.1) write (6,*)  ' Nearest grid point used'
...
!c gpc make sea ice interpolation simply nearest grid point 	
	IF(KPDS5.EQ.91)  INTTYP = 1 
        if(inttyp.eq.1) write (6,*)  ' Nearest grid point used'
!
        CALL LA2GA(DATA,IMAX,JMAX,RLNGRB,RLTGRB,WLON,RNLAT,INTTYP,
     &             GDATA,LEN,LMASK,RSLMSK,SLMASK
