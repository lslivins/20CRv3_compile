...
!  Allocatable data
  ...
  character(2) charnprocs_group
!!!!!!!!
  integer(kint_mpi):: global_comm, global_size, global_rank, nprocs_group, color, key, ierr
  character(50):: inputfile,stdoutfile

! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
!  Initial setup
  call mpstart(global_comm,global_rank,global_size,iret)

!  nprocs_group could be made to read in from a file or as a command line argument later, if needed
!!!!!!!!!!!!!!!!!!!
! nprocs_group=32
  call getenv("nprocs_group",charnprocs_group)
  read(charnprocs_group,'(i2)') nprocs_group
  if (nprocs_group <= 0) then
      if (global_rank .eq. 0) print *,'nprocs_group not defined'
      call mpstop(iret)
      stop
  else
      if (global_rank .eq. 0) print *,'nprocs_group=',nprocs_group
  endif
!!!!!!!!!!!!!!!!!!!
  color=global_rank/nprocs_group
  rank=global_rank-color*nprocs_group

! the total number of processors are split into global_size/nprocs_group number of sub groups, color is the gorup index
  call MPI_Comm_split(global_comm,color, key, comm,ierr)
  call MPI_Comm_rank(comm,rank,ierr)
  call MPI_Comm_size(comm,size,ierr)
  if (size.ne.nprocs_group) call MPI_Abort(ierr)

! redirect standard output to a file for each group
  write(stdoutfile,"('stdoutfile.',i2.2)") color+1
  if (rank.eq.0) open(unit=6,file=stdoutfile,action='write')

 if (global_rank.eq.0) print *, 'running on ',global_size, ' number of processors'
...
!  Set defaults and read namelist
  ...
  zz=(/12.,20.,30.,50.,80.,100.,150.,200.,250.,300.,500.,700.,1000.,(0.,k=13+1,komax)/)
  kth=3 !gpc added fpr isentropic levels
  th=(/300.,330.,350.,(0.,k=3+1,komax)/)
  kpv=8
...
! read(*,nampgb,iostat=iret)
  write(inputfile,"('global_postgp.nml.',i2.2)") color+1  
...
!  Output height above sea level scalar
  ...
  do nk=nfzzs1e,nfzzs2e
    ...
    ipue(nk)=jpdszzs(5,n)
    ipve(nk)=-1
    itle(nk)=105 ! jsw: change to 105 from 103 (above ground, not MSL)
    il1e(nk)=0
    il2e(nk)=nint(zz(k))
    ifue(nk)=1
    ip1e(nk)=nint(sighead%fhour)
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
!  Finish up
  call MPI_Barrier(global_comm,iret)
!  call mpstop(iret)
...
!  compute sundry fields, compute precipitation type, and copy flux fields.
  do i=1,im
    ...
    ! jsw hack: interpolate to height above ground, not sea level
...
! added dswrf_toa
      kpdsfoxs(i,ifoxs_dswrf_toa         )=kpdsflxs(i,iflxs_dswrf_toa         )
...
! gpc added dswrf_toa
    lfoxs(ifoxs_dswrf_toa         )=lflxs(iflxs_dswrf_toa         )
...
! gpc added dswrf_toa
    ffoxs(ifoxs_dswrf_toa         )=fflxs(iflxs_dswrf_toa         )
