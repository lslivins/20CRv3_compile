...
!     a        logical (jda,kma) input array
!   Output argument list:
!     b        logical (kdb,jmb) output array
!
! Subprograms called:
...
!$$$
  ...
  logical,dimension(jm,km,mpisize):: tatmp,tbtmp
...
!  externally transpose data
  tatmp = ta; tbtmp = tb
  call mpi_alltoall(tatmp,jm*km,MPI_LOGICAL,tbtmp,jm*km,MPI_LOGICAL,mpicomm,ierr)
  ta = tatmp; tb = tbtmp
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
!     kmb      integer(kint_mpi) output grid undecomposed range
!     kdb      integer(kint_mpi) output grid undecomposed dimension
!     a        logical (ida,jda,kma) input array
!   Output argument list:
!     b        logical (idb,kdb,jmb) output array
!
! Subprograms called:
!   mpi_alltoall  mpi exchange of data between every process pair
!
...
!$$$
  ...
  logical,dimension(im,jm,km,mpisize):: tatmp,tbtmp
...
!  externally transpose data
  tatmp = ta; tbtmp = tb
  call mpi_alltoall(ta,im*jm*km,MPI_LOGICAL,tb,im*jm*km,MPI_LOGICAL,mpicomm,ierr)
  ta = tatmp; tb = tbtmp
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
!  internally transpose output array
!$omp parallel do private(kb)
...
!$$$
  ...
  real(4), allocatable, dimension(:,:,:,:) :: ta,tb
  ...
  allocate(a(1+dot_product(im-1,isa)+&
                      dot_product(jma-1,jsa)+&
                      dot_product(kma-1,ksa)))
  allocate(b(1+dot_product(im-1,isb)+&
                      dot_product(jmb-1,jsb)+&
                      dot_product(kmb-1,ksb)))
  allocate(ta(product(im),product(jm),product(km),mpisize))
  allocate(tb(product(im),product(jm),product(km),mpisize))
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
