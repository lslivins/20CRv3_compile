# 1 "surface_chgres.F"
 module surface_chgreser
!$$$ module documentation block
!
! module: surface_chgres    interpolate land fields from one
!                           grid to another.
!   prgmmr: gayno           org: w/np2     date: 2005-10-25
!
! abstract: a collection of routines to interpolate land fields
!           from one grid to another.
!
! program history log:
!  2005-10-25  gayno   - initial version
!  2006-04-25  gayno   - use ipolates library to interpolate
!                        continous fields.  modified for use
!                        with gfs and wrf.
!
! usage: use surface_chgres
!
! attributes:
!   langauge: fortran 90
!
!$$$
!-----------------------------------------------------------------------
! some variable definitions.
!
! climo_fields_opt       option for determining climo fields on
!                        output grid.  1 ONLY!!
!                        1-interpolate all from input grid
!                        2-interpolate veg, soil, slope type
!                          from input grid.  others from
!                          sfccycle program.
!                        3-all from sfccycle program.
! landice_opt            1-no landice input grid -> landice output grid
!                        2-landice input grid -> landice output grid
!                        3-no landice input grid -> no landice output grid
!                        4-landice input grid -> no landice output grid
!                        5-landice output grid regardless of whether
!                          input grid has landice or not.
!-----------------------------------------------------------------------

     *, private                 :: climo_fields_opt  ! only for 1
     *er, private                 :: landice_opter

     *, allocatable, private    :: iindx_output(:)
     *er, allocatable, private    :: jindx_output(:)

     * private                    :: mdl_res_input   ! model resol in degrees
     * private                    :: mdl_res_output  ! model resol in degrees

!-----------------------------------------------------------------------
! these are flag values for veg and soil type at land ice points.
! they depend on the raw data source used.
!
! zobler soil type -> 9
! statsgo soil type -> 16
! usgs veg type -> 24
! sib veg type -> 13
!-----------------------------------------------------------------------

     *er, private                 :: veg_type_ice 
     *er, private                 :: veg_type_ice_inputer
     *, private                 :: soil_type_iceer

!-----------------------------------------------------------------------
! note: "_input" refers to the input grid, "_output" refers
! to the output grid, "_output_ext" refers to data on output grid
! from an external process, such as sfccycle or gayno's wrf si.
!
! smcref_input/output    onset of soil moisture stress
! smcdry_input/output    air dry soil moisture limit
! smcwilt_input/output   plant wilting point
! smclow_input/output    soil moisture scalar multiplier
! smchigh_input/output   soil moisture scalar multiplier
! smcmax_input/output    maximum soil moisture content
! beta_input/output      soil 'b' parameter
! psis_input/output      saturated soil potential
! satdk_input/output     saturated soil hydraulic conductivity
!-----------------------------------------------------------------------

     *, parameter, private      :: max_soil_types=50
     * er, parameter, private      :: max_veg_types=50
     * private                    :: salp_output
     * private                    :: snup_output(max_veg_types)

     * private                    :: beta_input(max_soil_types)
     * private                    :: beta_output(max_soil_types)
     * private                    :: psis_input(max_soil_types)
     * private                    :: psis_output(max_soil_types)
     * private                    :: satdk_input(max_soil_types)
     * private                    :: satdk_output(max_soil_types)
     * private                    :: smcdry_input(max_soil_types)
     * private                    :: smcdry_output(max_soil_types)
     * private                    :: smchigh_input
     * private                    :: smchigh_output
     * private                    :: smclow_input
     * private                    :: smclow_output
     * private                    :: smcmax_input(max_soil_types)
     * private                    :: smcmax_output(max_soil_types)
     * private                    :: smcref_input(max_soil_types)
     * private                    :: smcref_output(max_soil_types)
     * private                    :: smcwilt_input(max_soil_types)
     * private                    :: smcwilt_output(max_soil_types)

!-----------------------------------------------------------------------
! note: "_input" refers to the input grid, "_output" refers
! to the output grid, "_output_ext" refers to data on output grid
! from an external process, such as sfccycle or gayno's wrf si.
!
! alnsf            - near ir albedo, strong cosz dependence
! alnwf            - near ir albedo, weak cosz dependence
! alvsf            - vis albedo, strong cosz dependence
! alvwf            - vis albedo, weak cosz dependence
! canopy_mc        - canopy moisture content
! facsf            - fraction, strong cosz dependence
! facwf            - fraction, weak cosz dependence
! sea_ice_fract    - sea ice fraction, decimal
! greenfrc         - greenness fraction
! greenfrc_max     - max annual greenness fraction
! greenfrc_min     - min annual greenness fraction
! sea_ice_depth    - sea ice depth
! lsmask           - land/sea mask
! mxsnow_alb       - maximum snow albedo
! orog             - orography
! sea_ice_flag     - yes/no sea ice flag
! skin_temp        - skin temperature, sst over water
! snow_depth       - physical snow depth
! snow_liq_equiv   - liq equivalent snow depth
! soilm_liq        - liquid soil moisture
! soilm_tot        - total soil moisture
! soil_temp        - soil temperature
! slope_type       - soil slope type (category)
! soil_type        - soil type (category)
! substrate_temp   - soil substrate temperature
! veg_type         - vegetation type (category)
! z0               - roughness length
!-----------------------------------------------------------------------

     *er, allocatable, private              :: slope_type_output_ext(:)
     *er, allocatable, private              :: soil_type_output_ext(:)
     *er, allocatable, private              :: veg_type_output_ext(:)

     * allocatable, private                 :: alnsf_output_ext(:)
     * allocatable, private                 :: alnwf_output_ext(:)
     * allocatable, private                 :: alvsf_output_ext(:)
     * allocatable, private                 :: alvwf_output_ext(:)
     * allocatable, private                 :: facsf_output_ext(:)
     * allocatable, private                 :: facwf_output_ext(:)
     * allocatable, private                 :: greenfrc_output_ext(:)   
     * allocatable, private                 :: greenfrc_max_output_ext(:
     * allocatable, private                 :: greenfrc_min_output_ext(:
     * allocatable, private                 :: mxsnow_alb_output_ext(:) 
     * allocatable, private                 :: snow_free_albedo_output_e
     * allocatable, private                 :: substrate_temp_output_ext
     * allocatable, private                 :: z0_output_ext(:)         

!-----------------------------------------------------------------------
! these structures are to be used by the program that uses this
! module to hold the input and output data.
!-----------------------------------------------------------------------

     * public :: sfc1dl
     *, allocatable    :: albedo(:)
     *l, allocatable    :: alnsf(:)
     *l, allocatable    :: alnwf(:)
     *l, allocatable    :: alvsf(:)
     *l, allocatable    :: alvwf(:)
     *l, allocatable    :: canopy_mc(:)
     *l, allocatable    :: facsf(:)
     *l, allocatable    :: facwf(:)
     *l, allocatable    :: sea_ice_fract(:)
     *l, allocatable    :: greenfrc(:)
     *l, allocatable    :: greenfrc_max(:)
     *l, allocatable    :: greenfrc_min(:)
     *l, allocatable    :: sea_ice_depth(:)
     *l, allocatable    :: lats(:)
     *l, allocatable    :: lons(:)
     *l, allocatable    :: lsmask(:)
     *l, allocatable    :: mxsnow_alb(:)
     *l, allocatable    :: orog(:)
     *l, allocatable    :: sea_ice_temp(:)
     *l, allocatable    :: skin_temp(:)
     *l, allocatable    :: snow_depth(:)
     *l, allocatable    :: snow_free_albedo(:)
     *l, allocatable    :: snow_liq_equiv(:)
     *l, allocatable    :: soilm_liq(:,:)
     *l, allocatable    :: soilm_tot(:,:)
     *l, allocatable    :: soil_temp(:,:)
     *l, allocatable    :: substrate_temp(:)
     *l, allocatable    :: z0(:)
     *eger, allocatable :: sea_ice_flag(:)
     *eger, allocatable :: slope_type(:)
     *eger, allocatable :: soil_type(:)
     *eger, allocatable :: veg_type(:)
     *ype sfc1d

     * public ::  sfc2dl
     *, allocatable    :: alnsf(:,:)
     *l, allocatable    :: alnwf(:,:)
     *l, allocatable    :: alvsf(:,:)
     *l, allocatable    :: alvwf(:,:)
     *l, allocatable    :: canopy_mc(:,:)
     *l, allocatable    :: facsf(:,:)
     *l, allocatable    :: facwf(:,:)
     *l, allocatable    :: sea_ice_fract(:,:)
     *l, allocatable    :: greenfrc(:,:)
     *l, allocatable    :: greenfrc_max(:,:)
     *l, allocatable    :: greenfrc_min(:,:)
     *l, allocatable    :: sea_ice_depth(:,:)
     *l, allocatable    :: lsmask(:,:)
     *l, allocatable    :: mxsnow_alb(:,:)
     *l, allocatable    :: orog(:,:)
     *l, allocatable    :: sea_ice_temp(:,:)
     *l, allocatable    :: skin_temp(:,:)
     *l, allocatable    :: snow_depth(:,:)
     *l, allocatable    :: snow_liq_equiv(:,:)
     *l, allocatable    :: snow_free_albedo(:,:)
     *l, allocatable    :: soilm_liq(:,:,:)
     *l, allocatable    :: soilm_tot(:,:,:)
     *l, allocatable    :: soil_temp(:,:,:)
     *l, allocatable    :: substrate_temp(:,:)
     *l, allocatable    :: z0(:,:)
     *eger, allocatable :: sea_ice_flag(:,:)
     *eger, allocatable :: slope_type(:,:)
     *eger, allocatable :: soil_type(:,:)
     *eger, allocatable :: veg_type(:,:)
     *ype sfc2dinsutine

# 229

     * surface_chgres_driver(imdl_output, jmdl_output,   &
                                  ijmdl_output, nsoil_output, &
                                  lonsperlat_output,          &
                                  kgds_output, output,        &
                                  imdl_input, jmdl_input,     &
                                  nsoil_input, hour, month,   &
                                  day, year, fhour,           &
                                  kgds_input, input, iret)
!$$$ subprogram documentation block
!
! subprogram: surface_chgres_driver  driver routine for this module
!   prgmmr: gayno           org: w/np2     date: 2005-10-19
!
! abstract: call some prep routines, then call main interpolation
!           routine.
!
! program history log:
!  2005-10-19  gayno   - initial version
!  2006-04-25  gayno   - created common interface for gfs and wrf.
!
! usage: call surface_chgres_driver(imdl_output, jmdl_output,   &
!                                   ijmdl_output, nsoil_output, &
!                                   lonsperlat_output,          &
!                                   kgds_output, output,        &
!                                   imdl_input, jmdl_input,     &
!                                   nsoil_input, hour, month,   &
!                                   day, year, fhour,           &
!                                   kgds_input, input, iret)
!   input arguments:
!     day                    cycle day
!     fhour                  forecast hour
!     hour                   cycle hour
!     imdl_input             i-dimension, input grid
!     imdl_output            i-dimension, output grid
!     input                  land data on input grid
!     jmdl_input             j-dimension, input grid
!     jmdl_output            j-dimension, output grid
!     kgds_input             grib grid description section of input grid
!     kgds_output            grib grid description section of output grid
!     lonserplat_output      number of i points in each j row, output grid
!     month                  cycle month
!     nsoil_input            number of soil layers, input grid
!     nsoil_output           number of soil layers, output grid
!     year                   cycle year
!   outputs:
!     iret                   error status, non-zero if there is a
!                            problem in this module.
!     output                 land data on output grid
!
! subprograms called:
!   setup                - read program namelist, calculate soil parameters
!   get_ext_climo_global - call sfccycle program to get climo
!                          fields on output grid
!   call interp          - interpolate and initialize land states on
!                          output grid
! attributes:
!   langauge: fortran 90
!
     *cit nonesfc2d

     *)                       :: inputsfc1d
     *)                       :: outputer

     *, intent(in)               :: hour, month, day, yearer
     *                           :: i, j, ij, ii, jjer
     *, intent(in)               :: imdl_inputer
     *, intent(in)               :: imdl_outputer
     *, intent(in)               :: ijmdl_outputer
     *, intent(inout)            :: ireter
     *, intent(in)               :: jmdl_inputer
     *, intent(in)               :: jmdl_outputer
     *, intent(in)               :: kgds_input(200)
     *er, intent(in)               :: kgds_output(200)
     *er, intent(in)               :: lonsperlat_output((jmdl_output+1)/
     *er, intent(in)               :: nsoil_inputer
     *, intent(in)               :: nsoil_output

     * intent(in)                  :: fhour
 real                              :: r

!-----------------------------------------------------------------------
! perform various setup tasks.
!-----------------------------------------------------------------------

 iret = 0  ! becomes non-zero if there is an error in this module.

     **,"- CALL SETUP ROUTINE"
 call setup (kgds_input, input, imdl_input, jmdl_input, kgds_output, ire
     *ret /= 0) returngds_output

!-----------------------------------------------------------------------
! the output arrays are 1-d.  keep track of their 2-d indices for
! various diagnostics.
!
! Get any output grid fixed fields (such as greenness) that are not
! to be interpolated from the input grid.  for gfs, we call the
! sfccycle program, for nmm we read them in from grib files.
!-----------------------------------------------------------------------

     *(1) == 4 .or. kgds_output(1) == 0) then ! gaussian/latlon grid
     *ocate (iindx_output(ijmdl_output))
     *ocate (jindx_output(ijmdl_output))
   ii = 0
   do j=1,jmdl_outputj
     * = jf
     * (j .gt. (jmdl_output+1)/2) jj = jmdl_output - j + 1
     * = imdl_output / lonsperlat_output(jj)
     *o i=1,lonsperlat_output(jj)
       ii = ii + 1
       iindx_output(ii) = nint((i-1.0)*r)+1
       jindx_output(ii) = jnddodont
# 343
     **,'- CALL CYCLE TO GET SURFACE STATIC/CLIMO FIELDS ON OUTPUT GRIl 
     *get_ext_climo_global(ijmdl_output, output%lsmask, output%orog, &
                             output, hour, month, day, year, fhour)
 else ! e-grid
     *ocate(iindx_output(ijmdl_output))
     *ocate(jindx_output(ijmdl_output))
   ij = 0
   do j = 1,jmdl_outputo
     * i = 1, imdl_output
       ij=ij+1
       iindx_output(ij)=i
       jindx_output(ij)=jnddodont
# 357
     **,'- GET SURFACE STATIC/CLIMO FIELDS ON OUTPUT GRIDS.'l get_ext_cl
     *imo_nmm(imdl_output,jmdl_output,hour, & 
                          month, day, year, iret)
   if (iret /= 0) return   ! error in routine


 print*,"- CALL INTERP ROUTINE"

 call interp (imdl_input, jmdl_input, kgds_input, ijmdl_output,  &
              nsoil_input, nsoil_output,  &
              input, output, imdl_output, jmdl_output, & 
              kgds_output, iret)

 deallocate(iindx_output,jindx_output)

 return

 end subroutine surface_chgres_driver

 subroutine interp (imdl_input, jmdl_input,    &
                    kgds_input, ijmdl_output,  &
                    nsoil_input, nsoil_output, &
                    input, output, imdl_output, &
                    jmdl_output, kgds_output, iret)
     *ubprogram documentation block
!
! subprogram: interp        interpolate land states
!   prgmmr: gayno           org: w/np2     date: 2005-10-19
!
! abstract: interpolate land states from one grid to another.
!           discreet fields (such as soil/veg type) and fields
!           that are a function of these discreet fields are
!           always interpolated via nearest neighbor.  other
!           continous fields are interpolated by budget,
!           nearest neighbor or bilinear depending on the
!           resolutions of the input and output grids.  one
!           exception is snow, which is never interpolated
!           using bilinear.
!
! program history log:
!  2005-10-19  gayno   - initial version
!  2006-04-25  gayno   - modified to use ipolates library.
!                        modified to handle both gfs and nmm.
!
! usage: call interp (imdl_input, jmdl_input,    &
!                     kgds_input, ijmdl_output,  &
!                     nsoil_input, nsoil_output, &
!                     input, output, imdl_output, &
!                     jmdl_output, kgds_output, iret)
!
!   input arguments:
!     imdl_input             i-dimension, input grid
!     imdl_output            i-dimension, output grid
!     ijmdl_output           number of grid points, output grid
!     input                  land data on input grid
!     jmdl_input             j-dimension, input grid
!     jmdl_output            j-dimension, output grid
!     kgds_input             grib grid description section of input grid
!     kgds_output            grib grid description section of output grid
!     nsoil_input            number of soil layers, input grid
!     nsoil_output           number of soil layers, output grid
!   outputs:
!     iret                   error status, non-zero if a problem
!     output                 land data on output grid
!
! subprograms called:
!   ll2xy_gaussian_1d      - convert from lat/lon to x/y on a gaussian grid
!   ll2xy_egrid            - convert from lat/lon to x/y on an e-grid grid
!   find_nn_new            - finds nearest neighbor input point for each
!                            output point that is the same type (land,
!                            landice, non-land)
!   adjust_soilt_for_orog  - adjust soil temperature for differences in
!                            orography between input and output grids
!   calc_albedo            - calculate albedo based on snow cover
!   calc_liq_soilm         - calculate liquid soil moisture
!   rescale_soilm          - rescale soil moisture for changes in
!                            soil type
!
! attributes:
!   langauge: fortran 90
!
 use ll2xy_utils, only       : ll2xy_gaussian_1d, ll2xy_egrid

!use physcons, only          : frz_ice => con_tice, frz_h20 => con_ttp
!use consts, only            : frz_ice,    &
!                              frz_h20

 use interp_utils, only      : find_nn_new

 use soil_utils, only        : rescale_soilm,         &
                               adjust_soilt_for_orog, &
                               calc_liq_soilm, &
                               calc_albedo

 implicit none

 character*6                       :: grid_type
     * parameter                   :: frz_ice=271.2, frz_h20=273.16
     * er                           :: count_land_output, count_nonland_
     *oer                           :: count_sea_ice_outputer
     *, parameter                :: flag_value = -999
     * er                           :: i,j, ij, ii, jj, ner
     *, allocatable              :: ibi(:), ibo(:)
     *er, intent(in)               :: ijmdl_outputer
     *, allocatable              :: ijsav_land_output(:), ijsav_nonlan
                                      ijsav_sea_ice_output(:)
     *er, intent(in)               :: imdl_input, jmdl_inputer
     *, intent(in)               :: imdl_output, jmdl_outputer
     *                           :: ipopt(20), int_opt, no 
     *er                           :: ipopt_snow(20), int_opt_snower
     *, allocatable              :: ipts(:,:),jpts(:,:)
     *er, intent(inout)            :: ireter
     *, intent(in)               :: kgds_input(200), kgds_output(200)
     *er                           :: kgds_output_tmp(200), kgdso1 
     *er, allocatable              :: mask_input(:,:)
     *er, allocatable              :: mask_output(:)
     *er                           :: nsoiler
     *, intent(in)               :: nsoil_inputer
     *, intent(in)               :: nsoil_outputer
     *, allocatable              :: nn_iindx_wrt_input_grid(:)
     *er, allocatable              :: nn_jindx_wrt_input_grid(:)
     *er, allocatable              :: soil_type_sav(:)

 real                              :: center_lat_input, center_lon_input
 real                              :: dx_input, dy_input
     * allocatable                 :: input_dat(:,:,:)
     * allocatable                 :: lats_land_output(:), lons_land_out
     * allocatable                 :: lats_nonland_output(:), lons_nonla
     * allocatable                 :: lats_sea_ice_output(:), lons_sea_i
     * allocatable                 :: lsmask_output_temp(:)
     * allocatable                 :: orog_sav(:)
     * allocatable                 :: output_data_nonland(:), output_dat
     * allocatable                 :: output_data_sea_ice(:), output_dat
     * allocatable                 :: output_data_sea_ice2(:,:)
     * allocatable                 :: snow_m(:)
     * allocatable                 :: soilm_sav(:,:)
     * allocatable                 :: xindx_wrt_input_grid(:)
     * allocatable                 :: yindx_wrt_input_grid(:)

 logical*1, allocatable            :: bitmap_land_input(:,:) 
     *al*1, allocatable            :: bitmap_land_output(:)
     *al*1, allocatable            :: bitmap_sea_ice_input(:,:) 
     *al*1, allocatable            :: bitmap_sea_ice_output(:) 
     *al*1, allocatable            :: bitmap_nonland_input(:,:)
     *al*1, allocatable            :: bitmap_nonland_output(:)
     *al*1, allocatable            :: bitmap_land_input2(:,:,:)
     *al*1, allocatable            :: bitmap_land_output2(:,:)
     *al*1, allocatable            :: bitmap_sea_ice_input2(:,:,:)
     *al*1, allocatable            :: bitmap_sea_ice_output2(:,:)
     *al                           :: rescale_soil_moistal
     *                           :: sea_ice_defaultsal
     *                           :: veg_from_input

 type(sfc2d)                       :: inputsfc1d
     *)                       :: output

!-----------------------------------------------------------------------
! the following variables are setup for use by the ipolates routines.
!
! to properly handle coastlines, the ipolates routines are passed
! the land and non-land points separately.
!-----------------------------------------------------------------------

 iret = 0

 count_land_output=0    
     *_nonland_output=0 
     * = 1, ijmdl_output
   if (output%lsmask(ij) > 0.0) thenount_land_output
     *=count_land_output+1
# 528
     * eount_nonland_output=count_nonland_output+1
     *if


!-----------------------------------------------------------------------
! note: there are separate options for handling snow (avoid
!       bilinear method).
!-----------------------------------------------------------------------

 ipopt=0
     * dl_res_input < (0.35*mdl_res_output)) thennt
     **,"- INTERPOLATE CONTINUOUS DATA FIELDS USING BUDGET METHOD."
     *pt(1)=-1  ! break model grid cell into 25 points.
     *pt(2)=-1  ! 25 points are weighted equally.
     *pt(20) = nint(1.0 / mdl_res_input) + 1   ! search box width of 1 deg.
     *so1 = kgds_output(1) - 255 ! subset of grid
     *_opt = 3
     *_opt_snow = int_optpt_snow
     * = ipoptf
     * (mdl_res_input <= (0.75*mdl_res_output)) thennt
     **,"- INTERPOLATE CONTINUOUS DATA FIELDS USING BILINEAR METHOD."
     *s_output_tmp = kgds_outputso1
     * = -1  ! for subsection of model grid.
     *_opt = 0
     *pt(1)=1
     *_opt_snow = 2    ! use neighbor method instead for snow.
     *pt_snow = 0
     *pt_snow(1) = nint(1.0 / mdl_res_input) + 1   ! search box width of 1 deg.
 else 
     *nt*,"- INTERPOLATE CONTINUOUS DATA FIELDS USING NEIGHBOR METHOD."
     *pt(1) = nint(1.0 / mdl_res_input) + 1   ! search box width of 1 deg.
     *so1 = -1  ! for subsection of model grid.
     *_opt = 2
     *_opt_snow = int_optpt_snow
     * = ipoptf


!-----------------------------------------------------------------------
! set up bitmap to tell ipolates where the land and non-land points
! are on the input and output grids.
!-----------------------------------------------------------------------

 allocate(bitmap_land_output(count_land_output))
     *p_land_output = .false.
     *ate(output_data_land(count_land_output))
     *t_data_land=0.0

 allocate(ijsav_land_output(count_land_output))
     *ate(lats_land_output(count_land_output))
     *ate(lons_land_output(count_land_output))

 count_land_output=0
     * = 1, ijmdl_output
   if (output%lsmask(ij) > 0.0) thenount_land_output
     *=count_land_output+1
     *jsav_land_output(count_land_output)=ijats_land_output
     *(count_land_output)=output%lats(ij)
     *ons_land_output(count_land_output)=output%lons(ij)
     *if


 allocate(bitmap_land_input(imdl_input,jmdl_input))
     *p_land_input=.false.
     *(input%lsmask > 0.0) bitmap_land_input=.true.   

! non-land

 allocate(output_data_nonland(count_nonland_output))
     *t_data_nonland=0.0
     *ate(bitmap_nonland_output(count_nonland_output))
     *p_nonland_output = .false.

 allocate(ijsav_nonland_output(count_nonland_output))
     *ate(lats_nonland_output(count_nonland_output))
     *ate(lons_nonland_output(count_nonland_output))

 count_nonland_output=0
     * = 1, ijmdl_output
   if (output%lsmask(ij) == 0.0) thenount_nonland_output
     *=count_nonland_output+1
     *jsav_nonland_output(count_nonland_output)=ijats_nonland_output
     *(count_nonland_output)=output%lats(ij)
     *ons_nonland_output(count_nonland_output)=output%lons(ij)
     *if


 allocate(bitmap_nonland_input(imdl_input,jmdl_input))
     *p_nonland_input=.false.
     *(input%lsmask == 0.0) bitmap_nonland_input=.true.   

 allocate(bitmap_sea_ice_input(imdl_input,jmdl_input))  ! sea ice
     *p_sea_ice_input=.false.
     *(input%sea_ice_flag == 1) bitmap_sea_ice_input=.true.

!-----------------------------------------------------------------------
! given the grib gds info of the input grid, and the lats and lons of
! output grid, calculate the x/y location of the output grid points
! with respect to the input grid.
!-----------------------------------------------------------------------

 allocate (xindx_wrt_input_grid(ijmdl_output))
     *ate (yindx_wrt_input_grid(ijmdl_output))

 print*,"- DETERMINE CORRESPONDING X/Y ON INPUT GRID" 
     *ds_input(1) == 4) thenl
     * ll2xy_gaussian_1d(kgds_input, ijmdl_output,    &
                          output%lats, output%lons,    &
                          xindx_wrt_input_grid,        &
                          yindx_wrt_input_grid)
     *d_type="global"
     *f(kgds_input(1) == 203) thenter_lat_input
     * = float(kgds_input(7)) * 0.001
     *ter_lon_input = float(kgds_input(8)) * 0.001
     *input = -(float(kgds_input(9)) * 0.001)
     *input = float(kgds_input(10)) * 0.001
     *use of the e-grid's stagger, the routine ll2xy_egrid routineocate
! outputs nearest i/j whereas the gaussian routine (based on ipolates)
! outputs a float value.  the rest of this module expects a float,
! so convert temp arrays i/jpts to a float value after routine call.
     * (ipts(imdl_input,jmdl_input))
     *ocate (jpts(imdl_input,jmdl_input))
     *l ll2xy_egrid(output%lats, output%lons, imdl_input, jmdl_input, &
                    center_lat_input, center_lon_input, dx_input, dy_inp
                    imdl_output, jmdl_output, ipts, jpts)
     *dx_wrt_input_grid = reshape(float(ipts),(/ijmdl_output/))
     *dx_wrt_input_grid = reshape(float(jpts),(/ijmdl_output/))
     *llocate(ipts,jpts)
     *d_type="egrid"
     *from_input

!-----------------------------------------------------------------------
! landice points are based on the vegetation type.  so, need to
! handle this field first.
!-----------------------------------------------------------------------

     *=.true.   ! get veg_type from input grid
     *llocated(veg_type_output_ext)) thennt
     **,'- REPLACE VEG TYPE WITH EXTERNAL DATA.'
     *put%veg_type = 0
     *re (output%lsmask > 0.0) output%veg_type = veg_type_output_ext_fro
     *m_input=.false.  ! veg_type from externally prepared process.
                           ! don't get from input grid
     *llocate (veg_type_output_ext)
     *fate

!-----------------------------------------------------------------------
! for each point on the output grid, find the nearest neighbor
! point on the input grid.  if output point is land (landice, water),
! the nearest neighbor will always be land (landice, water).
!-----------------------------------------------------------------------

     * (nn_iindx_wrt_input_grid(ijmdl_output))
     *ate (nn_jindx_wrt_input_grid(ijmdl_output))

     *ate (mask_output(ijmdl_output))
     *ate (mask_input(imdl_input,jmdl_input))

     *andice_opt == 2 .and. .not.(veg_from_input)) then
   do j = 1, jmdl_input
   do i = 1, imdl_inputf
     * (input%lsmask(i,j) > 0.0) then
       if (input%veg_type(i,j) == veg_type_ice_input) then
         mask_input(i,j) = 2
       else
         mask_input(i,j) = 1
       end iflse

       mask_input(i,j) = 0
     *ndifdodo
# 698

   do ij = 1, ijmdl_outputf
     * (output%lsmask(ij) > 0.0) then
       if (output%veg_type(ij) == veg_type_ice) then
         mask_output(ij) = 2
       else
         mask_output(ij) = 1
       end iflse

       mask_output(ij) = 0
     *ndifdo

 else 
   do j = 1, jmdl_input
   do i = 1, imdl_inputf
     * (input%lsmask(i,j) > 0.0) then
       mask_input(i,j) = 1
     *lse
       mask_input(i,j) = 0
     *ndifdodo
# 719

   do ij = 1, ijmdl_outputf
     * (output%lsmask(ij) > 0.0) then
       mask_output(ij) = 1
     *lse
       mask_output(ij) = 0
     *ndifdof
# 728

     **,"- CALC NEAREST NEIGHBOR POINTS."
 call find_nn_new(imdl_input, jmdl_input, mask_input,          &
                  ijmdl_output, mask_output,                   &
                  flag_value, grid_type, mdl_res_input,        &
                  iindx_output, jindx_output,                  &
                  xindx_wrt_input_grid, yindx_wrt_input_grid,  &
                  nn_iindx_wrt_input_grid,                     &
                  nn_jindx_wrt_input_grid)

     *ocate (mask_output)
     *ocate (mask_input)
     *ocate (xindx_wrt_input_grid)
     *ocate (yindx_wrt_input_grid)

!-----------------------------------------------------------------------
! if user selects, interpolate veg type from input grid.  always
! use nearest neighbor approach on this discreet field.
!-----------------------------------------------------------------------

     *eg_from_input) thennt
     **,"- INTERPOLATE VEG TYPE FROM INPUT GRID"
   do ij = 1, ijmdl_outputf
     * (output%lsmask(ij) == 0.0) then ! non-land points
         output%veg_type(ij) = 0   
     *lse
       if ( (nn_iindx_wrt_input_grid(ij) /= flag_value)   .and.  &
            (nn_jindx_wrt_input_grid(ij) /= flag_value) ) then
         ii = nn_iindx_wrt_input_grid(ij)
         jj = nn_jindx_wrt_input_grid(ij)
         output%veg_type(ij) = input%veg_type(ii,jj)
       else  ! no nearest neighbor that is land.  use a default.
         output%veg_type(ij) = 6
       endifnd
     * ifdofaxval
# 765

!-----------------------------------------------------------------------
! sea ice flag...and when using sea ice model - fraction and depth.
! logic is as follows:
!
! no ice model input -> no ice model output : interpolate ice flag
! as 0 or 100% coverage.
!
! no ice model input -> ice model output : interpolate ice flag
! as 0 or 100% coverage.  set fract and depth to default values.
!
! ice model input -> ice model output : interpolate ice fraction.
! if > 50%, set ice flag to yes.  interpolate ice depth.  ensure
! consistency with ice flag.
!
! ice model input -> no ice model output : interpolate ice fraction.
! if > 50%, set ice flag to yes.
!-----------------------------------------------------------------------

     *(input%sea_ice_fract) < -999.) then  !input grid is pre-seaice model
     *nt*,"- INTERPOLATE SEA ICE FLAG FROM INPUT GRID."
     *map_nonland_output=.false.
     *put_data_nonland=0.0
     *s_output_tmp=kgds_outputs_output_tmp
     *(1) = kgdso1count_nonland_outputocate
# 791
     *(ibo(1))
     *ocate(input_dat(imdl_input,jmdl_input,1))
     *ut_dat(:,:,1)=float(input%sea_ice_flag)
     *l ipolates(int_opt, ipopt, kgds_input, kgds_output_tmp,   &
                (imdl_input*jmdl_input), count_nonland_output,    &
                 1, 1, bitmap_nonland_input, input_dat,  &
                 no, lats_nonland_output, lons_nonland_output, ibo, &
                 bitmap_nonland_output, output_data_nonland, iret)
   if (iret /= 0) thenrint
     **,'- ERROR IN IPOLATES ',ireteturnifllocate
# 803
     *(ibo)
     *put%sea_ice_flag = 0 ! land
   do ij = 1, count_nonland_outputf
     * (bitmap_nonland_output(ij)) then
       output%sea_ice_flag(ijsav_nonland_output(ij))=nint(output_data_no
     *lse
       if(abs(lats_nonland_output(ij)) > 55.0) then  ! use latitude based default
         output%sea_ice_flag(ijsav_nonland_output(ij))=1  ! search failed, use default
       else
         output%sea_ice_flag(ijsav_nonland_output(ij))=0  ! search failed, use default
       end ifndifdollocate
# 816
     *(input_dat)
     *_ice_defaults=.true.
 else  ! input grid used sea ice model,
     *nt*,"- INTERPOLATE SEA ICE FRACTION FROM INPUT GRID."
     *map_nonland_output=.false.
     *put_data_nonland=0.0
     *s_output_tmp=kgds_outputs_output_tmp
     *(1) = kgdso1ocate
     *(ibo(1))
     *count_nonland_outputl
     * ipolates(int_opt, ipopt, kgds_input, kgds_output_tmp,   &
                (imdl_input*jmdl_input), count_nonland_output,       &
                 1, 1, bitmap_nonland_input, input%sea_ice_fract,  &
                 no, lats_nonland_output, lons_nonland_output, ibo,  &
                 bitmap_nonland_output, output_data_nonland, iret)
   if (iret /= 0) thenrint
     **,'- ERROR IN IPOLATES ',ireteturnifllocate
# 835
     *(ibo)
     *put%sea_ice_flag = 0  ! land
   do ij = 1, count_nonland_outputf

     * (bitmap_nonland_output(ij)) then
       if (output_data_nonland(ij) >= .5) then  ! make this a variable?
         output%sea_ice_flag(ijsav_nonland_output(ij))=1
       else
         output%sea_ice_flag(ijsav_nonland_output(ij))=0
       endif 
     *lse  ! use a default value
       if(abs(lats_nonland_output(ij)) > 55.0) then
         output%sea_ice_flag(ijsav_nonland_output(ij))=1
       else
         output%sea_ice_flag(ijsav_nonland_output(ij))=0
       end ifndifdo_ice_defaults
# 853
     *=.false.
   if (allocated(output%sea_ice_fract)) then  ! output grid to use sea ice model
     *utput%sea_ice_fract = 0.0 ! land
     *o ij = 1, count_nonland_output
       if (bitmap_nonland_output(ij)) then
         if (output_data_nonland(ij) >= .5) then  ! make this a variable?
           output%sea_ice_fract(ijsav_nonland_output(ij))=output_data_no
         else
           output%sea_ice_fract(ijsav_nonland_output(ij))=0.0
         endif 
       else
         if (output%sea_ice_flag(ijsav_nonland_output(ij)) == 1) then
           output%sea_ice_fract(ijsav_nonland_output(ij))=1.0
         end if
       endifnddo
# 869
     * iff_sea_ice_output
# 871

!-----------------------------------------------------------------------
! now that we know the sea ice on the output grid, set the
! mask for ice for future ipolates calls.
!-----------------------------------------------------------------------

     *=0
     * = 1, ijmdl_output
   if (output%sea_ice_flag(ij) == 1) thenount_sea_ice_output
     *=count_sea_ice_output+1
     *if
# 883

     *ate(output_data_sea_ice(count_sea_ice_output))
     *t_data_sea_ice=0.0
     *ate(bitmap_sea_ice_output(count_sea_ice_output))
     *p_sea_ice_output = .false.

     *ate(ijsav_sea_ice_output(count_sea_ice_output))
     *ate(lats_sea_ice_output(count_sea_ice_output))
     *ate(lons_sea_ice_output(count_sea_ice_output))

     *_sea_ice_output=0
     * = 1, ijmdl_output
   if (output%sea_ice_flag(ij) == 1) thenount_sea_ice_output
     *=count_sea_ice_output+1
     *jsav_sea_ice_output(count_sea_ice_output)=ijats_sea_ice_output
     *(count_sea_ice_output)=output%lats(ij)
     *ons_sea_ice_output(count_sea_ice_output)=output%lons(ij)
     *if
# 902

!------------------------------------------------------------------------
! output grid to use sea ice model.
!
! if sea_ice_defaults logical is true, then the input grid did not
! run with the sea ice model, so need to set fract and depth to
! default values.
!
! if logical is false, then input grid did run with ice model.
! fraction was calculated above, so now interpolate depth.
!------------------------------------------------------------------------
     *llocated(output%sea_ice_fract) .and. allocated(output%sea_ice_dept
   if (sea_ice_defaults) thenrint
     **,"- INITIALIZE SEA ICE FRACTION AND DEPTH WITH DEFAULT VALUESo ij
     * = 1, ijmdl_output
       if (output%sea_ice_flag(ij) == 1) then
         output%sea_ice_fract(ij) = 1.0  
         if (output%lats(ij) > 0.0) then
           output%sea_ice_depth(ij) = 3.0  ! in meters
         else
           output%sea_ice_depth(ij) = 1.5  ! in meters
         end if
       else
         output%sea_ice_fract(ij) = 0.0
         output%sea_ice_depth(ij) = 0.0
       endifnddoerint
# 930
     **,"- INTERPOLATE SEA ICE DEPTH FROM INPUT GRID."itmap_sea_ice_outp
     *ut=.false.utput_data_sea_ice=0.0gds_output_tmp=kgds_outputgds_outp
     *ut_tmp(1) = kgdso1llocate(ibo(1))o=count_sea_ice_outputall ipolate
     *s(int_opt, ipopt, kgds_input, kgds_output_tmp,   &
# 931

# 938

                  (imdl_input*jmdl_input), count_sea_ice_output,        
                   1, 1, bitmap_sea_ice_input, input%sea_ice_depth,  &
                   no, lats_sea_ice_output, lons_sea_ice_output,  &
                   ibo, bitmap_sea_ice_output,     &
                   output_data_sea_ice, iret)
     *f (iret /= 0) then
       print*,'- ERROR IN IPOLATES ',iret
       returnndifeallocate
# 948
     *(ibo)
     *utput%sea_ice_depth = 0.0  ! open water/land
     *o ij = 1, count_sea_ice_output
       if (bitmap_sea_ice_output(ij)) then 
         output%sea_ice_depth(ijsav_sea_ice_output(ij)) = &
                              output_data_sea_ice(ij)
       else  ! use a default value
         if (output%sea_ice_flag(ijsav_sea_ice_output(ij)) == 1) then
           output%sea_ice_depth(ijsav_sea_ice_output(ij))=1.5
         end if
       endifnddoiff
# 961


!-----------------------------------------------------------------------
! always use externally generated substrate temps as these are
! tied to the terrain.  when running with land ice options, ensure
! it is below freezing.  note: for wrf grids, this field is read in
! the driver and passed in.  so, only need to do land ice option check.
!-----------------------------------------------------------------------

 if (kgds_output(1) == 4 .or. kgds_output(1) == 0) then ! gaussian/latlon grid
     *put%substrate_temp = substrate_temp_output_extllocate
     * (substrate_temp_output_ext)
     *f

 if (landice_opt == 1 .or. landice_opt == 2) thennt
     **,"- ENSURE SUBSTRATE TEMP BELOW FREEZING AT LAND ICE."
   do ij = 1, ijmdl_outputf
     * (output%lsmask(ij) > 0.0 .and.  &
         output%veg_type(ij) == veg_type_ice) then
       output%substrate_temp(ij) = min(output%substrate_temp(ij), frz_h2
     *ndifdo
# 983


 where (output%lsmask == 0.0) output%substrate_temp = 280.0 ! water flat
     * (output%sea_ice_flag == 1) output%substrate_temp = frz_ice ! sea ice flag

!-----------------------------------------------------------------------
! treat cmc as discreet field because it is a function of veg type.
!-----------------------------------------------------------------------
     **,"- INTERPOLATE CANOPY MOISTURE CONTENT" = 1, ijmdl_output

   if (output%lsmask(ij) == 0.0) then  ! non-land points
     *utput%canopy_mc(ij) = 0
# 996
     * ef ( (nn_iindx_wrt_input_grid(ij) /= flag_value)   .and.  &
          (nn_jindx_wrt_input_grid(ij) /= flag_value) ) then
       ii = nn_iindx_wrt_input_grid(ij)
       jj = nn_jindx_wrt_input_grid(ij)
       output%canopy_mc(ij) = input%canopy_mc(ii,jj)
     *lse  ! no nearest neighbor that is land.  use default.
       output%canopy_mc(ij) = 0
     *ndifif
# 1005


!-----------------------------------------------------------------------
! treat soil moist as discreet field because it is a function of
! soil type.  note: may want to consider other interpolation
! methods in the future as long as they use a soil type "mask."
!-----------------------------------------------------------------------
 print*,"- INTERPOLATE TOTAL SOIL MOISTURE" = 1, ijmdl_output

   if (output%lsmask(ij) == 0.0) then  ! non-land points
     *utput%soilm_tot(ij,:) = 1.0
# 1017
     * ef ( (nn_iindx_wrt_input_grid(ij) /= flag_value)   .and.  &
          (nn_jindx_wrt_input_grid(ij) /= flag_value) ) then
       ii = nn_iindx_wrt_input_grid(ij)
       jj = nn_jindx_wrt_input_grid(ij)
       if (nsoil_output == nsoil_input) then
         do n = 1, nsoil_output
           output%soilm_tot(ij,n) = input%soilm_tot(ii,jj,n)
         enddo
       elseif (nsoil_output > nsoil_input) then
         output%soilm_tot(ij,1) = input%soilm_tot(ii,jj,1)
         do n = 2, nsoil_output
           nsoil = min(n,nsoil_input)
           output%soilm_tot(ij,n) = input%soilm_tot(ii,jj,nsoil)
         enddo
       else  ! logic hardwired for 4->2 layers with thicknesses of
             ! 0-.1,.1-.4,.4-1.0,1.0-2.0 and 0-.1,1.0-2.0 meters.
         output%soilm_tot(ij,1) = input%soilm_tot(ii,jj,1)
         output%soilm_tot(ij,2) =(0.3*input%soilm_tot(ii,jj,2) + &
                                  0.6*input%soilm_tot(ii,jj,3) + &
                                  1.0*input%soilm_tot(ii,jj,4))/1.9
       endiflse
     *  ! no nearest neighbor that is land.  use default.
           ! will be overwritten later if landice.
       output%soilm_tot(ij,:)  = 0.2 
     *ndifif
# 1043


!-----------------------------------------------------------------------
! treat roughness as a discreet field as it is tied to vegetation
! type over land. (over water, it is a state variable, so might want
! to consider an approach other than nearest neighbor someday.)
!------------------------------------------------------------------------
 print*,"- INTERPOLATE Z0 FROM INPUT GRID." = 1, ijmdl_output

   if ( (nn_iindx_wrt_input_grid(ij) /= flag_value)   .and.  &
        (nn_jindx_wrt_input_grid(ij) /= flag_value) ) theni
     * = nn_iindx_wrt_input_grid(ij)
     *j = nn_jindx_wrt_input_grid(ij)
     *utput%z0(ij) = input%z0(ii,jj)
     *e  ! use a default value
     *f (output%lsmask(ij) > 0.0) then  ! points with land
       output%z0(ij)           = 30.0      ! cm
     *lse   
       if (output%sea_ice_flag(ij) == 1) then
         output%z0(ij)           = 1.0       ! cm
       else       ! open water
         output%z0(ij)           = 0.01      ! cm
       end ifndifif
# 1069
     *------------------------------------------------------------------
     *ace interpolated z0 with externally generated z0he
     * output grid (if this data was read in).  the externallyrated
     * data is only valid over land.  
     *------------------------------------------------------------------
     *llocated(z0_output_ext)) thennt
     **,'- REPLACE Z0 WITH EXTERNAL DATA AT LAND POINTS.'
     *re(output%lsmask > 0.0) output%z0 = z0_output_extllocate
     * (z0_output_ext)
     *f

!-----------------------------------------------------------------------
! skin temperature
!-----------------------------------------------------------------------
 print*,"- INTERPOLATE SKIN TEMPERATURE FROM INPUT GRID."p_land_output=.
     *false.t_data_land=0.0output_tmp=kgds_outputoutput_tmp(1) = kgdso1a
     *te(ibo(1))unt_land_output
# 1089

 call ipolates(int_opt, ipopt, kgds_input, kgds_output_tmp,   &
              (imdl_input*jmdl_input), count_land_output,               
               1, 1, bitmap_land_input, input%skin_temp,  &
               no, lats_land_output, lons_land_output, ibo,  &
               bitmap_land_output, output_data_land, iret)
     *ret /= 0) thennt
     **,'- ERROR IN IPOLATES ',ireturn
# 1099
     *ocate(ibo)
     *t%skin_temp= 0.0 
     * = 1, count_land_output
   if (bitmap_land_output(ij)) thenutput
     *%skin_temp(ijsav_land_output(ij))=output_data_land(ij)
     *e  ! default value
     *utput%skin_temp(ijsav_land_output(ij)) = &
                       output%substrate_temp(ijsav_land_output(ij))
     *if

! now do over non-land.  note that skint is a mix of ice and open water temp.
 bitmap_nonland_output=.false.
     *t_data_nonland=0.0
     *output_tmp=kgds_outputoutput_tmp
     *(1) = kgdso1ate
     *(ibo(1))
     *unt_nonland_output
 call ipolates(int_opt, ipopt, kgds_input, kgds_output_tmp,   &
              (imdl_input*jmdl_input), count_nonland_output,            
               1, 1, bitmap_nonland_input, input%skin_temp,  &
               no, lats_nonland_output, lons_nonland_output, ibo,  &
               bitmap_nonland_output, output_data_nonland, iret)
     *ret /= 0) thennt
     **,'- ERROR IN IPOLATES ',ireturn
# 1125
     *ocate(ibo)
     * = 1, count_nonland_output
   if (bitmap_nonland_output(ij)) thenutput
     *%skin_temp(ijsav_nonland_output(ij))=output_data_nonland(ij)
     *eutput
     *%skin_temp(ijsav_nonland_output(ij)) = 280.0 ! default value
     *if


! at sea ice points, don't let skin temp go above the freezing point.
! at open water points, don't let sst go below freezing point.

 do ij = 1, ijmdl_output
   if (output%lsmask(ij) == 0.0) then
   if (output%sea_ice_flag(ij) == 1) thenutput
     *%skin_temp(ij) = min(output%skin_temp(ij),frz_ice)
     *eutput
     *%skin_temp(ij) = max(output%skin_temp(ij),(frz_ice+.01))
     *ifif
# 1145


!-----------------------------------------------------------------------
! set sea ice temperature.  relationship between skin temp and
! sea ice temp is:
!   skint = icefract*tice + (1-icefract)*271.21
! where 271.21K is the temp of any open water in the grid cell.
!-----------------------------------------------------------------------
 if (allocated(output%sea_ice_temp)) then
   do ij = 1, ijmdl_outpututput
     *%sea_ice_temp(ij) = output%skin_temp(ij)
     *f(output%sea_ice_flag(ij) == 1) then
       output%sea_ice_temp(ij) = (output%skin_temp(ij)                  
         -(frz_ice)*(1.-output%sea_ice_fract(ij)))/output%sea_ice_fract(
     *nd ifdof
# 1161


!-----------------------------------------------------------------------
! soil temperature over land.  treat as discreet field.
!-----------------------------------------------------------------------
 allocate (orog_sav(ijmdl_output))
     *sav=0.01
     *, nsoil_outputput
     *%soil_temp(:,n)=output%skin_temp  ! flag value open water
# 1171
     * = 1, ijmdl_output
   if (output%lsmask(ij) > 0.0) thenf
     * ( (nn_iindx_wrt_input_grid(ij) /= flag_value)   .and.  &
          (nn_jindx_wrt_input_grid(ij) /= flag_value) ) then
       ii = nn_iindx_wrt_input_grid(ij)
       jj = nn_jindx_wrt_input_grid(ij)
       if (nsoil_output == nsoil_input) then
         do n = 1, nsoil_output
           output%soil_temp(ij,n) = input%soil_temp(ii,jj,n)
         enddo
       elseif (nsoil_output > nsoil_input) then
         output%soil_temp(ij,1) = input%soil_temp(ii,jj,1)
         do n = 2, nsoil_output
           nsoil = min(n,nsoil_input)
           output%soil_temp(ij,n) = input%soil_temp(ii,jj,nsoil)
         enddo
       else  ! logic hardwired for 4->2 layers with thicknesses of
             ! 0-.1,.1-.4,.4-1.0,1.0-2.0 and 0-.1,1.0-2.0 meters.
         output%soil_temp(ij,1) = input%soil_temp(ii,jj,1)
         output%soil_temp(ij,2) =(0.3*input%soil_temp(ii,jj,2) +  &
                                  0.6*input%soil_temp(ii,jj,3) +  &
                                  1.0*input%soil_temp(ii,jj,4))/1.9
       endif
       orog_sav(ij) = input%orog(ii,jj)
     *lse
       print*,'- *WARNING* SETTING TO DEFAULT VALUES AT POINT ', &
                  iindx_output(ij), jindx_output(ij)
       orog_sav(ij)            = output%orog(ij) ! turn off soil t adjustment
       output%soil_temp(ij,:)  = output%substrate_temp(ij)
     *ndifif
# 1203
     *------------------------------------------------------------------
     *l" temperature over sea ice.  treat as continuous field.
! ipolates expects the number of ice levels to be the same.
! hence, the temporary variables for the input and output
! grids are both allocated to nsoil_input.  the vertical
! interpolation to the output grid occurs in the logic
! after the ipolates call.
!-----------------------------------------------------------------------
     **,"- INTERPOLATE SEA ICE COLUMN TEMPS FROM INPUT GRID."
     *ate(bitmap_sea_ice_output2(count_sea_ice_output,nsoil_input))
     *p_sea_ice_output2=.false.
     *ate(bitmap_sea_ice_input2(imdl_input,jmdl_input,nsoil_input))
     *1, nsoil_inputmap_sea_ice_input2
     *(:,:,n)=bitmap_sea_ice_input
# 1218
     *ate(output_data_sea_ice2(count_sea_ice_output,nsoil_input))
     *t_data_sea_ice2=0.0
     *ate(input_dat(imdl_input,jmdl_input,nsoil_input))
     *1, nsoil_inputut_dat
     *(:,:,n)=input%soil_temp(:,:,n)
     * 
     *output_tmp=kgds_outputoutput_tmp
     *(1) = kgdso1unt_sea_ice_outputate
# 1227
     *(ibo(nsoil_input))
     *ate(ibi(nsoil_input))

 call ipolates(int_opt, ipopt, kgds_input, kgds_output_tmp,   &
               (imdl_input*jmdl_input), count_sea_ice_output,           
                nsoil_input, ibi, bitmap_sea_ice_input2, input_dat,  &
                no, lats_sea_ice_output, lons_sea_ice_output, ibo,  &
                bitmap_sea_ice_output2, output_data_sea_ice2, iret)
     *ret /= 0) thennt
     **,'- ERROR IN IPOLATES ',ireturn
# 1239
     *ocate (ibi, ibo)
     * = 1, count_sea_ice_output
   if (bitmap_sea_ice_output2(ij,1)) thenf
     * (nsoil_output == nsoil_input) then
       do n = 1, nsoil_output
         output%soil_temp(ijsav_sea_ice_output(ij),n) = &
                output_data_sea_ice2(ij,n)
       enddolseif
     * (nsoil_output > nsoil_input) then
       output%soil_temp(ijsav_sea_ice_output(ij),1) =  &
                output_data_sea_ice2(ij,1)
       do n = 2, nsoil_output
         nsoil = min(n,nsoil_input)
         output%soil_temp(ijsav_sea_ice_output(ij),n) =  &
                output_data_sea_ice2(ij,nsoil)
       enddolse
     *  ! logic hardwired for 4->2 layers with thicknesses of
           ! 0-.1,.1-.4,.4-1.0,1.0-2.0 and 0-.1,1.0-2.0 meters.
       output%soil_temp(ijsav_sea_ice_output(ij),1) =  &
                                output_data_sea_ice2(ij,1)
       output%soil_temp(ijsav_sea_ice_output(ij),2) = &
                               (0.3*output_data_sea_ice2(ij,2) +  &
                                0.6*output_data_sea_ice2(ij,3) +  &
                                1.0*output_data_sea_ice2(ij,4))/1.9
     *ndife
     *  ! use a default value
     *utput%soil_temp(ijsav_sea_ice_output(ij),:) = frz_iceif
# 1268
     *ocate (bitmap_sea_ice_output2, bitmap_sea_ice_input2,  &
             output_data_sea_ice2, input_dat)

!-----------------------------------------------------------------------
! for snow liq equiv, interpolate snow at land and sea ice points
! separately to prevent large gfs-imposed depths over land from
! influencing the snow depth at ice.
!-----------------------------------------------------------------------
     **,"- INTERPOLATE SNOW LIQUID EQUIV FROM INPUT GRID."
     *p_land_output=.false.
     *t_data_land=0.0
     *output_tmp=kgds_outputoutput_tmp
     *(1) = kgdso1unt_land_outputate
# 1282
     *(ibo(1))
 call ipolates(int_opt_snow, ipopt_snow, kgds_input, kgds_output_tmp,   
              (imdl_input*jmdl_input), count_land_output,               
               1, 1, bitmap_land_input, input%snow_liq_equiv,  &
               no, lats_land_output, lons_land_output, ibo, bitmap_land_
               output_data_land, iret)
     *ret /= 0) thennt
     **,'- ERROR IN IPOLATES ',ireturn
# 1292
     *ocate(ibo)
     *t%snow_liq_equiv= 0.0 ! non-land
! the budget interpolation can spread very shallow amounts of snow over
! somewhat large areas.  eliminate these.  make sure these zeroed
! amounts agree with the snow depth calculated later.
     * = 1, count_land_output
   if (bitmap_land_output(ij)) thenf
     * (int_opt == 3 .and. output_data_land(ij) < 0.2) then
       output%snow_liq_equiv(ijsav_land_output(ij))=0.0
     *lse
       output%snow_liq_equiv(ijsav_land_output(ij))=output_data_land(ij)
     *nd ifef
# 1305
     *(abs(lats_land_output(ij)) > 55.0) then
       output%snow_liq_equiv(ijsav_land_output(ij))= 2.5  ! search failed, use default (mm)
     *nd ifdif
# 1310
! now do snow over sea ice.
     *p_sea_ice_output=.false.
     *t_data_sea_ice=0.0
     *output_tmp=kgds_outputoutput_tmp
     *(1) = kgdso1unt_sea_ice_outputate
# 1316
     *(ibo(1))
 call ipolates(int_opt_snow, ipopt_snow, kgds_input, kgds_output_tmp,   
              (imdl_input*jmdl_input), count_sea_ice_output,            
               1, 1, bitmap_sea_ice_input, input%snow_liq_equiv,  &
               no, lats_sea_ice_output, lons_sea_ice_output, ibo,  &
               bitmap_sea_ice_output, output_data_sea_ice, iret)
     *ret /= 0) thennt
     **,'- ERROR IN IPOLATES ',ireturn
# 1326
     *ocate(ibo)
     * = 1, count_sea_ice_output
   if (bitmap_sea_ice_output(ij)) thenutput
     *%snow_liq_equiv(ijsav_sea_ice_output(ij))=output_data_sea_iceif
# 1332

!-----------------------------------------------------------------------
! set state variables when initializing a land ice run.
!-----------------------------------------------------------------------
     *andice_opt == 1 .or. landice_opt == 2) thennt
     **,"- INITIALIZE STATE FIELDS AT POINTS WITH PERMANENT LAND ICE"
   do ij = 1, ijmdl_outputf
     * (output%lsmask(ij) > 0.0 .and. output%veg_type(ij) == veg_type_i
       output%canopy_mc(ij)      = 0.0
       output%snow_liq_equiv(ij) = max(output%snow_liq_equiv(ij),100.) ! in mm
       output%soilm_tot(ij,:)    = 1.0
     * difdofllocated
# 1346

!-----------------------------------------------------------------------
! physical snow depth.  not used for pre noah lsm runs.
!
! if input data was pre noah lsm, the snow depth on the input grid
! will contain a -9999 flag value. in this case, set depth to
! 10 times the snow liquid water equivalent.
!
! when initializing land ice case, ensure depth is at least 1 meter.
!-----------------------------------------------------------------------

     *(output%snow_depth)) thennt
     **,"- PROCESS SNOW DEPTH"
   if (maxval(input%snow_depth) < -999.) thenrint
     **,"- INITIALIZE SNOW DEPTH FROM LIQ EQUIV."
     *o ij = 1, ijmdl_output
       output%snow_depth(ij) = output%snow_liq_equiv(ij)*10.0
     *nddoerint
# 1365
     **,"- INTERPOLATE SNOW DEPTH FROM INPUT GRID."
     *itmap_land_output=.false.
     *utput_data_land=0.0
     *gds_output_tmp=kgds_outputgds_output_tmp
     *(1) = kgdso1llocate
     *(ibo(1))
     *o=count_land_outputall
     * ipolates(int_opt_snow, ipopt_snow, kgds_input, kgds_output_tmp
                  (imdl_input*jmdl_input), count_land_output,           
                   1, 1, bitmap_land_input, input%snow_depth,  &
                   no, lats_land_output, lons_land_output, ibo, bitmap_l
                   output_data_land, iret)
     *f (iret /= 0) then
       print*,'- ERROR IN IPOLATES ',iret
       returnndifeallocate
# 1381
     *(ibo)
! gfs does not use snow depth over sea ice. so just set to a
! 10:1 ratio at all non-land points for display purposes.
! then let the code below overwrite the data over land.
! note: very shallow amounts of liquid equivalent are zeroed out
! when the budget interpolation is used.  make sure depth is consistent.
     *utput%snow_depth= output%snow_liq_equiv*10.0 ! non-land
     *o ij = 1, count_land_output
       if (bitmap_land_output(ij)) then
         if (output%snow_liq_equiv(ijsav_land_output(ij))==0.0) then
           output%snow_depth(ijsav_land_output(ij))=0.0
         else
           output%snow_depth(ijsav_land_output(ij))=output_data_land(ij)
         end if
       else  ! default value.
         output%snow_depth(ijsav_land_output(ij)) = &
                           output%snow_liq_equiv(ijsav_land_output(ij))*
       endifnddof
# 1400
     * (landice_opt == 1 .or. landice_opt == 2) then
       do ij = 1, ijmdl_output
         if (output%lsmask(ij) > 0.0 .and.  &
             output%veg_type(ij) == veg_type_ice) then
           output%snow_depth(ij) = max(output%snow_depth(ij),1000.) ! in mm
         end if
       enddondififf
# 1410

!-----------------------------------------------------------------------
! adjust soil temperatures for new orography
!-----------------------------------------------------------------------

 call adjust_soilt_for_orog(output%soil_temp, orog_sav, output%orog, &
                            output%lsmask, ijmdl_output, nsoil_output)

     *ocate (orog_sav)

!-----------------------------------------------------------------------
! if input grid had landice, and output grid has landice, then
! need to ensure terrain adjustment did not raise sub-surface
! temperature above freezing.  if initializing landice,
! use the substrate temp, which has already been qc'd for t>freezing.
!-----------------------------------------------------------------------

     *andice_opt == 2) thennt
     **,"- ENSURE COLUMN TEMPERATURES BELOW FREEZING AT LAND ICE"
   do ij = 1, ijmdl_outputf
     * (output%lsmask(ij) > 0.0 .and.  &
         output%veg_type(ij) == veg_type_ice) then
       output%skin_temp(ij)   = min(output%skin_temp(ij), frz_h20)
       output%soil_temp(ij,:) = min(output%soil_temp(ij,:), frz_h20)
     *ndifdo
# 1437

     *andice_opt == 1) thennt
     **,"- INITIALIZE COLUMN TEMPERATURES AT POINTS WITH LAND ICE"
   do ij = 1, ijmdl_outputf
     * (output%lsmask(ij) > 0.0 .and.   &
         output%veg_type(ij) == veg_type_ice) then
       output%skin_temp(ij)   = output%substrate_temp(ij)
       output%soil_temp(ij,:) = output%substrate_temp(ij)
     *difdof
# 1448

!-----------------------------------------------------------------------
! soil type is discreet field, always use nearest neighbor.
!-----------------------------------------------------------------------
     **,"- INTERPOLATE SOIL TYPE"
     * = 1, ijmdl_output
   if (output%lsmask(ij) == 0.0) then  ! non-land points
     *utput%soil_type(ij) = 0
# 1457
     * ef ( (nn_iindx_wrt_input_grid(ij) /= flag_value)   .and.  &
          (nn_jindx_wrt_input_grid(ij) /= flag_value) ) then
       ii = nn_iindx_wrt_input_grid(ij)
       jj = nn_jindx_wrt_input_grid(ij)
       output%soil_type(ij) = input%soil_type(ii,jj)
     *lse  ! no nearest neighbor that is land.  use default.
       if (landice_opt == 2 .and. output%veg_type(ij) == veg_type_ice) t
         output%soil_type(ij) = soil_type_ice
       else
         output%soil_type(ij) = 2
       end ifndifif
# 1471
!-----------------------------------------------------------------------
! the soil moisture rescaling algorithms depend on any changes to
! soil type.  so, save the soil type interpolated from the input grid.
!-----------------------------------------------------------------------
     *le_soil_moist = .false.

!-----------------------------------------------------------------------
! set soil type flag value at landice when initializing landice runs.
!-----------------------------------------------------------------------
     *andice_opt == 1) then
   do ij = 1, ijmdl_outputf
     * (output%lsmask(ij) > 0.0 .and.  &
         output%veg_type(ij) == veg_type_ice) then
       output%soil_type(ij) = soil_type_icendifdofate
# 1488

     *(soil_type_sav(ijmdl_output))
     *type_sav    = output%soil_type  ! interpolated from input grid.

!-----------------------------------------------------------------------
! replace interpolated soil type with externally generated soil type
! on the output grid (if this data was read in).
!-----------------------------------------------------------------------
     *llocated(soil_type_output_ext)) thennt
     **,'- REPLACE SOIL TYPE WITH EXTERNAL DATA.'
     *cale_soil_moist = .true.
     *put%soil_type = 0
     *re (output%lsmask > 0.0) output%soil_type = soil_type_output_extll
     *ocate (soil_type_output_ext)
   if (landice_opt == 1 .or. landice_opt == 2) theno
     * ij = 1, ijmdl_output
       if (output%lsmask(ij) > 0.0 .and.  &
           output%veg_type(ij) == veg_type_ice) then
           output%soil_type(ij) = soil_type_ice
       endifnddo
# 1509
     * iffandice_opt
# 1511

!-----------------------------------------------------------------------
! this option is used when the input grid has land ice, but the
! user does not want land ice on the output grid.  need to set
! a soil moisture at these points.  use the reference value for the
! soil type as a default start value.
!-----------------------------------------------------------------------
     * == 4) then 
   do ij = 1, ijmdl_outputf
     * (output%lsmask(ij) > 0.0 .and.    &
         output%soilm_tot(ij,1) > 0.99) then  ! flag for landice. at these
                                              ! points all layers are set to 1.0
                                              ! so just check the top layer.
       soil_type_sav(ij) = output%soil_type(ij) ! turn off soilm rescaling
       output%soilm_tot(ij,:) = smcref_output(output%soil_type(ij))
     *ndifdo
# 1529

!-----------------------------------------------------------------------
! greenness.  valid only over land.
!-----------------------------------------------------------------------
     *llocated(greenfrc_output_ext)) thennt
     **,'- USE EXTERNAL DATA TO SET GREENNESS.'
     *put%greenfrc = 0.
     *re(output%lsmask > 0.) output%greenfrc = greenfrc_output_extllocat
     *e (greenfrc_output_ext)
 else ! greenness interpolated from input grid.
     *nt*,"- INTERPOLATE GREENNESS FROM INPUT GRID."
     *map_land_output=.false.
     *put_data_land=0.0
     *s_output_tmp=kgds_outputs_output_tmp
     *(1) = kgdso1ocate
     *(ibo(1))
     *count_land_outputl
     * ipolates(int_opt, ipopt, kgds_input, kgds_output_tmp,   &
                (imdl_input*jmdl_input), count_land_output,             
                 1, 1, bitmap_land_input, input%greenfrc,  &
                 no, lats_land_output, lons_land_output, ibo,  &
                 bitmap_land_output, output_data_land, iret)
   if (iret /= 0) thenrint
     **,'- ERROR IN IPOLATES ',ireteturnifllocate
# 1555
     *(ibo)
     *put%greenfrc= 0.0 ! non-land
   do ij = 1, count_land_outputf
     * (bitmap_land_output(ij)) then
       output%greenfrc(ijsav_land_output(ij))=output_data_land(ij)
     *lse
       output%greenfrc(ijsav_land_output(ij))=0.4  ! search failed, use default
     *ndifdof
# 1565
!-----------------------------------------------------------------------
! once greenness is set, zero output any canopy moisture content
! at points with zero vegetation.  recall, a greenness of 1% actually
! means bare ground.
!-----------------------------------------------------------------------
     * = 1, ijmdl_output
   if (output%greenfrc(ij) < 0.011) thenutput
     *%canopy_mc(ij) = 0.0
     *if
# 1575
!-----------------------------------------------------------------------
! greenness is zero at landice points.
!-----------------------------------------------------------------------
     *andice_opt ==1 .or. landice_opt == 2) thennt
     **,"- SET GREENNESS AT LANDICE POINTS."
   do ij = 1, ijmdl_outputf
     * (output%lsmask(ij) > 0.0 .and.  &
         output%veg_type(ij) == veg_type_ice) then
       output%greenfrc(ij) = 0.0
     *ndifdofllocated
# 1587

!-----------------------------------------------------------------------
! min/max greenness.  only processed when using the noah lsm.
!-----------------------------------------------------------------------

     *(output%greenfrc_min) .and. allocated(output%greenfrc_max)
   if (allocated(greenfrc_min_output_ext) .and. &
       allocated(greenfrc_max_output_ext)) thenrint
     **,'- USE EXTERNAL DATA FOR MAX/MIN GREEENNESS.'
     *utput%greenfrc_min = 0.
     *here(output%lsmask > 0.) output%greenfrc_min = greenfrc_min_output
     *eallocate (greenfrc_min_output_ext)
     *utput%greenfrc_max = 0.
     *here(output%lsmask > 0.) output%greenfrc_max = greenfrc_max_output
     *eallocate (greenfrc_max_output_ext)
     *erint
     **,"- INTERPOLATE MAX/MIN GREENNESS FROM INPUT GRID."
     *llocate(bitmap_land_output2(count_land_output,2))
     *itmap_land_output2=.false.
     *llocate(bitmap_land_input2(imdl_input,jmdl_input,2))
     *itmap_land_input2(:,:,1)=bitmap_land_inputitmap_land_input2
     *(:,:,2)=bitmap_land_inputllocate
     *(output_data_land2(count_land_output,2))
     *utput_data_land2=0.0
     *llocate(input_dat(imdl_input,jmdl_input,2))
     *nput_dat(:,:,1)=input%greenfrc_minnput_dat
     *(:,:,2)=input%greenfrc_maxgds_output_tmp
     *=kgds_outputgds_output_tmp
     *(1) = kgdso1llocate
     *(ibo(2))
     *o=count_land_outputall
     * ipolates(int_opt, ipopt, kgds_input, kgds_output_tmp,   &
                  (imdl_input*jmdl_input), count_land_output,           
                   2, (/1,1/), bitmap_land_input2, input_dat,  &
                   no, lats_land_output, lons_land_output, ibo,  &
                   bitmap_land_output2, output_data_land2, iret)
     *f (iret /= 0) then
       print*,'- ERROR IN IPOLATES ',iret
       returnndifeallocate
# 1627
     *(ibo)
     *utput%greenfrc_min= 0.0 ! non-land
     *utput%greenfrc_max= 0.0 ! non-land
     *o ij = 1, count_land_output
       if (bitmap_land_output2(ij,1)) then
         output%greenfrc_min(ijsav_land_output(ij))=output_data_land2(ij
       else
         output%greenfrc_min(ijsav_land_output(ij))=0.2  ! search failed, use default
       endifnddoo
# 1637
     * ij = 1, count_land_output
       if (bitmap_land_output2(ij,2)) then
         output%greenfrc_max(ijsav_land_output(ij))=output_data_land2(ij
       else
         output%greenfrc_max(ijsav_land_output(ij))=0.5  ! search failed, use default
       endifnddoeallocate
# 1644
     *(bitmap_land_input2, bitmap_land_output2, input_dat, outp
     * if
!-----------------------------------------------------------------------
! set greenness to zero at land ice points.
!-----------------------------------------------------------------------
   if (landice_opt ==1 .or. landice_opt == 2) thenrint
     **,"- SET MAX/MIN GREENNESS TO ZERO AT LAND ICE."
     *o ij = 1, ijmdl_output
       if (output%lsmask(ij) > 0.0 .and.  &
           output%veg_type(ij) == veg_type_ice) then
         output%greenfrc_min(ij) = 0.0
         output%greenfrc_max(ij) = 0.0
       endifnddo
# 1658
     * iffllocated
# 1660

!-----------------------------------------------------------------------
! slope type not used for pre-noah lsm runs.  treat as discreet field.
!-----------------------------------------------------------------------
     * (output%slope_type)) then
   if (allocated(slope_type_output_ext)) thenrint
     **,'- USE EXTERNAL DATA FOR SLOPE TYPE.'
     *utput%slope_type = 0h
     *ere (output%lsmask > 0.0) output%slope_type = slope_type_output_ee
     *allocate (slope_type_output_ext)
     *e  ! interpolate from input grid
     *rint*,'- INTERPOLATE SLOPE TYPE FROM INPUT GRID.'o
     * ij = 1, ijmdl_output
       if (output%lsmask(ij) == 0.0) then  ! non-land points
           output%slope_type(ij) = 0
       else
         if ( (nn_iindx_wrt_input_grid(ij) /= flag_value)   .and.  &
              (nn_jindx_wrt_input_grid(ij) /= flag_value) ) then
           ii = nn_iindx_wrt_input_grid(ij)
           jj = nn_jindx_wrt_input_grid(ij)
           output%slope_type(ij) = input%slope_type(ii,jj)
         else  ! no nearest neighbor that is land.  use default.
           output%slope_type(ij) = 2
         endif
       endifnddo
# 1686
     * if
   if (landice_opt == 1 .or. landice_opt == 2) thenrint
     **,"- INITIALIZE SLOPE TYPE AT LANDICE POINTS."
     *o ij = 1, ijmdl_output
       if (output%lsmask(ij) > 0.0 .and.  &
           output%veg_type(ij) == veg_type_ice) then
         output%slope_type(ij) = 9
       endifnddo
# 1695
     * iffREE_ALB
# 1697

!-----------------------------------------------------------------------
! snow free albedo. note: at non-land points, the gfs calculates
! albedo internally.  for surface files prior to 200501, the
! albedo was set to a flag value of 0.01, otherwise, it is set to
! 0.06.
!-----------------------------------------------------------------------

     * : if (allocated(output%alvsf)) then ! using 4 component albedo

     *llocated(alvsf_output_ext) .and.  &
     *llocated(alnsf_output_ext) .and.  &
     *llocated(alnwf_output_ext) .and.  &
     *llocated(alvwf_output_ext) .and.  &
     *llocated(facsf_output_ext) .and.  &
     *llocated(facwf_output_ext)) then
   if (allocated(output%sea_ice_fract) .and. &
       allocated(output%sea_ice_depth) ) then  ! surface file is ivs 200501
     *utput%alvsf = 0.06   
     *utput%alnsf = 0.06  
     *utput%alnwf = 0.06  
     *utput%alvwf = 0.06e
     *    ! surface file prior to ivs 200501
     *utput%alvsf = 0.01   
     *utput%alnsf = 0.01 
     *utput%alnwf = 0.01 
     *utput%alvwf = 0.01
     *ifnt
     **,'- SET ALVSF WITH EXTERNAL DATA OVER LAND'
     *re (output%lsmask > 0.0) output%alvsf = alvsf_output_extllocate
     * (alvsf_output_ext)
     *nt*,'- SET ALNSF WITH EXTERNAL DATA OVER LAND'
     *re (output%lsmask > 0.0) output%alnsf = alnsf_output_extllocate
     * (alnsf_output_ext)
     *nt*,'- SET ALNWF WITH EXTERNAL DATA OVER LAND'
     *re (output%lsmask > 0.0) output%alnwf = alnwf_output_extllocate
     * (alnwf_output_ext)
     *nt*,'- SET ANVWF WITH EXTERNAL DATA OVER LAND'
     *re (output%lsmask > 0.0) output%alvwf = alvwf_output_extllocate
     * (alvwf_output_ext)
     *nt*,'- SET FACSF WITH EXTERNAL DATA OVER LAND'
     *put%facsf = 0.0  ! non-land
     *re (output%lsmask > 0.0) output%facsf = facsf_output_extllocate
     * (facsf_output_ext)
     *nt*,'- SET FACWF WITH EXTERNAL DATA OVER LAND'
     *put%facwf = 0.0
     *re (output%lsmask > 0.0) output%facwf = facwf_output_extllocate
     * (facwf_output_ext)
 else ! interp from input grid.
     *nt*,"- INTERP SNOW-FREE ALBEDO FROM INPUT GRID"
     *llocate(bitmap_land_output2(count_land_output,6))
     *itmap_land_output2=.false.
     *llocate(bitmap_land_input2(imdl_input,jmdl_input,6))
     *itmap_land_input2(:,:,1)=bitmap_land_inputitmap_land_input2
     *(:,:,2)=bitmap_land_inputitmap_land_input2
     *(:,:,3)=bitmap_land_inputitmap_land_input2
     *(:,:,4)=bitmap_land_inputitmap_land_input2
     *(:,:,5)=bitmap_land_inputitmap_land_input2
     *(:,:,6)=bitmap_land_inputllocate
     *(output_data_land2(count_land_output,6))
     *utput_data_land2=0.0
     *llocate(input_dat(imdl_input,jmdl_input,6))
     *nput_dat(:,:,1)=input%alnsfnput_dat
     *(:,:,2)=input%alnwfnput_dat
     *(:,:,3)=input%alvsfnput_dat
     *(:,:,4)=input%alvwfnput_dat
     *(:,:,5)=input%facsfnput_dat
     *(:,:,6)=input%facwfgds_output_tmp
     *=kgds_outputgds_output_tmp
     *(1) = kgdso1llocate
     *(ibo(6))
     *o=count_land_outputall
     * ipolates(int_opt, ipopt, kgds_input, kgds_output_tmp,   &
                  (imdl_input*jmdl_input), count_land_output,          &
                   6, (/1,1,1,1,1,1/), bitmap_land_input2, input_dat,  &
                   no, lats_land_output, lons_land_output, ibo,  &
                   bitmap_land_output2, output_data_land2, iret)
     *f (iret /= 0) then
       print*,'- ERROR IN IPOLATES ',iret
       returnndifeallocate
# 1778
     *(ibo)
     *utput%facsf=0.0  ! non-land
     *utput%facwf=0.0  ! non-land
     *f (allocated(output%sea_ice_fract) .and. & ! non-land points
         allocated(output%sea_ice_depth) ) then  ! surface file is ivs 200501
       output%alnsf=0.06
       output%alnwf=0.06
       output%alvsf=0.06
       output%alvwf=0.06
     *lse
       output%alnsf=0.01
       output%alnwf=0.01
       output%alvsf=0.01
       output%alvwf=0.01
     *ndif         
     *o ij = 1, count_land_output
       if (bitmap_land_output2(ij,1)) then
         output%alnsf(ijsav_land_output(ij))=output_data_land2(ij,1)
       else
         output%alnsf(ijsav_land_output(ij))=0.25  ! search failed, use default
       endifnddoo
# 1800
     * ij = 1, count_land_output
       if (bitmap_land_output2(ij,2)) then
         output%alnwf(ijsav_land_output(ij))=output_data_land2(ij,2)
       else
         output%alnwf(ijsav_land_output(ij))=0.2  ! search failed, use default
       endifnddoo
# 1807
     * ij = 1, count_land_output
       if (bitmap_land_output2(ij,3)) then
         output%alvsf(ijsav_land_output(ij))=output_data_land2(ij,3)
       else
         output%alvsf(ijsav_land_output(ij))=0.15  ! search failed, use default
       endifnddoo
# 1814
     * ij = 1, count_land_output
       if (bitmap_land_output2(ij,4)) then
         output%alvwf(ijsav_land_output(ij))=output_data_land2(ij,4)
       else
         output%alvwf(ijsav_land_output(ij))=0.1  ! search failed, use default
       endifnddoo
# 1821
     * ij = 1, count_land_output
       if (bitmap_land_output2(ij,5)) then
         output%facsf(ijsav_land_output(ij))=output_data_land2(ij,5)
       else
         output%facsf(ijsav_land_output(ij))=0.5  ! search failed, use default
       endifnddoo
# 1828
     * ij = 1, count_land_output
       if (bitmap_land_output2(ij,6)) then
         output%facwf(ijsav_land_output(ij))=output_data_land2(ij,6)
       else
         output%facwf(ijsav_land_output(ij))=0.5  ! search failed, use default
       endifnddoeallocate
# 1835
     *(bitmap_land_input2, bitmap_land_output2, input_dat, outpff
# 1837
!-----------------------------------------------------------------------
! single snowfree albedo used by wrf/nmm
!-----------------------------------------------------------------------
     * (allocated(output%snow_free_albedo)) then
   if (allocated(snow_free_albedo_output_ext)) then
      print*,"- SET SNOW FREE ALBEDO WITH EXTERNAL DATA"
      output%snow_free_albedo = 0.06  ! open water
      where (output%lsmask > 0.0) output%snow_free_albedo =  &
                                  snow_free_albedo_output_ext  ! land
      where (output%sea_ice_flag == 1) output%snow_free_albedo = 0.65  ! sea ice
     *erint
     **,"- INTERPOLATE SNOW-FREE (BASE) ALBEDO FROM INPUT GRID."
     *itmap_land_output=.false.
     *utput_data_land=0.0
     *gds_output_tmp=kgds_outputgds_output_tmp
     *(1) = kgdso1o
     *=count_land_outputllocate
     *(ibo(1))
     *all ipolates(int_opt, ipopt, kgds_input, kgds_output_tmp,   &
                  (imdl_input*jmdl_input), count_land_output,     &
                   1, 1, bitmap_land_input, input%snow_free_albedo,  &
                   no, lats_land_output, lons_land_output, ibo,   &
                   bitmap_land_output, output_data_land, iret)
     *f (iret /= 0) then
       print*,'- ERROR IN IPOLATES ',iret
       returnndifeallocate
# 1864
     *(ibo)
     *utput%snow_free_albedo = 0.06  ! open water
     *o ij = 1, count_land_output
       if (bitmap_land_output(ij)) then
         output%snow_free_albedo(ijsav_land_output(ij))=output_data_land
       else
         output%snow_free_albedo(ijsav_land_output(ij))=0.2  ! search failed, use default
       endifnddohere
# 1873
     * (output%sea_ice_flag == 1) output%snow_free_albedo = 0.65  ! sea ice
     *if
     * SNOWFREE_ALBllocated

!-----------------------------------------------------------------------
! maximum snow albedo for noah runs.
!-----------------------------------------------------------------------
     * (output%mxsnow_alb)) then
   if (allocated(mxsnow_alb_output_ext)) thenrint
     **,'- SET MAX SNOW ALBEDO WITH EXTERNAL DATA.'
     *utput%mxsnow_alb = 0.0
     *here (output%lsmask>0.0) output%mxsnow_alb = mxsnow_alb_output_ext
     *eallocate (mxsnow_alb_output_ext)
     *e  ! interpolate from input grid
     *rint*,"- INTERPOLATE MAX SNOW ALBEDO FROM INPUT GRID."
     *itmap_land_output=.false.
     *utput_data_land=0.0
     *gds_output_tmp=kgds_outputgds_output_tmp
     *(1) = kgdso1o
     *=count_land_outputllocate
     *(ibo(1))
     *all ipolates(int_opt, ipopt, kgds_input, kgds_output_tmp,   &
                  (imdl_input*jmdl_input), count_land_output,           
                   1, 1, bitmap_land_input, input%mxsnow_alb,  &
                   no, lats_land_output, lons_land_output, ibo, bitmap_l
                   output_data_land, iret)
     *f (iret /= 0) then
       print*,'- ERROR IN IPOLATES ',iret
       returnndifeallocate
# 1903
     *(ibo)
     *utput%mxsnow_alb= 0.0 ! non-land
     *o ij = 1, count_land_output
       if (bitmap_land_output(ij)) then
         output%mxsnow_alb(ijsav_land_output(ij))=output_data_land(ij)
       else
         output%mxsnow_alb(ijsav_land_output(ij))=0.7  ! search failed, use default
       endifnddo
# 1912
     * iffescale_soil_moist
# 1914

!-----------------------------------------------------------------------
! if the soil types have changed, need to rescale
! the soil moisture.
!-----------------------------------------------------------------------
     *) thennt
     **,'- RESCALE SOIL MOISTURE FOR NEW SOIL TYPE.'o
     *cate (soilm_sav(ijmdl_output,nsoil_output))
     *lm_sav = output%soilm_totocate
     * (lsmask_output_temp(ijmdl_output))
     *ask_output_temp = output%lsmask
!   don't bother doing this at landice points as soil moisture is
!   not used.
   if (landice_opt == 1 .or. landice_opt == 2) theno
     * ij = 1, ijmdl_output
       if (output%lsmask(ij) > 0.0 .and.  &
           output%veg_type(ij) == veg_type_ice) then
         lsmask_output_temp(ij) = 0.0
       endifnddoifl
# 1935
     * rescale_soilm(soilm_sav, output%soilm_tot,              &
                      soil_type_sav, output%soil_type,          &
                      smcdry_input, smcdry_output,              &
                      smcwilt_input, smcwilt_output,            &
                      smcref_input, smcref_output,              &
                      smcmax_input, smcmax_output,              &
                      lsmask_output_temp, output%greenfrc,      &
                      ijmdl_output,                &
                      nsoil_output, max_soil_types)
     *llocate (soilm_sav)
     *llocate (lsmask_output_temp)
     *focate
     *(soil_type_sav)

!-----------------------------------------------------------------------
! now calculate the liquid portion of the total soil moisture.
!-----------------------------------------------------------------------
     *llocated (output%soilm_liq)) thennt
     **,'- CALCULATE LIQUID PORTION OF TOTAL SOIL MOISTURE.'
     *put%soilm_liq = 1.0  ! flag value for non-land points
     *ocate (lsmask_output_temp(ijmdl_output))
     *ask_output_temp = output%lsmask
! don't bother doing this at landice points as soil moisture is
! not used.
   if (landice_opt == 1 .or. landice_opt == 2) theno
     * ij = 1, ijmdl_output
       if (output%lsmask(ij) > 0.0 .and.  &
           output%veg_type(ij) == veg_type_ice) then
         lsmask_output_temp(ij) = 0.0
       endifnddoifl
# 1967
     * calc_liq_soilm(output%soil_type, output%soilm_tot, &
                       output%soil_temp, output%soilm_liq, &
                       lsmask_output_temp, beta_output,         &
                       psis_output, smcmax_output,         &
                       max_soil_types,                     &
                       ijmdl_output, nsoil_output)
     *llocate (lsmask_output_temp)
     *fllocated

!-----------------------------------------------------------------------
! for single albedo, need to calc snow effects.
!-----------------------------------------------------------------------
     * (output%albedo)) thent
     **,"- CALCULATE SNOW EFFECT ON ALBEDO"
     *ut%albedo=0.06  ! open water value
     *cate (snow_m(ijmdl_output))
     *_m = output%snow_liq_equiv * 0.001
     * calc_albedo(output%lsmask, output%veg_type, ijmdl_output, &
                   salp_output, snup_output, max_veg_types, &
                   output%snow_free_albedo, output%mxsnow_alb, &
                   snow_m, output%greenfrc, output%albedo)
     *e(output%sea_ice_flag == 1) output%albedo = 0.65 ! as in eta after may 3, 05
     *locate (snow_m)
     *focate

     * (bitmap_land_output, bitmap_land_input)
     *ocate (bitmap_nonland_output, bitmap_nonland_input)
     *ocate (bitmap_sea_ice_output, bitmap_sea_ice_input)
     *ocate (ijsav_land_output, ijsav_nonland_output, ijsav_sea_ice_outp
     *ocate (lats_land_output, lons_land_output)
     *ocate (lats_nonland_output, lons_nonland_output)
     *ocate (lats_sea_ice_output, lons_sea_ice_output)
     *ocate (nn_iindx_wrt_input_grid, nn_jindx_wrt_input_grid)
     *ocate (output_data_land, output_data_nonland, output_data_sea_ice)

     *nubroutine

     * interputine
!-----------------------------------------------------------------------
     * setup(kgds_input, input, imdl_input, jmdl_input, & 
                  kgds_output, iret)
!$$$ subprogram documentation block
!
! subprogram: setup    perform various setup tasks
!   prgmmr: gayno           org: w/np2     date: 2005-10-19
!
! abstract: read configuration namelist, calculate model resolution,
!           calculate some soil parameters.
!
! program history log:
!  2005-10-19  gayno   - initial version
!
! usage: 'call setup' with the following arguments
!   input arguments (input/output denotes input/output grid):
!     kgds_input             grib grid description section of input grid
!     input                  surface data on input grid
!     i/jmdl_input           i/j dimensions of input grid
!     kgds_output            grib grid description section of output grid
!   output arguments
!     iret                   error status, non-zero means an error occurred
!   other outputs:
!     mdl_res_input/output   resolution (in degrees) of model
!                            input/output grid
!     smcref_input/output    onset of soil moisture stress, input/output
!                            grid values
!     smcdry_input/output    air dry soil moisture limit, input/output
!                            grid values
!     smcwilt_input/output   plant wilting point, input/output grid
!                            values
!     soil_type_ice          soil type for land ice points
!     veg_type_ice           veg type for land ice (output grid)
!     veg_type_ice_input     veg type for land ice (input grid)
!
! namelists:
!   options:
!     climo_fields_opt       option for determining climo fields on
!                            output grid.  1 ONLY!!
!                            1-interpolate all from input grid
!                            2-interpolate veg, soil, slope type
!                              from input grid.  others from
!                              cycle program.
!                            3-all from cycle program.
!     landice_opt            1-no landice input grid -> landice output grid
!                            2-landice input grid -> landice output grid
!                            3-no landice input grid -> no landice output grid
!                            4-landice input grid -> no landice output grid
!                            5-landice on output grid regardless of
!                              whether input grid has landice or not.
!   soil_parameters (input/output grid values):
!     soil_src_input/output  source of soil type database (ex: zobler)
!     smclow_input/output    soil moisture scalar multiplier
!     smchigh_input/output   soil moisture scalar multiplier
!     smcmax_input/output    maximum soil moisture content
!     beta_input/output      soil 'b' parameter
!     psis_input/output      saturated soil potential
!     satdk_input/output     saturated soil hydraulic conductivity
!
!   veg_parameters:
!     veg_src_input/output   source of veg type database (ex: usgs)
!     salp_output            plant factor in albedo calculation
!     snup_output            plant factor in albedo calculation
!
! input files:
!   unit 81    configuration namelists
!
! subprograms called:
!   calc_soil_parms  - calculate soil parameters
!
! attributes:
!   langauge: fortran 90
!
!$$$
     *oil_utils,  only            : calc_soil_parmscit

     * nonecter

     **10                    :: soil_src_input, soil_src_output, &
                                    veg_src_input, veg_src_outputer

     *, intent(in)             :: imdl_input, jmdl_inputer
     *                         :: i,j, istater
     *, intent(in)             :: kgds_input(200), kgds_output(200)
     *er, intent(inout)          :: iretsfc2d

     *)                     :: inputist

     * /soil_parameters/        soil_src_input,  &
                                   smclow_input,    &
                                   smchigh_input,   &
                                   smcmax_input,    &
                                   beta_input,      &
                                   psis_input,      &
                                   satdk_input,     &
                                   soil_src_output, &
                                   smclow_output,   &
                                   smchigh_output,  &
                                   smcmax_output,   &
                                   beta_output,     &
                                   psis_output,     &
                                   satdk_outputist

     * /veg_parameters/         veg_src_input,   &
                                   veg_src_output,  &
                                   salp_output,     &
                                   snup_outputist

     * /options/                climo_fields_opt, &
                                   landice_opt0

# 2117
     **,"- READ CONFIGURATION NAMELIST."
     *81, iostat=istat, err=900)
     *81, nml=soil_parameters, iostat=istat, err=910)
     *81, nml=veg_parameters, iostat=istat, err=910)
     *81, nml=options, iostat=istat, err=910)
     *(81)

     *gds_input(1) == 4) then   ! gaussian grid
     *_res_input = 360.0 / float(kgds_input(2))
     *f (kgds_input(1) == 203) then  ! e-grid
     *_res_input = sqrt( ((float(kgds_input(9))*.001)**2) +  &
                         ((float(kgds_input(10))*.001)**2) )
     *f
     **,"- RESOLUTION OF INPUT GRID IN DEGREES IS: ", mdl_res_inputgds_o
     *utput
# 2131

     *(1) == 4 .or. kgds_output(1) == 0) then ! gaussian/latlon grid
     *_res_output = 360.0 / float(kgds_output(2))
     *f (kgds_output(1) == 203) then_res_output
     * = sqrt( ((float(kgds_output(9))*.001)**2) +  &
                         ((float(kgds_output(10))*.001)**2) )
     *f
     **,"- RESOLUTION OF OUTPUT GRID IN DEGREES IS: ", mdl_res_outputt

!-----------------------------------------------------------------------
! the flag values of soil and veg type for landice depend on
! what raw databases were used.
!-----------------------------------------------------------------------

     * case (trim(soil_src_output))
     *e("zobler")
     *oil_type_ice=9e
     *("statsgo")
     *oil_type_ice=16e
     * defaultrint
     **,'- BAD CHOICE OF OUTPUT GRID SOIL SOURCE ',trim(soil_src_outret
     *=1
     * eturnelectt
# 2155

     * case (trim(veg_src_output))
     *e("usgs")
     *eg_type_ice=24e
     *("sib")
     *eg_type_ice=13e
     * defaultrint
     **,'- BAD CHOICE OF OUTPUT GRID VEG SOURCE ',trim(veg_src_outpuret
     *=2
     * eturnelectt
# 2166

     * case (trim(veg_src_input))
     *e("usgs")
     *eg_type_ice_input=24e
     *("sib")
     *eg_type_ice_input=13e
     * defaultrint
     **,'- BAD CHOICE OF INPUT GRID VEG SOURCE ',trim(veg_src_input)
     *ret=3
     * eturnelect
# 2177

!-----------------------------------------------------------------------
! parameters for soil type on input/output grids.
!-----------------------------------------------------------------------

     **,'- CALCULATE SOIL PARAMETERS'
 call calc_soil_parms(smclow_input, smchigh_input,              &
                      smcmax_input, beta_input,                 &
                      satdk_input, psis_input, max_soil_types,  &
                      smcref_input, smcwilt_input, smcdry_input)

 call calc_soil_parms(smclow_output, smchigh_output,                 &
                      smcmax_output, beta_output,                    &
                      satdk_output, psis_output, max_soil_types,     &
                      smcref_output, smcwilt_output, smcdry_output)

!-----------------------------------------------------------------------
! certain landice options are only valid if the input grid has
! or does not have landice.  check for the soil moisture flag value
! of 1.0 at land ice points.
!-----------------------------------------------------------------------

     *andice_opt == 1 .or. landice_opt == 3) then 
   do j = 1, jmdl_input
   do i = 1, imdl_inputf
     * (input%lsmask(i,j) > 0.0 .and. input%soilm_tot(i,j,1) > 0.99) th
       print*,"- LANDICE OPTION OF ", landice_opt, " IS NOT VALID WHEN"
       print*,"- INPUT DATA HAS LANDICE."
       iret=4
       returnnd
     * ifdodof
# 2210
     * (landice_opt == 2 .or. landice_opt == 4) then
   do j = 1, jmdl_input
   do i = 1, imdl_inputf
     * (input%lsmask(i,j) > 0.0 .and. input%soilm_tot(i,j,1) > 0.99) th
       returnnd
     * ifdodont
# 2218
     **,"- LANDICE OPTION OF ", landice_opt, " IS NOT VALID WHEN"
     *nt*,"- INPUT DATA DOES NOT HAVE LANDICE."
     *t=5
     *urnf
     * (landice_opt == 5) thennt
     **,"- WILL FORCE LANDICE INITIALIZATION."
     *dice_opt = 1    ! for rest of module, 1 and 5 are equivalent.
     *fnint

# 2228

     **,"- ERROR OPENING CONFIG NAMELIST. ISTAT IS ", istatet
     *=6
     *turnint

     **,"- ERROR READING CONFIG NAMELIST. ISTAT IS ", istatet
     *=7
     *turnubroutine

     * setuputine
!-----------------------------------------------------------------------
     * get_ext_climo_global(ijmdl_output, lsmask_output, orog_outpu
                                 output, &
                                 hour, month, day, &
                                 year, fhour)
!$$$ subprogram documentation block
!
! subprogram: get_ext_climo_global    get climo fields on global grid
!   prgmmr: gayno           org: w/np2     date: 2005-10-19
!
! abstract: get climo fields, such as soil type and albedo on the
!           output grid by calling the surface cycle code.  according
!           to the option selected by the user, the climo fields on the
!           output grid will come from either surface cycle or be
!           interpolated from the input grid.  if a field is to be
!           interpolated from the input grid, the corresponding array
!           from surface cycle will be deallocated.
!           note!! surface cycle is only called to get the climo
!           fields on the grid.  NOT to update the SST, snow or sea ice.
!
! program history log:
!  2005-10-19  gayno   - initial version
!
! usage: call subroutine get_ext_climo_global  &
!                               (ijmdl_output, &
!                                lsmask_output, orog_output, &
!                                hour, month, day, &
!                                year, fhour)
!   input arguments:
!     ijmdl_output           number of grid points, output grid
!     lsmask_output          landmask (0-nonland, 1-land) output grid
!     orog_output            orography of output grid
!     hour/month/day/year    YYYYMMDDHH of cycle
!     fhour                  forecast hour with respect to cycle time
!   outputs:
!     substrate_temp_output_ext   substrate temperature from sfccycle
!     soil_type_output_ext        soil type from sfccycle
!     veg_type_output_ext         vegetation type from sfccycle
!     slope_type_output_ext       slope type from sfccycle
!     mxsnow_alb_output_ext       max snow albedo from sfccycle
!     z0_output_ext               roughness from sfccycle
!     greenfrc_output_ext         greenness fraction from sfccycle
!     greenfrc_min_output_ext     min greenness fraction from sfccycle
!     greenfrc_max_output_ext     max greenness fraction from sfccycle
!     alnsf_output_ext            near ir albedo, strong cosz depend. from sfccycle
!     alnwf_output_ext            near ir albedo, weak cosz depend. from sfccycle
!     alvsf_output_ext            vis albedo, strong cosz depend. from sfccycle
!     alvwf_output_ext            vis albedo, weak cosz depend. from sfccycle
!     facsf_output_ext            fraction, strong cosz dependence from sfccycle
!     facwf_output_ext            fraction, weak cosz dependence from sfccycle
!
! subprograms called:
!   sfccycle  - calculate soil parameters
!
! attributes:
!   langauge: fortran 90
!
!$$$
     *achine, only                 : kind_io8cit

     * noneer

     *, intent(in)                   :: hour, month, day, yearer
     *, intent(in)                   :: ijmdl_outputer
     *                               :: lsoiler
     *, parameter                    :: lugb = 51
     * er                               :: nlunit

     * intent(in)                      :: fhour
     * intent(in)                      :: lsmask_output(ijmdl_output)
     * intent(in)                      :: orog_output(ijmdl_output)

 real (kind=kind_io8), allocatable :: sig1t(:), &
      slmask(:), orog(:),  sihfcs(:), sicfcs(:), sitfcs(:),&
      swdfcs(:), slcfcs(:,:), vmnfcs(:), vmxfcs(:),   &
      slpfcs(:), absfcs(:),  TSFFCS(:), SNOFCS(:), ZORFCS(:), &
      ALBFCS(:,:), TG3FCS(:), CNPFCS(:), SMCFCS(:,:), STCFCS(:,:),  &
      slifcs(:), AISFCS(:), F10M(:), vegfcs(:), vetfcs(:), &
      sotfcs(:), ALFFCS(:,:), CVFCS(:), CVBFCS(:), CVTFCS(:), &
      lats(:), lons(:)

     *kind=kind_io8)                   :: deltsfc, fhsfc1d

     *), intent(in)   :: output

     * = 4
     *fc = 0.0
 fh = fhourate

     * (sig1t(ijmdl_output),  slmask(ijmdl_output), &
           orog(ijmdl_output),   sihfcs(ijmdl_output), &
           sicfcs(ijmdl_output), swdfcs(ijmdl_output), &
           sitfcs(ijmdl_output),                       &
           slcfcs(ijmdl_output,lsoil), vmnfcs(ijmdl_output), &
           vmxfcs(ijmdl_output), slpfcs(ijmdl_output),  &
           absfcs(ijmdl_output), TSFFCS(ijmdl_output),  &
           SNOFCS(ijmdl_output), zorfcs(ijmdl_output),  &
           ALBFCS(ijmdl_output,4), tg3fcs(ijmdl_output),  &
           CNPFCS(ijmdl_output), SMCFCS(ijmdl_output,lsoil),  &
           STCFCS(ijmdl_output,lsoil), slifcs(ijmdl_output), &
           AISFCS(ijmdl_output), F10M(ijmdl_output),         &
           vegfcs(ijmdl_output), vetfcs(ijmdl_output),       &
           sotfcs(ijmdl_output), ALFFCS(ijmdl_output,2),     &
           CVFCS(ijmdl_output), CVBFCS(ijmdl_output),        &
           CVTFCS(ijmdl_output), lats(ijmdl_output), & 
           lons(ijmdl_output))

     *k = lsmask_output
 orog = orog_outputt
     * = 35

     *  = 0.0; sihfcs = 0.0; sicfcs = 0.0; swdfcs = 0.0
     *s = 0.0; vmnfcs = 0.0; vmxfcs = 0.0; slpfcs = 0.0
     *s = 0.0; tsffcs = 0.0; snofcs = 0.0; zorfcs = 0.0
     *s = 0.0; tg3fcs = 0.0; cnpfcs = 0.0; smcfcs = 0.0
     *s = 0.0; slifcs = 0.0; aisfcs = 0.0; f10m   = 0.0
     *s = 0.0; vetfcs = 0.0; sotfcs = 0.0; alffcs = 0.0
     *  = 0.0; cvbfcs = 0.0; cvtfcs = 0.0; sitfcs = 0.0
     *output%lats; lons=output%lons
# 2351
 call SFCCYCLE(LUGB,ijmdl_output,LSOIL,SIG1T,DELTSFC,     &
               year,month,day,hour,FH,                    &
               lats, lons, SLMASK,OROG,     &
               SIHFCS,SICFCS,SITFCS,                      &
               SWDFCS,SLCFCS,                             &
               VMNFCS, VMXFCS, SLPFCS, ABSFCS,            &
               TSFFCS,SNOFCS,ZORFCS,ALBFCS,TG3FCS,        &
               CNPFCS,SMCFCS,STCFCS,slifcs,AISFCS,F10M,   &
               VEGFCS, VETFCS, SOTFCS, ALFFCS,            &
               CVFCS,CVBFCS,CVTFCS,0,NLUNIT)
# 2363
!-----------------------------------------------------------------------
! if an array is deallocated, the rest of code knows to interpolate
! that field from the input grid.
!
! always use tbot from surface cycle as it is imitmately tied to the
! output terrain.
!-----------------------------------------------------------------------

 allocate (substrate_temp_output_ext(ijmdl_output))
     *rate_temp_output_ext = tg3fcslimo_fields_opt

     * == 3) thenocate
     * (soil_type_output_ext(ijmdl_output))
     *l_type_output_ext = nint(sotfcs)
     *ocate (veg_type_output_ext(ijmdl_output))
     *_type_output_ext = nint(vetfcs)
     *ocate (slope_type_output_ext(ijmdl_output))
     *pe_type_output_ext = nint(slpfcs)
     *flimo_fields_opt

     * == 2 .or. climo_fields_opt == 3) thenocate
     * (mxsnow_alb_output_ext(ijmdl_output))
     *now_alb_output_ext = absfcsocate
     * (z0_output_ext(ijmdl_output))
     *output_ext = zorfcsocate
     * (greenfrc_output_ext(ijmdl_output))                         
     *enfrc_output_ext = vegfcsocate
     * (greenfrc_min_output_ext(ijmdl_output))                     
     *enfrc_min_output_ext = vmnfcsocate
     * (greenfrc_max_output_ext(ijmdl_output))                     
     *enfrc_max_output_ext = vmxfcsocate
     * (alnsf_output_ext(ijmdl_output))
     *sf_output_ext = albfcs(:,3)
     *ocate (alnwf_output_ext(ijmdl_output))
     *wf_output_ext = albfcs(:,4)
     *ocate (alvsf_output_ext(ijmdl_output))
     *sf_output_ext = albfcs(:,1)
     *ocate (alvwf_output_ext(ijmdl_output))
     *wf_output_ext = albfcs(:,2)
     *ocate (facsf_output_ext(ijmdl_output))
     *sf_output_ext = alffcs(:,1)
     *ocate (facwf_output_ext(ijmdl_output))
     *wf_output_ext = alffcs(:,2)
     *focate

     * (sig1t, slmask, orog, sihfcs, sicfcs, swdfcs, &
             slcfcs, vmnfcs, vmxfcs, slpfcs, absfcs, sitfcs,   &
             TSFFCS, SNOFCS, zorfcs, ALBFCS, tg3fcs,      &
             CNPFCS, SMCFCS, STCFCS, slifcs, AISFCS, F10M,&
             vegfcs, vetfcs, sotfcs, ALFFCS, CVFCS, CVBFCS, CVTFCS, &
             lats, lons)

     *nubroutine
     * get_ext_climo_globalutine
!-----------------------------------------------------------------------
     * get_ext_climo_nmm(imdl_output, jmdl_output, curr_hour, &
                              curr_mon, curr_day, curr_year, iret)
!$$$ subprogram documentation block
!
! subprogram: get_ext_climo_nmm    get climo fields on nmm grid
!   prgmmr: gayno           org: w/np2     date: 2006-04-14
!
! abstract: get climo fields, such as soil type and albedo on the
!           output grid by reading them from grib files.
!           user selects what fields to read in by setting the
!           namelist entries for the path/name of the grib file.
!           if a field is NOT read in from the grib file, it
!           will be interpolated from the input grid.
!
! program history log:
!  2006-04-14  gayno   - initial version
!
! usage: 'call get_ext_climo_global' with the following arguments
!   input arguments:
!     i/jmdl_output           number of grid points, output grid
!                             in the i/j direction
!     curr_hour/mon/day/year  current time (used to interpolate
!                                           monthly datasets)
!   outputs:
!     soil_type_output_ext        soil type
!     veg_type_output_ext         vegetation type
!     slope_type_output_ext       slope type
!     mxsnow_alb_output_ext       max snow albedo
!     z0_output_ext               roughness
!     greenfrc_output_ext         greenness fraction
!     snow_free_output_ext        snowfree albedo
!
! namelists:
!   optional_output_fields:  (path/name of climo grib files)
!     snow_free_albedo_output_file
!     greenfrc_output_file
!     mxsnow_alb_output_file
!     slope_type_output_file
!     soil_type_output_file
!     veg_type_output_file
!     z0_output_file
!
! input files:
!   unit 81    configuration namelists
!
! subprograms called:
!   read_grib_data  - read single grib field
!   degrib_climo    - degrib multiple time period grib file
!   time_interp     - time interpolate a field to current date
!   inventory       - inventory multiple time period grib file
!
! attributes:
!   langauge: fortran 90
!
!$$$

     *ead_write_utils, only     : read_grib_data,  &
                                  date, &
                                  degrib_climo, &
                                  time_interp, &
                                  inventorycit

     * noneist

     * /optional_output_fields/ snow_free_albedo_output_file,  &
                                   greenfrc_output_file, &
                                   mxsnow_alb_output_file, &
                                   slope_type_output_file,  &
                                   soil_type_output_file, &
                                   veg_type_output_file,  &
                                   z0_output_filecter

     **150                  :: soil_type_output_filecter
     **150                  :: veg_type_output_filecter
     **150                  :: slope_type_output_filecter
     **150                  :: mxsnow_alb_output_filecter
     **150                  :: greenfrc_output_filecter
     **150                  :: snow_free_albedo_output_filecter
     **150                  :: z0_output_fileer

     *, intent(in)            :: imdl_output, jmdl_output, &
                                   curr_hour, curr_day, &
                                   curr_year, curr_moner

     *                        :: iret, param_num, tot_num_recs, &
                                   curr_minute, ijmdl_output, istat

     * allocatable              :: dummy(:)
     * allocatable              :: data_climo(:,:,:)

 type (date), allocatable       :: dates(:)

 iret = 0

     **,"- READ CONFIGURATION NAMELIST."

     *81, iostat=istat, err=900)
     *81, nml=optional_output_fields, iostat=istat, err=910)
     *(81)

     *_output = imdl_output * jmdl_outputen_trim

!-----------------------------------------------------------------------
! if user chooses, read soil type, veg type, slope type and
! max snow albedo.
!-----------------------------------------------------------------------

     *(soil_type_output_file) > 0) thennt
     **,"- DEGRIB SOIL TYPE ON OUTPUT GRID"
     *ocate (dummy(ijmdl_output))
     *ocate (soil_type_output_ext(ijmdl_output))
     *l read_grib_data(soil_type_output_file, 224, dummy, &
                       ijmdl_output, iret)
   if (iret /= 0) thenrint
     **,'- ERROR DEGRIBBING OUTPUT GRID SOIL TYPE'
     *eturn
     * ifl_type_output_ext
     * = nint(dummy)
     *llocate (dummy)
     *fen_trim

     *(veg_type_output_file) > 0) thennt
     **,"- DEGRIB VEG TYPE ON OUTPUT GRID"
     *ocate (dummy(ijmdl_output))
     *ocate (veg_type_output_ext(ijmdl_output))
     *l read_grib_data(veg_type_output_file, 225, dummy, &
                       ijmdl_output, iret)
   if (iret /= 0) thenrint
     **,'- ERROR DEGRIBBING OUTPUT GRID VEG TYPE'
     *eturn
     * if_type_output_ext
     * = nint(dummy)
     *llocate (dummy)
     *fen_trim

     *(slope_type_output_file) > 0) thennt
     **,"- DEGRIB SLOPE TYPE ON OUTPUT GRID"
     *ocate (dummy(ijmdl_output))
     *ocate (slope_type_output_ext(ijmdl_output))
     *l read_grib_data(slope_type_output_file, 222, dummy, &
                       ijmdl_output, iret)
   if (iret /= 0) thenrint
     **,'- ERROR DEGRIBBING OUTPUT GRID SLOPE TYPE'
     *eturn
     * ifpe_type_output_ext
     * = nint(dummy)
     *llocate (dummy)
     *fen_trim

     *(mxsnow_alb_output_file) > 0) thennt
     **,"- DEGRIB MAX SNOW ALBEDO ON OUTPUT GRID"
     *ocate (mxsnow_alb_output_ext(ijmdl_output))
     *l read_grib_data(mxsnow_alb_output_file, 159,     &
                       mxsnow_alb_output_ext, ijmdl_output, iret)
   if (iret /= 0) thenrint
     **,'- ERROR DEGRIBBING OUTPUT GRID MAX SNOW ALBEDO'
     *eturn
     * ifnow_alb_output_ext
! grib standard is %, but model expects decimal.
     * = mxsnow_alb_output_ext * 0.01
     *fen_trim

!-----------------------------------------------------------------------
! greenness fraction.
!-----------------------------------------------------------------------

     *(greenfrc_output_file) > 0) thennt
     **,"- DEGRIB GREENNESS ON OUTPUT GRID"
     *am_num = 87
     *l inventory(greenfrc_output_file, param_num, tot_num_recs, iret, 0
   if (iret /= 0) thenrint
     **,'- ERROR DEGRIBBING OUTPUT GRID GREENNESS FRACTION.'
     *eturn
     * ifocate
     * (dates(tot_num_recs))
     *ocate (data_climo(imdl_output,jmdl_output,tot_num_recs))
     *l degrib_climo(data_climo, dates, ijmdl_output,           &
                     param_num, greenfrc_output_file,           &
                     tot_num_recs, iret, 0)
   if (iret /= 0) thenrint
     **,'- ERROR DEGRIBBING OUTPUT GRID GREENNESS FRACTION.'
     *eturn
     * ifocate
     * (greenfrc_output_ext(ijmdl_output))
     *enfrc_output_ext = 0.0
!  need to adjust this for fcst hour????
     *r_minute = 0
     *l time_interp(data_climo, dates, tot_num_recs,                &
                    ijmdl_output, curr_year, curr_mon, curr_day,    &
                    curr_hour, curr_minute, greenfrc_output_ext)
!  grib standard is %, but model expects decimal.
     *enfrc_output_ext = greenfrc_output_ext * 0.01
     *llocate (dates)
     *llocate (data_climo)
     *fen_trim

!-----------------------------------------------------------------------
! albedo (snow free)
!-----------------------------------------------------------------------

     *(snow_free_albedo_output_file) > 0) thennt
     **,"- DEGRIB SNOW FREE ALBEDO ON OUTPUT GRID"
     *am_num = 170
     *l inventory(snow_free_albedo_output_file, param_num,  &
                  tot_num_recs, iret, 0)
   if (iret /= 0) thenrint
     **,'- ERROR DEGRIBBING OUTPUT GRID SNOW-FREE ALBEDO'
     *eturn
     * ifocate
     * (dates(tot_num_recs))
     *ocate (data_climo(imdl_output,jmdl_output,tot_num_recs))
     *l degrib_climo(data_climo, dates, ijmdl_output,           &
                     param_num, snow_free_albedo_output_file,    &
                     tot_num_recs, iret, 0)
   if (iret /= 0) thenrint
     **,'- ERROR DEGRIBBING OUTPUT GRID SNOW-FREE ALBEDO'
     *eturn
     * ifocate
     * (snow_free_albedo_output_ext(ijmdl_output))
     *w_free_albedo_output_ext = 0.0
!  need to adjust this for fcst hour????
     *r_minute = 0
     *l time_interp(data_climo, dates, tot_num_recs,                &
                    ijmdl_output, curr_year, curr_mon, curr_day,    &
                    curr_hour, curr_minute, snow_free_albedo_output_ext)
!  grib standard is %, but model uses decimal.
     *w_free_albedo_output_ext = snow_free_albedo_output_ext * 0.01
     *llocate (dates)
     *llocate (data_climo)
     *fen_trim

!-----------------------------------------------------------------------
! roughness length.
!-----------------------------------------------------------------------

     *(z0_output_file) > 0) thennt
     **,"- DEGRIB ROUGHNESS ON OUTPUT GRID"
     *am_num = 83
     *l inventory(z0_output_file, param_num, tot_num_recs, iret, 0)
   if (iret /= 0) thenrint
     **,'- ERROR DEGRIBBING OUTPUT GRID ROUGHNESS LENGTH'
     *eturn
     * ifocate
     * (dates(tot_num_recs))
     *ocate (data_climo(imdl_output,jmdl_output,tot_num_recs))
     *l degrib_climo(data_climo, dates, ijmdl_output,      &
                     param_num, z0_output_file, tot_num_recs, iret, 0)
   if (iret /= 0) thenrint
     **,'- ERROR DEGRIBBING OUTPUT GRID ROUGHNESS LENGTH'
     *eturn
     * ifocate
     * (z0_output_ext(ijmdl_output))
     *output_ext = -1.0
!nmm does not use a time varying field.  no need to time interp.
   if (tot_num_recs > 1) thenurr_minute
     * = 0
     *all time_interp(data_climo, dates, tot_num_recs, &
                      ijmdl_output, curr_year, curr_mon, curr_day, &
                      curr_hour, curr_minute, z0_output_ext)
     *e0_output_ext
     * = reshape(data_climo(:,:,1),(/ijmdl_output/))
     * ifoutput_ext
!-----------------------------------------------------------------------
! interp code expects z0 in cm, raw data in meters.
!-----------------------------------------------------------------------
     * = z0_output_ext * 100.0
     *llocate (data_climo)
     *llocate (dates)
     *fnint

# 2689

     **,"- ERROR OPENING CONFIG NAMELIST. ISTAT IS ", istatet
     * = 11
     *turnint

     **,"- ERROR READING CONFIG NAMELIST. ISTAT IS ", istatet
     * = 12
     *turnubroutine

     * get_ext_climo_nmmutine

!-----------------------------------------------------------------------
     * surface_chgres_ax2d(dum)
!$$$ subprogram documentation block
!
! subprogram: surface_chgres_ax2d    free up memory
!   prgmmr: gayno           org: w/np2     date: 2005-10-19
!
! abstract: deallocate a sfc2d data structure
!
! program history log:
!  2005-10-19  gayno   - initial version
!
! usage: call surface_chgres_ax2d(dum)
!   input arguments:
!     dum            data structure containing several surface fields
!
! attributes:
!   langauge: fortran 90
!
!$$$
 type (sfc2d), intent(inout)   :: dumllocated

     * (dum%alnsf))           deallocate (dum%alnsf)
     *llocated (dum%alnwf))           deallocate (dum%alnwf)
     *llocated (dum%alvsf))           deallocate (dum%alvsf)
     *llocated (dum%alvwf))           deallocate (dum%alvwf)
     *llocated (dum%canopy_mc))       deallocate (dum%canopy_mc)
     *llocated (dum%facsf))           deallocate (dum%facsf)
     *llocated (dum%facwf))           deallocate (dum%facwf)
     *llocated (dum%sea_ice_fract))   deallocate (dum%sea_ice_fract)
     *llocated (dum%greenfrc))        deallocate (dum%greenfrc)
     *llocated (dum%greenfrc_max))    deallocate (dum%greenfrc_max)
     *llocated (dum%greenfrc_min))    deallocate (dum%greenfrc_min)
     *llocated (dum%sea_ice_depth))   deallocate (dum%sea_ice_depth)
     *llocated (dum%sea_ice_temp))    deallocate (dum%sea_ice_temp)
     *llocated (dum%lsmask))          deallocate (dum%lsmask)
     *llocated (dum%mxsnow_alb))      deallocate (dum%mxsnow_alb)
     *llocated (dum%orog))            deallocate (dum%orog)
     *llocated (dum%skin_temp))       deallocate (dum%skin_temp)
     *llocated (dum%snow_depth))      deallocate (dum%snow_depth)
     *llocated (dum%snow_liq_equiv))  deallocate (dum%snow_liq_equiv)
     *llocated (dum%soilm_liq))       deallocate (dum%soilm_liq)
     *llocated (dum%soilm_tot))       deallocate (dum%soilm_tot)
     *llocated (dum%soil_temp))       deallocate (dum%soil_temp)
     *llocated (dum%z0))              deallocate (dum%z0)
     *llocated (dum%sea_ice_flag))    deallocate (dum%sea_ice_flag)
     *llocated (dum%slope_type))      deallocate (dum%slope_type)
     *llocated (dum%soil_type))       deallocate (dum%soil_type)
     *llocated (dum%veg_type))        deallocate (dum%veg_type)

     *ubroutine surface_chgres_ax2dutine
!-----------------------------------------------------------------------
     * surface_chgres_ax1d(dum)
!$$$ subprogram documentation block
!
! subprogram: surface_chgres_ax1d    free up memory
!   prgmmr: gayno           org: w/np2     date: 2005-10-19
!
! abstract: deallocate a sfc1d data structure
!
! program history log:
!  2005-10-19  gayno   - initial version
!
! usage: call surface_chgres_ax1d(dum)
!   input arguments:
!     dum            data structure containing several surface fields
!
! attributes:
!   langauge: fortran 90
!
!$$$
 type (sfc1d), intent(inout)   :: dumllocated

     * (dum%alnsf))           deallocate (dum%alnsf)
     *llocated (dum%alnwf))           deallocate (dum%alnwf)
     *llocated (dum%alvsf))           deallocate (dum%alvsf)
     *llocated (dum%alvwf))           deallocate (dum%alvwf)
     *llocated (dum%canopy_mc))       deallocate (dum%canopy_mc)
     *llocated (dum%facsf))           deallocate (dum%facsf)
     *llocated (dum%facwf))           deallocate (dum%facwf)
     *llocated (dum%sea_ice_fract))   deallocate (dum%sea_ice_fract)
     *llocated (dum%greenfrc))        deallocate (dum%greenfrc)
     *llocated (dum%greenfrc_max))    deallocate (dum%greenfrc_max)
     *llocated (dum%greenfrc_min))    deallocate (dum%greenfrc_min)
     *llocated (dum%sea_ice_depth))   deallocate (dum%sea_ice_depth)
     *llocated (dum%sea_ice_temp))    deallocate (dum%sea_ice_temp)
     *llocated (dum%lsmask))          deallocate (dum%lsmask)
     *llocated (dum%mxsnow_alb))      deallocate (dum%mxsnow_alb)
     *llocated (dum%orog))            deallocate (dum%orog)
     *llocated (dum%skin_temp))       deallocate (dum%skin_temp)
     *llocated (dum%snow_depth))      deallocate (dum%snow_depth)
     *llocated (dum%snow_liq_equiv))  deallocate (dum%snow_liq_equiv)
     *llocated (dum%soilm_liq))       deallocate (dum%soilm_liq)
     *llocated (dum%soilm_tot))       deallocate (dum%soilm_tot)
     *llocated (dum%soil_temp))       deallocate (dum%soil_temp)
     *llocated (dum%z0))              deallocate (dum%z0)
     *llocated (dum%sea_ice_flag))    deallocate (dum%sea_ice_flag)
     *llocated (dum%slope_type))      deallocate (dum%slope_type)
     *llocated (dum%soil_type))       deallocate (dum%soil_type)
     *llocated (dum%veg_type))        deallocate (dum%veg_type)
     *llocated (dum%lats))            deallocate (dum%lats)
     *llocated (dum%lons))            deallocate (dum%lons)

     *ubroutine surface_chgres_ax1dodule

     * surface_chgres
