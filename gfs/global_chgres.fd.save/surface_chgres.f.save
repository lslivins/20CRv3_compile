...
!-----------------------------------------------------------------------
 ...
 elseif(kgds_input(1) == 203) then
   ...
   dx_input = -(float(kgds_input(9)) * 0.001)
   dy_input = float(kgds_input(10)) * 0.001
...
!-----------------------------------------------------------------------
 ...
 if (maxval(input%sea_ice_fract) < -999.) then  !input grid is pre-seaice model
...
! at open water points, don't let sst go below freezing point.
 ...
 do ij = 1, ijmdl_output
   if (output%lsmask(ij) == 0.0) then
   if (output%sea_ice_flag(ij) == 1) then
     output%skin_temp(ij) = min(output%skin_temp(ij),frz_ice)
   else
...
! when the budget interpolation is used.  make sure depth is consistent.
     output%snow_depth= output%snow_liq_equiv*10.0 ! non-land
     do ij = 1, count_land_output
       if (bitmap_land_output(ij)) then
...
!-----------------------------------------------------------------------
 ...
 deallocate (bitmap_sea_ice_output, bitmap_sea_ice_input)
 deallocate (ijsav_land_output, ijsav_nonland_output, ijsav_sea_ice_output)
 deallocate (lats_land_output, lons_land_output)
 deallocate (lats_nonland_output, lons_nonland_output)
 deallocate (lats_sea_ice_output, lons_sea_ice_output)
 deallocate (nn_iindx_wrt_input_grid, nn_jindx_wrt_input_grid)
 deallocate (output_data_land, output_data_nonland, output_data_sea_ice)
