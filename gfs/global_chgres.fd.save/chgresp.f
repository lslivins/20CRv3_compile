...
C
C ABSTRACT: THIS PROGRAM CHANGES THE RESOLUTION OF THE SIGMA AND SURFACE
C   RESTART FILES FROM THE GLOBAL SPECTRAL MODEL.  THE INPUT FILES
...
C$$$
     ...
     &                 LSOIL,IVSSFC,IVSSIG,NVCOORD,OUTTYP,IDRT
      INTEGER NSIGI,NORO,NSIL,NO3C,NO3T,NSIGO
      INTEGER IRET,IOSORO,IOSSIL,ITER,IRET1
       integer ipi,ipj,ipk
      INTEGER NCI,NCO,IMI,JMI,IMO,JMO,IJX,NTRACM,J1,J2,JL,IJL,J,JN,JS
     ...
     &                   TO(:,:,:),UO(:,:,:),VO(:,:,:),QO(:,:,:,:)
      ...
      INTEGER NSFCI,NSLM,NLPL,NSFCO,INPTYP
...
C Define variables for GFSIO
      ...
      INTEGER :: NREC, LEVSI, LEVSO, JREC, I, N
      CHARACTER(8) :: FILETYPE                 
      character(len=120) inputfile,stdoutfile,outputfile
      integer ierr,nproc,numproc
! mpi definitions.
      include 'mpif.h'
...
!  READ NAMELIST
      ...
      CALL GFUNCPHYS
!     READ(*,NAMCHG)
      write(inputfile,"('global_chgres.nml.mem',i3.3)") nproc+1  
      open(912,file=inputfile,form='formatted')
      read(912,namchg,iostat=ierr)
      close(912)
...
!  OPEN  FILES
      ...
      write(inputfile,"('chgres.inp.sig.',i3.3)") nproc+1  
      write(outputfile,"('chgres.out.sig.',i3.3)") nproc+1  
      CALL SIGIO_SROPEN(NSIGI,trim(inputfile),IRET)
      CALL SIGIO_SRHEAD(NSIGI,SIGHEADI,IRET1)
      IF(IRET.EQ.0 .AND. IRET1.EQ.0) THEN
        INPTYP=2
      ELSE
        CALL GFSIO_OPEN(GFILEI,trim(inputfile),'read',IRET=IRET)
     ...
     &       FORM='FORMATTED',STATUS='OLD',IOSTAT=IRET)
        ...
        IF(IRET.NE.0) NO3T=0
        CALL SIGIO_SWOPEN(NSIGO,trim(outputfile),IRET)
      ELSE
...
!         ----------------
          ...
          IF(IDVC.GT.0) SIGHEADO%IDVC=IDVC
          IF(IDVM.GT.0) SIGHEADO%IDVM=IDVM
          IF(IDSL.GT.0) SIGHEADO%IDSL=IDSL
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
!  READ OLD SIGMA FILE
        CALL SIGIO_ALDATA(SIGHEADI,SIGDATAI,IRET)
        IF(IRET.NE.0) THEN
        ...
        IF(IRET.NE.0) THEN
          ...
          CALL ERREXIT(4)
        ENDIF
        CALL SIGIO_SRDATA(NSIGI,SIGHEADI,SIGDATAI,IRET)
        IF(IRET.NE.0) THEN
          PRINT '("  ERROR READING ")'
          CALL ERREXIT(4)
        ENDIF
...
!  ALLOCATE DATA FOR GFSIO OUTPUT
          IF(OUTTYP.EQ.1 .OR. OUTTYP.EQ.0) THEN
            ...
            ALLOCATE(GFSDATAO%Q(IMO,JMO,LEVSO,NTRACM))
          ENDIF
...
!  ALLOCATE TEMPORARY SIGIO DATA
          ...
          ALLOCATE(QO(IMO,2*LATCH,LEVSO,NTRACM))
          ALLOCATE(TPO(IMO,2*LATCH,LEVSO))
          ALLOCATE(DPDTO(IMO,2*LATCH,LEVSO))
...
!  COMPUTE NEW SURFACE PRESSURE
     ...
     &        SIGHEADI%IDVC,SIGHEADI%IDSL,SIGHEADI%VCOORD,IRET,
     &        PS=PSI,T=TIV,PM=PI)
...
! -- Henry Juang's approach
     ...
     &                 SIGHEADO%IDVC,SIGHEADO%IDSL,
     &                 SIGHEADO%NVCOORD,SIGHEADO%VCOORD,
     &                 PI,TI,QI,PSO,PO,DPO)
         call instrument(7,kall,ttot,tmin,tmax)
            CALL VINTG(IJL,IJX,SIGHEADI%LEVS,SIGHEADO%LEVS,NTRACM,
     &                 PI,UI,VI,TI,QI,PO,UO,VO,TO,QO,DTDPO)
...
!             print*,' transform back to the new spectral space '
              CALL TRBSC(SIGHEADO%JCAP,NCO,SIGHEADO%LEVS,NTRACM,
     &                 SIGHEADO%IDVM,IDRT,IMO,JMO,IJX,J1,J2,1,
     &                 ZSO,PSO,TO,UO,VO,QO,
...
!         print*, ' prepare gfsio header '  
       ...
       ! --- fill in GFSIO header data  ---
          NREC=2+SIGHEADO%LEVS*(5+NTRACM)     !zs,ps,p,dp,t,u,v,q(ntracer)
          LEVSO=SIGHEADO%LEVS
...
!         print*, ' write out gfsio chgres.out.grd'
          write(outputfile,"('chgres.out.grd.',i3.3)") nproc+1  
          CALL GFSIO_OPEN(GFILEO,TRIM(outputfile),'write'
     ...
     &           GFSDATAO%ZS,IRET=IRET,IDRT=GFSHEADO%IDRT)
            IF(IRET.NE.0) PRINT*, ' ERROR GFSIO_WRITEREC ZS '
          JREC=JREC+1
            CALL GFSIO_WRITERECW34(GFILEO,JREC,
     &           GFSDATAO%PS,IRET=IRET,IDRT=GFSHEADO%IDRT)
            IF(IRET.NE.0) PRINT*, ' ERROR GFSIO_WRITEREC PS '
          DO K=1,LEVSO
          JREC=JREC+1
            CALL GFSIO_WRITERECW34(GFILEO,JREC,
     &           GFSDATAO%P(:,:,K),IRET=IRET,IDRT=GFSHEADO%IDRT)
            IF(IRET.NE.0) PRINT*, ' ERROR GFSIO_WRITEREC P '
          ENDDO
          DO K=1,LEVSO
          JREC=JREC+1
            CALL GFSIO_WRITERECW34(GFILEO,JREC,
     &           GFSDATAO%DP(:,:,K),IRET=IRET,IDRT=GFSHEADO%IDRT)
            IF(IRET.NE.0) PRINT*, ' ERROR GFSIO_WRITEREC DP '
          ENDDO
          DO K=1,LEVSO
          JREC=JREC+1
            CALL GFSIO_WRITERECW34(GFILEO,JREC,
     &           GFSDATAO%T(:,:,K),IRET=IRET,IDRT=GFSHEADO%IDRT)
            IF(IRET.NE.0) PRINT*, ' ERROR GFSIO_WRITEREC T '
          ENDDO
          DO K=1,LEVSO
          JREC=JREC+1
            CALL GFSIO_WRITERECW34(GFILEO,JREC,
     &           GFSDATAO%U(:,:,K),IRET=IRET,IDRT=GFSHEADO%IDRT)
            IF(IRET.NE.0) PRINT*, ' ERROR GFSIO_WRITEREC U '
          ENDDO
          DO K=1,LEVSO
          JREC=JREC+1
            CALL GFSIO_WRITERECW34(GFILEO,JREC,
     &           GFSDATAO%V(:,:,K),IRET=IRET,IDRT=GFSHEADO%IDRT)
...
!!        DO N=1,GFSHEADO%NTRAC
          ...
          JREC=JREC+1
            CALL GFSIO_WRITERECW34(GFILEO,JREC,
     &           GFSDATAO%Q(:,:,K,N),IRET=IRET,IDRT=GFSHEADO%IDRT)
...
!  UPDATE GFSIO OUTPUT FILE SECOND HEADER
        NREC=2+GFSHEADO%LEVS*(5+NTRACM)     !zs,ps,p,dp,t,u,v,q(ntracer)
        GFSHEADO%NREC=NREC               
        ALLOCATE(GFSHEADVO%VCOORD(LEVSO+1,SIGHEADO%NVCOORD))
        ALLOCATE(GFSHEADVO%RECNAME(NREC))
        ALLOCATE(GFSHEADVO%RECLEVTYP(NREC))
...
!  ALLOCATE TEMPORARY DATA
          ...
          ALLOCATE(QO(IMO,JMO,LEVSO,NTRACM))
          ALLOCATE(TPO(IMO,JMO,LEVSO))
          ALLOCATE(DPDTO(IMO,JMO,LEVSO))
...
!  GET NEW OROGRAPHY
     ...
     &                 KF,K,KPDS,KGDS,LB,OROGO,IOSORO)
          ELSE
            IOSORO=9
          ENDIF
          IF(IOSORO.EQ.0.AND.(KGDS(1).NE.4.OR.
     &     KGDS(2).NE.IMO.OR.KGDS(3).NE.JMO)) IOSORO=100
          IF(IOSORO.EQ.0) THEN
            PRINT '("  NEW OROGRAPHY READ IN")'
          ELSE
...
!  GET NEW SIGMA LEVELS
     ...
     &                  GFSHEADO%NVCOORD,GFSHEADVO%VCOORD,IOSSIL)
            IF(IOSSIL.EQ.0) THEN
              PRINT '("  NEW MODEL LEVELS READ IN")'
            ENDIF
...
!  COMPUTE NEW SURFACE PRESSURE
     ...
     &        GFSHEADI%IDVC,GFSHEADI%IDSL,GFSHEADVI%VCOORD,IRET,
     &        PS=PSI,T=TIV,PM=PI)
...
! -- Henry Juang's approach
     ...
     &                 GFSHEADO%IDVC,GFSHEADO%IDSL,
     &                 GFSHEADO%NVCOORD,GFSHEADVO%VCOORD,
     &                 PI,TI,QI,PSO,PO,DPO)
         call instrument(9,kall,ttot,tmin,tmax)
            CALL VINTG(IJL,IJX,GFSHEADI%LEVS,GFSHEADO%LEVS,NTRACM,
     &                 PI,UI,VI,TI,QI,PO,UO,VO,TO,QO,DTDPO)
            if( GFSHEADO%IDVC .eq. 3 ) then
              call checkdp(IJL,IJX,GFSHEADO%LEVS,
     &                 GFSHEADVO%VCOORD(1:(GFSHEADO%LEVS+1),1),
     &                 GFSHEADVO%VCOORD(1:(GFSHEADO%LEVS+1),2),
     &                 GFSHEADVO%VCOORD(1:(GFSHEADO%LEVS+1),3),
     &                 PSO,TO,QO)
            endif
         call instrument(10,kall,ttot,tmin,tmax)
! -- end of Henry Juang's approach
...
C  OPEN SURFACE FILES
      ...
      NLPL=23
      NSFCO=61
      print *,'do surface file',nsfco
      write(inputfile,"('chgres.inp.sfc.',i3.3)") nproc+1  
      write(outputfile,"('chgres.out.sfc.',i3.3)") nproc+1  
      CALL SFCIO_SROPEN(NSFCI,trim(inputfile),IRET)
      IF(IRET.EQ.0) THEN
     ...
     &       FORM='FORMATTED',STATUS='OLD',IOSTAT=IRET)
        IF(IRET.NE.0) NLPL=0
        CALL SFCIO_SWOPEN(NSFCO,trim(outputfile),IRET)
...
C  READ OLD SURFACE FILE
        CALL SFCIO_ALDATA(SFCHEADI,SFCDATAI,IRET)
        IF(IRET.NE.0) THEN
          PRINT '("  ERROR ALLOCATING ")'
          CALL ERREXIT(4)
        ENDIF
        CALL SFCIO_ALDATA(SFCHEADO,SFCDATAO,IRET)
        IF(IRET.NE.0) THEN
          PRINT '("  ERROR ALLOCATING ")'
          CALL ERREXIT(4)
        ENDIF
        CALL SFCIO_SRDATA(NSFCI,SFCHEADI,SFCDATAI,IRET)
        IF(IRET.NE.0) THEN
          PRINT '("  ERROR READING ")'
          CALL ERREXIT(4)
        ENDIF
...
!       ALLOCATE(LB(IMO,JMO))
     ...
     &               KF,K,KPDS,KGDS,LB,SLMSKO,IOSLM)
          IF(IOSLM.EQ.0.AND.(KGDS(1).NE.4.OR.
...
C  CALL SURFACE CHGRES DRIVER.
     ...
     &                           SFCHEADI%IDATE(4), SFCHEADI%FHOUR,
     &                           KGDS_INPUT, SFCINPUT, IRET)
      IF (IRET .NE. 0) THEN
...
C  WRITE THE NEW SURFACE FILE
        ...
        CALL SFCIO_SWDATA(NSFCO,SFCHEADO,SFCDATAO,IRET)
        CALL SFCIO_AXDATA(SFCDATAO,IRET)
...
C$$$
      ...
      REAL C1(IM+1,KM1,3+NT),C2(IM+1,KM2,3+NT),J2(IM+1,KM2,3+NT)
C - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
!$$$
      subroutine specsets(h,d,idrt)
        ...
        type(sigio_data),intent(inout):: d
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        call sigio_modpr(h%lonb*h%latb,h%lonb*h%latb,h%levs,h%nvcoord,
     &                   h%idvc,h%idsl,h%vcoord,iret,
...
C-----------------------------------------------------------------------
C-----------------------------------------------------------------------
      SUBROUTINE NEWPR1(IM,IX,KM,KMP,IDVC,IDSL,NVCOORD,VCOORD,
     &                 PP,TP,QP,PS,PM,DP)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C
...
C$$$
      ...
      REAL DP(IX,KM)
      REAL PP(IX,KMP),TP(IX,KMP),QP(IX,KMP),THETAP(KMP)
      REAL PO(KMP),TO(KMP),QO(KMP)
      REAL PN(KM ),TN(KM ),QN(KM )
      REAL TOV(KM),TRK,PIO(KM),PISAVE(IX,KM+1)
      FTV(AT,AQ)=AT*(1+FV*AQ)
C - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
! first guess : assume KMP=KM
        ...
        DO I=1,IM
          ...
          ENDDO
! iteration
         DO N=1,400	! default number of iterations
          CONVERG=0.0
          DPMIN=1000.0
          DO K=1,KM+1
            PIO(K)=PI(I,K)
...
! do interpolation by the intrinsic method to get TN and QN
          CALL VINTTQ(KMP,KM,PO,TO,QO,PN,TN,QN)
          DO K=2,KM
            TVU=FTV(TN(K  ),QN(K  ))
            TVD=FTV(TN(K-1),QN(K-1))
            TRK = (TVD+TVU)/(TOV(K-1)+TOV(K))
            TRK = TRK ** ROCPR
            PI(I,K)=AK(K)+BK(K)*PS(I)+CK(K)*TRK
            CONVERG=MAX(CONVERG,ABS(PI(I,K)-PIO(K))/(PI(I,K)+PIO(K)))
         ...
         ENDDO
 100     CONTINUE
!        PRINT *,'I=',I,' CONVERGED AT',N,' ITERATIONS',' DPMIN=',DPMIN
