...
! spectral space.
  ...
  if (me .eq. me_l_0) print*,'iauinc=',iauinc
   ...
   do n=1,nfiles
      if (.not. iauinc) filename = iaufiles_fg(n)
      if (ens_mem > 0) then
        write(charmem,'(i3.3)') ens_mem
        if (.not. iauinc) filename = trim(filename) // charmem
      endif
      if (.not. iauinc) then
...
!!$omp end workshare
   ...
   enddo
   deallocate(topospectmp_o)
   deallocate(topospectmp_e)
   if (.not. iauinc) then
      deallocate(vrtspectmp_e)
      deallocate(divspectmp_e)
      deallocate(virtempspectmp_e)
      deallocate(tracerspectmp_e)
      deallocate(lnpsspectmp_e)
   endif
...
! and dividing by length of iau window (in seconds).
   ...
   if ( nfiles > 1) then  ! IAU forcing files bookend interval
      if (t <= iaufhrs(1)*3600. .or. t > iaufhrs(nfiles)*3600.) then
         ...
         if (t > iaufhrs(nfiles)*3600.) iau=.false.
         return
      endif
