...
subroutine read_pattern(rpattern,pattern2d_e,pattern2d_o,lunptn)
   ...
   if (me .EQ. me_l_0) then
      ...
      read(lunptn) rpattern%seed
      allocate(pattern2din((jcap+1)*(jcap+2)))
      read(lunptn) pattern2din
       print*,'reading in random pattern (min/max/size/seed)',minval(pattern2din),maxval(pattern2din),size(pattern2din),rpattern%seed
    ...
    call mpi_bcast(rpattern%seed,1,mpi_integer,me_l_0,mc_comp,ierr)  ! blast out seed 
