...
!$$$
      ...
      integer,parameter :: MAXLEN=200,MAXTEMP=38
     ...
     &   1,1,1,1,1,4,1,1,1,4,1,4/
      ...
      data templates(34)%template_num /48/     !  Analysis or forecast at a horizontal level or 
      data templates(34)%mappdslen /26/        !  in a  horizontal layer at a point in time for
      data templates(34)%needext /.false./     !  Optical Properties of Aerosol
      data (templates(34)%mappds(j),j=1,26)    
     &  /1,1,2,1,-1,-4,-1,-4,1,-1,-4,-1,-4,1,1,1,2,1,1,4,1,-1,-4,
     &   1,-1,-4/

      data templates(35)%template_num /50/     !  Analysis or Forecast of a multi component
      data templates(35)%mappdslen /21/        !  parameter or matrix element at a point in time
      data templates(35)%needext /.false./     ! 
      data (templates(35)%mappds(j),j=1,21)
     &  /1,1,1,1,1,2,1,1,4,1,-1,-4,1,-1,-4,1,1,4,4,4,4/

      data templates(36)%template_num /51/     !  Categorical forecasts at a horizontal layer
      data templates(36)%mappdslen /16/        !  at a point in time
      data templates(36)%needext /.true./
      data (templates(36)%mappds(j),j=1,16)
     &  /1,1,1,1,1,2,1,1,4,1,-1,-4,1,-1,-4,1/

      data templates(37)%template_num /91/     !  Categorical forecasts at a horizontal level or
      data templates(37)%mappdslen /16/        !  in a horizontal layer in a continuous or
      data templates(37)%needext /.true./      !  non-continuous time interval
      data (templates(37)%mappds(j),j=1,16)
     &  /1,1,1,1,1,2,1,1,4,1,-1,-4,1,-1,-4,1/

      data templates(38)%template_num /32/     !  Analysis or forecast at a horizontal level or
      data templates(38)%mappdslen /10/        !  in a  horizontal layer at a point in time for
      data templates(38)%needext /.true./      !  for simulate (synthetic) Satellite data
      data (templates(38)%mappds(j),j=1,10)
     &  /1,1,1,1,1,2,1,1,2,1/
...
!$$$
           ...
           elseif ( number.eq.91 ) then
              do j=1,list(16)
                ...
                map(nummap+13)=1
                nummap=nummap+13
              if (list(nummap).gt.1 ) then
                do m=2,list(nummap)
                  nummap=nummap+1
                  do k=1,6
                    map(nummap+k)=map(nummap+k)
                  enddo
                  nummap=nummap+6
                enddo
              endif
