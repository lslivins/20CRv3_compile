...
! 2012-11-09  MOORTHI  Moved SPTRANF0 to inside OPENMP loop
C
...
C  TRANSFORM WAVE TO GRID
      IF(IDIR.GT.0) THEN
!$OMP PARALLEL DO PRIVATE(KWS,W,WTOP,G,IJKN,IJKS,i,j,k)
!$omp+private(EPS,EPSTOP,ENN1,ELONN1,EON,EONTOPP)
...
C  TRANSFORM GRID TO WAVE
      ELSE
!$OMP PARALLEL DO PRIVATE(KWS,W,WTOP,G,IJKN,IJKS,WINC,i,j,k)
!$omp+private(EPS,EPSTOP,ENN1,ELONN1,EON,EONTOPP)
!$omp+private(AFFT,CLAT,SLAT,WLAT,PLN,PLNTOP)
        DO K=1,KMAX
          CALL SPTRANF0(IROMB,MAXWV,IDRT,IMAX,JMAX,JB,JE,
