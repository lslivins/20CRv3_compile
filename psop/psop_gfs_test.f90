program psop
 ...
 character(64),allocatable,dimension(:):: cdiagbuf
 character (len=1) :: obtype
 character(len=19) :: statid
 character(len=30) :: statname
 character(len=13) :: obid
 character(len=64), allocatable :: statinfo(:)
...
!==> allocate some arrays for obs and obs metadata.
 ...
 biasob = 0.
 do nob=1,nobstot
      read(149,9801) statid,stattype(nob),obtype,oblocx(nob),oblocy(nob),&
           izob,obtime(nob),ob(nob),slpob,bias,stdevorig(nob),statname,obid
      statinfo(nob)=statid//' '//statname//' '//obid
      !print *,'statid,statname,obid ',statid(nob),' ',statname(nob),' ',obid(nob)
 ...
 call MPI_Bcast(biasob,nobstot,MPI_REAL,0, &
               MPI_COMM_WORLD,MPI_Status,ierr)
 call MPI_Bcast(statinfo,64*nobstot,MPI_CHARACTER,0, &
               MPI_COMM_WORLD,MPI_Status,ierr)
...
! now push back out to all other procs.
 ...
 do ii=1,nobstot
    ...
    cdiagbuf(ii)    = statinfo(ii)  ! station id
 ...
 if (nanal .eq. nanals+1) then
    ...
    do nob=1,nobstot
       ...
       write(9,9802) statinfo(nob),stattype(nob),rad2deg*oblocx(nob),rad2deg*oblocy(nob),&
               nint(zob(nob)),nint(anal_obz(nob)),obtime(nob),ob(nob),&
               anal_ob2(nanal,nob),stdevorig(nob),stdev(nob),iuseob(nob)
    enddo
    9802 format(a64,1x,i3,1x,f7.2,1x,f6.2,1x,i5,1x,i5,1x,f6.2,1x,f7.1,1x,&
