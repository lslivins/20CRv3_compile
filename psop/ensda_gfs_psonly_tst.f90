...
! if fit to nearby obs harmed by assimilating each ob).
...
! Jeff Whitaker <jeffrey.s.whitaker@noaa.gov> 20061221
...
integer idatein(4),idateout(4),nindx((nlons+1)*(nlats+2)),iunit,iunit_tmp,&
 ...
 nobmin,nobsps_sh,nobsps_nh,ierr,n,numproc,izob,nobsh,nobsuse,&
...
real fsg(nlevs),hsg(nlevs+1),covinflatenh,covinflatesh,lnsigcutoffnh,&
 ...
 corrlengthsh,corrlengthtr,lnsigl,errcheck_min,errcheck_max,&
 ...
 sumps_tr,biasps_tr,r,cf,pfht,height,gainfact,corrl,pfht_ob,&
 ...
 sumps_oberr_sh,sumps_oberr_tr,rlat,rlon,fhour,corr_fn,dist,betai,f,df,prob
...
real, dimension((nlons+1)*(nlats+2)) ::  hcovlocal,lons,lats,hdep
...
character (len=120) :: datapath,filename,obsfile,obsfileout,cmd
character (len=10) :: datestring,fgprefix
character (len=3) :: charnanal,charnav
character (len=1) :: c1 
character (len=8), dimension(4) :: lab
logical useheight, adaptive_local, postinflate, use_standlapse
...
namelist /nam_ensrf/datestring,datapath,obsfile,corrlengthnh,&
                    ...
                    use_standlapse,sprd_tol,errcheck_min,errcheck_max,&
...
sprd_tol = 9.9e31
! if y-Hx < errcheck_min, or y-Hx > errcheck_max, don't use that ob.
! (note asymmetry, reflects longer tail of climo distribution on
!  low pressure side). If |errcheck_min| > 1.e-5 and |errcheck_max| > 1.e-5,
! this check will be used, and sprd_tol will be ignored.
errcheck_min=0.
errcheck_max=0.
...
!if (mod(ndim-1,numproc-1) .ne. 0 .and. ndim .ne. numproc) then
!  print *,' illegal numproc....'
!  print *,' ninc,ndim-1,numproc-1 = ',ninc,ndim-1,numproc
!  print *,' ndim-1 should be a multiple of numproc-1!'
...
do i=1,nlons+1
   ij = (j-1)*(nlons+1) + i
   if (deglat .gt. 25.) then
     covinf = covinflatenh
     lnsigl = lnsigcutoffnh
   else if (deglat .le. 25 .and. deglat .ge. 15.) then
     covinf = ((25.-deglat)/10.)*covinflatetr + ((deglat-15.)/10.)*covinflatenh
     lnsigl = ((25.-deglat)/10.)*lnsigcutofftr + ((deglat-15.)/10.)*lnsigcutoffnh
   else if (deglat .lt. 15 .and. deglat .gt. -15.) then
     covinf = covinflatetr
     lnsigl = lnsigcutofftr
   else if (deglat .le. -15. .and. deglat .ge. -25.) then
     covinf = ((25.+deglat)/10.)*covinflatetr + ((-deglat-15.)/10.)*covinflatesh
     lnsigl = ((25.+deglat)/10.)*lnsigcutofftr + ((-deglat-15.)/10.)*lnsigcutoffsh
   else
     covinf = covinflatesh
     lnsigl = lnsigcutoffsh
   end if
   covinflate(ij,nn) = (1.+corr_fn(hgt(nn), lnsigl)*(covinf-1.))
!  if (i .eq. 1) print *,j,nn,deglat,hgt(nn),covinflate(nn,ij)
enddo
enddo
enddo
do nanal=1,nanals
...
! do gross background QC check.
ifailbg=0
do nob=1,nobstot
 if (iuseob(nob) .eq. 1) then
 if (abs(errcheck_min) .gt. 1.e-5 .and. abs(errcheck_max) .gt. 1.e-5) then
 yminushx = -obfit_prior(nob)
 if (yminushx .lt. errcheck_min .or. &
     yminushx .gt. errcheck_max .or. &
     ob(nob) .lt. 400. .or. ob(nob) .gt. 1100.) then
    iuseob(nob)=0
    ibadob(nob)=1
    ifailbg(nob)=1
 end if
 else
 if (abs(obfit_prior(nob)) .gt. sprd_tol*sqrt(obsprd_prior(nob)+oberrvar(nob)) &
     .or. (ob(nob) .lt. 400. .or. ob(nob) .gt. 1100.)) then
    iuseob(nob)=0
    ibadob(nob)=1
    ifailbg(nob)=1
 end if
...
! find indices for 'close' analysis points.
 ...
 if (deglat .gt. 25.) then
   ...
   corrl2 = ((25.-deglat)/10.)*corrlengthtr + ((deglat-15.)/10.)*corrlengthnh
 else if (deglat .lt. 15 .and. deglat .gt. -15.) then
   corrl2 = corrlengthtr
 else if (deglat .le. -15. .and. deglat .ge. -25.) then
   corrl2 = ((25.+deglat)/10.)*corrlengthtr + ((-deglat-15.)/10.)*corrlengthsh
 else
   corrl2 = corrlengthsh
 end if
 do ij=1,(nlons+1)*(nlats+2)
...
print *,count(ifailbud .gt. 0),' obs failed F-Test'

do nob=1,nobstot

 if (iuseob(nob) .eq. 1) then

 call obstats(obfit_prior(nob),oberrvar(nob),&
              obsprd_prior(nob),oblocy(nob),&
...
call printstats('ps',sumps_nh,biasps_nh,sumps_spread_nh,sumps_oberr_nh,nobsps_nh,&
              sumps_sh,biasps_sh,sumps_spread_sh,sumps_oberr_sh,nobsps_sh,&
              sumps_tr,biasps_tr,sumps_spread_tr,sumps_oberr_tr,nobsps_tr)
...
obsloop: do nob2=1,nobstot
      ...
      if (deglat .gt. 25.) then
         ...
         corrl = ((25.-deglat)/10.)*corrlengthtr + ((deglat-15.)/10.)*corrlengthnh
         ...
         corrl = ((25.+deglat)/10.)*corrlengthtr + ((-deglat-15.)/10.)*corrlengthsh
...
CALL WRSIG(IUNIT,FILENAME,LAB,0.0,IDATEOUT,SISL,HEADEXT,ZSNM,PSNM,VRTNM,DIVNM,&
           TEMPNM,QNM,NMDIM,NLEVS,NTRAC,NLONS,NLATS)
...
! only print statistics for obs that were not used.
! only use obs that were used in the assimilation (but may have
! been skipped by the F-test thinning algorithm).
! only use obs that are within +/-0.5h of the 6h fcst.
! if (obtype(nob) .eq. 'P' .and. ibadob(nob) .eq. 0 .and. (dtob(nob) .ge. 3.5 .and. dtob(nob) .le. 4.5)) then
! just compute posterior statistics if iuseob=1 and thinning turned on.
 if (sigtest_zscore .lt. 0.51 .or. iassim(nob) .eq. 0) then
 call obstats(obfit_post(nob),oberrvar(nob),&
              obsprd_post(nob),oblocy(nob),&
              sumps_nh,biasps_nh,sumps_spread_nh,sumps_oberr_nh,nobsps_nh,&
              sumps_sh,biasps_sh,sumps_spread_sh,sumps_oberr_sh,nobsps_sh,&
              sumps_tr,biasps_tr,sumps_spread_tr,sumps_oberr_tr,nobsps_tr)
 end if
...
enddo
if (sigtest_zscore .gt. 0.5) then
print *,'innovation statistics for posterior (using only good obs that were not assimilated):'
else
...
subroutine obstats(obfit,oberrvar,obsprd,oblocy,&
...
! compute innovation statistics in nh,sh,tropics.
 ...
 dtor = 4.*atan(1.0)/180.
 if (oblocy .gt. 20.*dtor) then
    if (nobs_nh .eq. 0) then
    sumfit_nh = obfit**2
    sumbias_nh = obfit
    sumspread_nh = obsprd
    ...
    nobs_nh = nobs_nh + 1
 else if (oblocy .lt. -20.*dtor) then
    if (nobs_sh .eq. 0) then
    sumfit_sh = obfit**2
    sumbias_sh = obfit
    sumspread_sh = obsprd
...
character (len=2) obtype
if (nobs_nh > 0) then
sum_nh = sum_nh/float(nobs_nh)
bias_nh = bias_nh/float(nobs_nh)
sum_oberr_nh = sum_oberr_nh/float(nobs_nh)
sum_spread_nh = sum_spread_nh/float(nobs_nh)
end if
if (nobs_sh > 0) then
sum_sh = sum_sh/float(nobs_sh)
bias_sh = bias_sh/float(nobs_sh)
sum_oberr_sh = sum_oberr_sh/float(nobs_sh)
sum_spread_sh = sum_spread_sh/float(nobs_sh)
end if
if (nobs_tr > 0) then
sum_tr = sum_tr/float(nobs_tr)
bias_tr = bias_tr/float(nobs_tr)
sum_oberr_tr = sum_oberr_tr/float(nobs_tr)
sum_spread_tr = sum_spread_tr/float(nobs_tr)
end if
if (nobs_nh > 0) then
print *, 'fit to ',nobs_nh,' nh ',obtype,' obs =  ',sqrt(sum_nh)
print *, 'bias for ',nobs_nh,' nh ',obtype,' obs =  ',bias_nh
print *, 'S+R,S at ',nobs_nh,' nh ',obtype,' ob locations =  ',sqrt(sum_spread_nh+sum_oberr_nh),sqrt(sum_spread_nh)
end if
if (nobs_sh > 0) then
print *, 'fit to ',nobs_sh,' sh ',obtype,' obs =  ',sqrt(sum_sh)
print *, 'bias for ',nobs_sh,' sh ',obtype,' obs =  ',bias_sh
print *, 'S+R,S at ',nobs_sh,' sh ',obtype,' ob locations =  ',sqrt(sum_spread_sh+sum_oberr_sh),sqrt(sum_spread_sh)
end if
if (nobs_tr > 0) then
print *, 'fit to ',nobs_tr,' tr ',obtype,' obs =  ',sqrt(sum_tr)
print *, 'bias for ',nobs_tr,' tr ',obtype,' obs =  ',bias_tr
print *, 'S+R,S at ',nobs_tr,' tr ',obtype,' ob locations =  ',sqrt(sum_spread_tr+sum_oberr_tr),sqrt(sum_spread_tr)
end if
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 ...
 SUBROUTINE WRSIG(IUNIT,FILENAME,LAB,FHOUR,IDATE,SISL,HEADEXT,ZSNM,PSNM,VRTNM,&
                  DIVNM,TEMPNM,QNM,NMDIM,NLEVS,NTRAC,NLONS,NLATS)
 real, dimension(44) :: headext
 real, dimension(201) :: sisl
 integer idate(4)
 real fhour
