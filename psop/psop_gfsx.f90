program psop
 ...
 real dxob,dyob,dtob,zerr,anal_obt,anal_obp,&
      delz_const,ensmean_ob,slpob,bias,preduce,palt,zthresh,zdiff,altob,errfact
...
!==> convert ob location to radians.
 ...
 do nob=1,nobstot
    ...
    anal_ob(nob) = &
    preduce(anal_ob(nob),anal_obp,anal_obt,zob(nob),anal_obz(nob))
...
real function preduce(ps,tpress,t,zmodel,zob)
...
! zob - station height
   ...
   implicit none
   real(r_kind), intent(in) :: t,tpress,zmodel,zob,ps
   real(r_kind) t0,alpha,tx,ty
   real(r_kind), parameter :: rlapse =0.0065
   ...
   t0 = t*(ps/tpress)**alpha ! eqn 4
   preduce = ps*((t0 + rlapse*(zob-zmodel))/t0)**(1./alpha) ! eqn 1
end function preduce

real function palt(ps,zs)
! compute QNH altimeter setting (in mb) given ps (in mb), zs (in m).
! see WAF, Sept 1998, p 833-850 (Pauley) section 2c
   use kinds, only: r_kind
   use constants, only: grav,rd
   real(r_kind), parameter :: rlapse = 0.0065
   real(r_kind), parameter :: t0 = 288.15
   real(r_kind), parameter :: p0 = 1013.25
   real(r_kind) alpha
   real,intent(in) :: ps,zs
   alpha = rd*rlapse/grav
