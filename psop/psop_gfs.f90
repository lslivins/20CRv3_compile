program psop
 ...
 integer iret,nlats,nlons,nlevs,ntrac,ntrunc,ierr,nanals,nfhr,nobstot,&
         ...
         nchar,nreal,ii,nn,nlevt,ntime,np,nobsh,izob,iunit_nml,iunito,idate
 ...
 namelist /nam_psop/nlevt,zerr,fhmin,fhmax,fhout,fhanal,datestring,rlapse,&
...
!==> convert ob location to radians.
 ...
 call addpolewrap(tempg(:,:,nlevt),analtemp(:,:,ntime),nlons,nlats)
 ...
 do nob=1,nobstot
    ...
    anal_ob(nob) = &
    preduce(anal_ob(nob),anal_obp,anal_obt,zob(nob),anal_obz(nob),rlapse)
